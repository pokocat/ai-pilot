...
export const NAV_SECTIONS: NavSection[] = [
  ...
  // 「短视频克隆定价」也在这一屏（页面第三段）：nav「配置」组已是 8 项，DESIGN.md 写着一组超 ~8 项
  // 就得拆组，为四个数字开第 9 项不划算；而本页的定位本就是平台级可写数值。别名要带上运营嘴里的叫法，
  // 否则那段配置只能靠滚页面撞见 —— 命令面板搜「数字人」「克隆」「钻石」都得能命中这里。
  { key: 'flags', label: '功能开关', hint: '合规一键降级、数值配置与短视频克隆定价', icon: 'shield', group: 'settings', aliases: ['开关', 'flag', '降级', '合规', '灰度', '告警', '阈值', '克隆定价', '克隆', '数字人', '声音', '音色', '短视频', '钻石单价', '训练价格', 'clone', 'pricing'] },
