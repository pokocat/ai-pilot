        ...
        {models.map((m) => (
          <div key={m.id} className="mem-card">
            ...
            <div className="mb" style={{ cursor: 'pointer' }} onClick={() => edit(m)}>
              ...
              <div className="mm">{m.provider} · {m.model || '—'}{modelSupportsThinking(m.provider, m.model) ? ` · Thinking:${m.thinkingMode}` : ''}{m.preset ? ` · 内置:${m.preset}` : ''}{(m.priceInput > 0 || m.priceOutput > 0) ? ` · 单价 入¥${m.priceInput}/出¥${m.priceOutput} 每1M` : ' · 单价待配'}</div>
            </div>
