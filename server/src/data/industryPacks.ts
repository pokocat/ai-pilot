...
export interface IndustryPack {
  ...
  // —— 深度字段（M4 PR-19，V6.0 §8；可选，先深度覆盖核心行业） ——
  decisionChain?: string;  // 客户决策链（从触达到复购的路径）
  ticketRange?: string;    // 客单价参考区间
  benchmarkCases?: string; // 行业标杆参考（仅作对标提示，不得据此编造客户数据）
  mingLink?: string;       // 天势关联：什么命格的人在这个行业容易做成（配合命盘表达铁律使用）
}
...
export const INDUSTRY_PACKS: IndustryPack[] = [
  ...
  },
  {
    ...
    name: '美业 / 医美',
    label: '美业 / 医美',
    aliases: ['美业', '医美', '美容', '美发', '美甲', 'spa', '护肤', '瘦身', '植发', '生活美容', '文绣', '皮肤管理'],
    persona:
      '你熟悉美业 / 医美的服务生意，理解获客靠内容与地推、成本高，靠会员复购与客单价支撑，重技师 / 医师依赖与口碑合规。',
    benchmark:
      '美业 / 医美典型基准：毛利率 60–80%、获客成本高（医美单客获客可达数百至数千元）、靠复购与会员储值回本，床位（椅位）效率与人效决定单店上限。',
    levers: [
      'LTV 主导：客单价 × 复购率 × 会员储值',
      '获客成本与转介绍占比（客户生命周期管理比拉新更重要）',
      '人效 / 床位（椅位）效率',
      '技师 / 医师依赖与标准化',
      '口碑与合规风险（医美尤甚）',
    ],
    glossary: '床位效率=每床位每日营收；储值=会员预存消费；复购率=老客重复消费比例。',
    decisionChain: '刷到内容 → 私域咨询 → 到店体验 → 办卡 → 复购 → 转介绍',
    ticketRange: '单次 300–2000 元（医美项目可上探至数万）',
    benchmarkCases: '文绣等垂类头部连锁的「内容获客 + 私域信任 + 高复购」模型',
    mingLink: '食神 / 伤官格擅长做服务体验与个人 IP；正财格适合做连锁标准化。',
  },
  {
    key: 'health',
    name: '大健康 / 养生',
    label: '大健康 / 养生',
    aliases: ['大健康', '养生', '中医', '理疗', '康复', '调理', '艾灸', '推拿', '健康管理', '口腔', '体检'],
    persona:
      '你熟悉大健康 / 养生的高信任生意，理解决策周期长、靠专业信任与效果口碑做高客单转化，重合规与长期服务关系。',
    benchmark:
      '大健康典型基准：高客单（单疗程数千到数万）、决策周期长、信任建立成本高；复购与长期服务协议决定 LTV，合规是生存底线。',
    levers: [
      '信任建立路径：专业内容 + 案例见证 + 体验环节',
      '高客单转化率与决策周期',
      '长期服务协议与复购（LTV 主导）',
      '医师 / 调理师专业背书与标准化',
      '合规风险（宣传口径尤其敏感）',
    ],
    glossary: '疗程=按周期打包的服务组合；LTV=客户生命周期总价值。',
    decisionChain: '痛点触发 → 信息搜索 → 信任建立 → 体验 → 高客单转化 → 长期服务',
    ticketRange: '单疗程 5000 元–10 万元',
    benchmarkCases: '大健康头部连锁的「专业信任 + 长期服务」模型',
    mingLink: '正印格适合专业品牌与知识背书路线；正官格适合合规稳健的连锁经营。',
  },
  {
    key: 'education',
    name: '教育 / 培训 / 知识付费',
