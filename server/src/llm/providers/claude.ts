...
export async function claudeDeliverable(ctx: GenContext, cfg: ResolvedAiConfig): Promise<Deliverable> {
...
export async function claudeChat(ctx: GenContext, cfg: ResolvedAiConfig): Promise<ChatReply> {
  const system = injectVariables(ctx.systemPrompt, ctx);
