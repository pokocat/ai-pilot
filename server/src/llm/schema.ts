...
/**
 * 普通对话的通用体例约束（追加在运营配置的 system prompt 之后）。此前这段字符串在
 * claude.ts / openai.ts 里散着 6 份复制粘贴，改一处必漏五处，故收到 SSOT。
 *
 * **长度契约是这里最要紧的一条**：`max_tokens` 是模型看不见的硬闸刀，没有任何 API 参数能让模型
 * 自觉写短（`effort` 调的是思考深度、不可靠地影响正文；`task_budget` 是 Anthropic 一方 beta，
 * 三方兼容网关不透传）。唯一可靠的自我收敛手段就是提示词。所以宁可让模型先给纲要再问用户展开，
