...
const LABEL_LABELS: Record<string, string> = {
  ...
  endpoint: '端点 ID', label: '端点', purpose: '在线用途', kind: '检测项',
};
...
export const ALERT_KNOWLEDGE: Record<string, AlertKnowledge> = {
  ...
  JunshiAiEndpointProbeFailing: K('上游端点检测连续失败', '在线端点的最新定时检测持续失败 25 分钟；恢复后继续保持 15 分钟再解除', '告警已精确到端点、在线用途和检测项；对应能力可能在用户下一次使用时失败。', '按检测项处理：连通性查网络或 Key，嵌入/重排查对应专用接口，Thinking 查协议方言，模型范围查 Key 授权。'),
  ...
  JunshiPayReconcileDelayed: K('支付对账任务未按时完成', '库里有待处理订单，且支付对账连续 15 分钟未成功', '订单终态可能无法及时收敛；已支未发会另外触发更高等级的资损告警。', '先查待处理订单状态，再检查支付对账任务的最近失败和数据库连接等待。'),
  ...
  JunshiNoRegistrations72h: K('正式开放后 72 小时无新注册', '“正式开放增长监控”已开启，且数据库最近 72 小时新用户为 0', '正式推广期可能意味着微信登录、短信或落地链路静默故障。', '核对最近注册时间和投放流量，再分渠道做不落正式账号的登录链路探测。'),
  JunshiSchedulerJobStalled: K('定时任务连续未完成', '已超过 max(3 个运行周期, 15 分钟)没有成功，并持续 5 分钟', '相应的自动补偿、通知或维护动作可能延迟；实际业务风险由各领域事实告警另行升级。', '按任务名查最近失败日志、运行时长与数据库连接等待；不要因为某个业务尚未开放就静音整个调度器。'),
