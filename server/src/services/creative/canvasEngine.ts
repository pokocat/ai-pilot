 ...
 *
 * 6000 → 12000 → 6000，两次都被实测纠回来，值得写清楚免得下次又调大：
 * · 调大的动机是「一页有密度的 HTML 写不下」——但实测产物只有 11–13k **字符**（≈3.5–4k token），
 *   6000 从来没有截断过它。也就是说那个动机本身站不住。
 * · 而开着 adaptive 思考时，**把上限调大等于允许模型想更久**：12000 正文 + 7000 思考 = 19000 的顶，
 *   单轮挂钟从约 95s 涨到超过 240s（预发实测直接顶穿超时）。多花的时间没有换来更密的版面，
 *   只换来更长的思考。
 * 结论：这一格该给的是**够用的正文**，不是「越多越好」。
 */
