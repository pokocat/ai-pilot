...
//   ② **带默认值的槽位在拼装时去掉花括号只留内容**（`{WARDROBE: 象牙白开司米}` → `象牙白开司米`）。
//      把字面 `{}` 发给生图模型没有意义；原文的花括号是给人看的「这里可替换」标记。
//      因此本文件里每个槽位都**必须带内容**（`{LABEL: …}` 形态），不允许出现光秃秃的 `{LABEL}`
//      —— 完整性单测钉住这一条。
//
// 三个调研发现在本文件的落点（另两个在 imagePrompt.ts）：
//   · `negativeSpaceClause` 是**必填**字段，且拼装器把它拼进 prompt **正文**（不是只放负向框）——
//     「声明了留白但没有显式禁入侵子句」是海报与普通图的唯一分界，模型会把阴影边缘、道具轮廓、
//     渐变条带塞进留白区。
//   · `negatives` 只写**风格专属忌讳**；通用基座在 imagePrompt.ts 的 BASE_NEGATIVES，拼装时合并。
//     两处分开是为了让「加一档」不必抄一遍 20 个通用负向词。
...
export interface PosterStyle {
  ...
  /** 文字安全区机器标识（渲染层定位用）。 */
  safeZone: SafeZone;
  /** prompt 骨架，含 `{SUBJECT}` 槽与若干 `{LABEL: 内容}` 槽（拼装时去括号留内容）。 */
  imagePromptSkeleton: string;
  /** 负空间显式禁入侵子句。**必填**，拼装器把它放进 prompt 正文。 */
  negativeSpaceClause: string;
  /** 风格专属忌讳（通用基座见 imagePrompt.BASE_NEGATIVES，拼装时合并去重）。 */
  negatives: string[];
  /** 给排版层的字体/字重/走向建议（进 canvasEngine 的 photo 变体提示词）。 */
  typographyHints: string;
  /** 3–5 色，供排版层取标题色与色块（必须是 #RRGGBB）。 */
  palette: string[];
 ...
 * 每条 `imagePromptSkeleton` 末尾**刻意不写** `no text anywhere in the image` 与 `3:4` ——
 * 那两句由拼装器统一追加在最后（调研发现②：no-text 必须是正文最后一句，写在骨架里就会被
 * 负空间子句挤到中间）。骨架里若混进 no-text，完整性单测会红。
 */
export const POSTER_STYLES: Record<PosterStyleKey, PosterStyle> = {
  quiet_luxury_grey: {
    ...
    safeZone: 'left_third',
    imagePromptSkeleton:
      'Full-bleed editorial photograph of {SUBJECT}, wearing {WARDROBE: undyed ivory or oatmeal cashmere, no visible branding}, '
      + '{ACTION: seated at a low stone table, hands resting on a go board}. Environment: a vast near-empty gallery-like room, '
      + 'warm grey micro-cement walls, one linen curtain, a single sculptural chair, nothing else. '
      + 'Lighting: broad soft window light from camera right, two-stop falloff into the corners, gentle wraparound shadow, '
      + 'no hard specular highlights. Camera: medium format 80mm, eye level, waist-up framing, subject placed in the right third. '
      + 'Finish: high-key desaturated grade with warm undertone, fine natural skin texture retained, editorial retouch, subtle 400-speed grain.',
    negativeSpaceClause:
      'The entire left third of the frame is bare wall — one flat even tone, no props, no furniture edge, no shadow line '
      + 'and no surface texture detail crossing into it, held clear as typography space.',
    negatives: [
  ...
  baroque_icon_gold: {
    ...
    safeZone: 'bottom_35',
    imagePromptSkeleton:
      'Baroque devotional oil painting of {SUBJECT}, {WARDROBE: heavy crimson and deep-green velvet with gold thread embroidery}, '
      + 'holding {PROP: a white lamb}, a thin gilded halo behind the head. Environment: a near-black umber void, '
      + 'one fold of dark drapery at the upper left. Lighting: single candlelit key high from camera left at 45 degrees, '
      + 'hard chiaroscuro, deep occluded shadow, warm gold rim on cheekbone and shoulder. '
      + 'Composition: centred half-length figure, head in the upper third, canvas weave and fine craquelure visible, aged varnish. '
      + 'Finish: Caravaggio-school modelling, sfumato skin transitions, museum oil texture.',
    negativeSpaceClause:
      'The bottom 35% of the frame falls away into near-black empty shadow — no fabric detail, no hands, no props, '
      + 'no visible canvas texture — one flat dark plane reserved for the title block.',
    negatives: [
  ...
  editorial_black_gold: {
    ...
    safeZone: 'top_40',
    imagePromptSkeleton:
      'High-contrast editorial photograph of {SUBJECT} in {WARDROBE: a sharply tailored black suit}, '
      + '{ACTION: hands in pockets, weight on one leg}. Environment: black seamless studio backdrop, '
      + 'a single brushed-brass vertical element just behind the shoulder. '
      + 'Lighting: hard focused key from top left through a small softbox, strong low-key falloff, '
      + 'a thin warm gold rim separating the shoulder from the black, controlled specular on the metal only. '
      + 'Camera: 85mm at f/4, eye level, chest-up framing, subject sitting in the lower right. '
      + 'Finish: rich true black point, gold-toned highlights, crisp commercial retouch, no ambient fill.',
    negativeSpaceClause:
      'The upper left 40% of the frame stays pure unbroken black with zero detail, no light spill and no gradient banding '
      + '— a solid field for headline type.',
    negatives: [
      'muddy grey blacks', 'gradient banding', 'lens flare crossing the empty area', 'busy props',
  ...
  neo_chinese_void: {
    ...
    imagePromptSkeleton:
      'Oriental minimalist photograph of {SUBJECT} in {WARDROBE: a loose raw-silk tea robe in bone white}, '
      + '{ACTION: pouring tea, holding a brush}. Environment: a rice-paper screen wall, one bare plum branch, '
      + 'a slate stone surface, everything else emptied out. Lighting: pale diffused north light, very low contrast, '
      + 'soft ink-like shadow pooling directly under the objects only. '
      + 'Composition: 40mm, eye level, small subject placed in the lower right, vast empty upper field, '
      + 'xieyi-style asymmetric balance, a faint ink-wash gradient bleeding down from the top edge. '
      + 'Finish: low saturation celadon and mist grey, fine paper grain, still air.',
    negativeSpaceClause:
      'The upper two thirds is empty rice-paper white carrying only a faint ink gradient — no branches, no seal marks, '
      + 'no wall texture and no shadow intruding — held clear for vertical calligraphic type.',
    negatives: [
  ...
  documentary_film_grain: {
    ...
    imagePromptSkeleton:
      'Candid documentary photograph of {SUBJECT} at work, {ACTION: mid-gesture, unaware of the camera}, '
      + 'inside {ENV: their own workshop, kitchen, or shopfront}, real working clutter pushed to one side of the frame. '
      + 'Lighting: available light only — late afternoon sun through a dusty window, a hot highlight on the forearm, '
      + 'open shade filling the face. Camera: 35mm at f/2, slight motion blur at the hands, shot slightly off-axis, medium shot. '
      + 'Finish: Kodak Portra 400 emulation, organic grain, warm true skin tones, mild halation, '
      + 'unretouched pores and flyaway hair, RAW look.',
    negativeSpaceClause:
  ...
  luxury_magazine_cover: {
    ...
    safeZone: 'top_30',
    imagePromptSkeleton:
      'Fashion cover photograph of {SUBJECT} in {WARDROBE: a monochrome sculptural coat}, still confident posture, chin level. '
      + 'Environment: a seamless paper backdrop in {BACKDROP_COLOUR: warm bone white}, one deliberate hard cast shadow on the paper. '
      + 'Lighting: beauty dish key slightly above and on axis, tight soft key with a crisp shadow under the jaw, '
      + 'subtle silver rim from behind, exactly one hard shadow in the scene. '
      + 'Camera: medium format 110mm at f/8, eye level, three-quarter body, subject centred but shifted low in the frame. '
      + 'Finish: flawless editorial skin retouch that still keeps texture, saturated but controlled colour, tack-sharp fabric weave.',
    negativeSpaceClause:
      'The top 30% of the seamless backdrop is left completely clean and evenly lit — no shadow gradient, '
      + 'no hair strays crossing it, no vignette — the masthead zone.',
    negatives: [
  ...
  airy_japanese_light: {
    ...
    safeZone: 'top_40',
    imagePromptSkeleton:
      'Soft natural-light photograph of {SUBJECT}, gentle unposed expression, {WARDROBE: a white cotton shirt}, '
      + '{ACTION: looking slightly away, holding a glass}. Environment: a bright pale room, one sheer white curtain, '
      + 'a single glass vase, everything washing toward white. Lighting: backlit by a hazy window, '
      + 'blown-out highlight bloom behind the head, soft veiling flare, almost no shadow contrast, high key. '
      + 'Camera: 50mm at f/1.8, slight lens haze, shallow focus, medium close-up, subject sitting low in the frame. '
      + 'Finish: Fujifilm Pro 400H emulation, cool-clean cast, low contrast, fine grain, airy.',
    negativeSpaceClause:
      'The upper 40% washes to a near-white empty gradient with no objects, no curtain folds and no window mullions '
      + '— soft and even enough to carry light-weight type.',
    negatives: [
  ...
  cyber_tech_blue: {
    ...
    safeZone: 'left_third',
    imagePromptSkeleton:
      'Cinematic tech portrait of {SUBJECT} in {WARDROBE: a dark technical jacket}, arms folded, calm and still. '
      + 'Environment: a dark data-hall corridor, out-of-focus cool blue light bars receding into depth, '
      + 'faint volumetric haze, a reflective dark floor. Lighting: cyan practical rim from behind camera left, '
      + 'cool blue fill, one small warm accent to keep the skin alive, low-key with clean blacks. '
      + 'Camera: 50mm at f/2.2, slightly low angle, chest-up framing, subject in the right third. '
      + 'Finish: deep navy-to-black gradient, restrained glow, crisp micro-contrast, no grain.',
    negativeSpaceClause:
      'The left 40% is a smooth dark navy gradient with a single soft glow falloff — no light bars, no cables, '
      + 'no HUD elements, no particles — a quiet field for the headline.',
    negatives: [
  ...
  retro_hongkong: {
    ...
    imagePromptSkeleton:
      ...
      + 'leaning at {ENV: a tiled stair landing, a rain-wet alley, a diner booth}. '
      + 'Lighting: mixed practicals — a green fluorescent tube overhead, magenta neon spill from the right, '
      + 'warm tungsten from below, heavy colour crosstalk across the skin. '
      + 'Camera: anamorphic 40mm at f/2, framed through a doorway, slight dutch tilt, medium shot. '
      + 'Finish: expired-film colour shift, crushed teal shadows, visible grain and gate weave, '
      + 'faint vertical smear on the highlights, nostalgic.',
    negativeSpaceClause:
      'An unlit dark wall panel occupies the left edge for a third of the frame width — deep, flat and detail-free, '
      + 'no signage and no neon spill reaching it — so vertical type can sit on it.',
    negatives: [
  ...
  glossy_3d_trend: {
    ...
    safeZone: 'top_40',
    imagePromptSkeleton:
      'Studio 3D render: {SUBJECT} as a single hero object floating slightly above a soft curved backdrop, '
      + '{PROP: a few small geometric props} orbiting it at a distance. '
      + 'Materials: glossy soft-touch plastic, frosted glass, one brushed chrome accent, subtle subsurface glow. '
      + 'Lighting: large HDRI studio softbox from the front top, two coloured gradient kickers left and right, '
      + 'a clean contact shadow beneath the hero, no blown speculars. '
      + 'Composition: hero centred low in the frame, generous headroom, wide gently curved backdrop. '
      + 'Finish: Octane-grade clean render, physically accurate reflections, gentle bloom, candy-clean surfaces.',
    negativeSpaceClause:
      'The curved backdrop above the hero stays an unbroken soft colour gradient across the top 40% — no floating props, '
      + 'no cast shadows, no reflections and no orbiting elements enter it.',
    negatives: [
  ...
  mono_authority_portrait: {
    ...
    imagePromptSkeleton:
      ...
      + 'hands loosely visible. Environment: a mid-grey painted wall, nothing else in the scene. '
      + 'Lighting: one large softbox at camera left 45 degrees with a black flag on the right side, '
      + 'deep unfilled shadow side, a catchlight in both eyes, full tonal range from paper white to true black. '
      + 'Camera: 85mm at f/2.8, eye level, tight head-and-shoulders, subject occupying the right half. '
      + 'Finish: silver-gelatin tonality, real skin texture and lines kept, no smoothing, fine grain, timeless and unadorned.',
    negativeSpaceClause:
      'The left half is smooth mid-grey wall falling gently to shadow, entirely featureless — no texture, '
  ...
  surreal_object_metaphor: {
    ...
    imagePromptSkeleton:
      'Conceptual still-life photograph: {SUBJECT} as the only subject, placed on a wide seamless surface. '
      + 'Environment: a large empty studio field in {BACKDROP_COLOUR: warm bone grey}, one long soft cast shadow, '
      + 'faint dust in the air. Lighting: a single hard sun-like source high from camera right, long directional shadow, '
      + 'gentle gradient falloff across the background, quiet drama. '
      + 'Camera: 100mm macro at f/8, slightly above eye level, the object small in the lower third, exaggerated emptiness above it. '
      + 'Finish: gallery still-life realism, matte surfaces, museum-clean, no distracting reflections.',
    negativeSpaceClause:
      "Everything above the lower third is empty graded background — the object's cast shadow must not enter it, "
      + 'no secondary props, no dust concentration, no vignette at the edges.',
    negatives: [
 ...
 *   · `product` → 3D 潮流：唯一以「物件」为主角的档（skeleton 也写的是 hero object），
 *     产品海报套人像档必然违和。
 */
