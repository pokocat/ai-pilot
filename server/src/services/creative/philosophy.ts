...
const TEMPLATE_HINT: Record<TemplateKey, string> = {
  ...
  manifesto_min: '这一版只放一句主张：哲学要交代留白如何承担全部语气，以及那一句字该有多大的重量。',
  quote_card: '这一版是金句卡：哲学要交代引号排印的分寸，以及署名如何让这句话有出处。',
  data_stat: '这一版由一个关键数字撑场：哲学要交代数字如何成为图形，其余信息如何退到它身后。',
  info_list: '这一版是要点清单：哲学要交代密集信息的节奏——编号、间距与对齐如何替代装饰。',
  agenda_event: '这一版是活动信息：哲学要交代时间地点议程的层级，以及行动区如何显著到不用找。',
};
