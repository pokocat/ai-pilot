...
// 放行条件（任一）：
//   1) 共享密钥：请求头 x-admin-token 或 Authorization: Bearer <token> 命中环境变量 ADMIN_TOKEN（常量时间比对）；
//   2) 管理员账号：x-user-id 解析到 role='admin' 的用户。
// 否则：已登录的非管理员 → 403；无任何凭证 → 401。
//
// 说明：ADMIN_TOKEN 在「请求时」直接读 process.env（而非启动期缓存），便于密钥轮换与测试注入。
//      未配置 ADMIN_TOKEN 时，仅放行 role='admin' 账号（安全默认）；演示环境请在 .env 配置 ADMIN_TOKEN。
