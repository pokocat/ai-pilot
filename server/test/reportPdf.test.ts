...
describe('reportPdf 服务（纯函数）', () => {
  ...
  test('缓存 key 确定性推导：{prefix?}pdf/{id}-long-m.pdf（手机宽单页长 PDF 版本位，与旧缓存分离）', () => {
    // 测试环境未配 OSS 前缀 → 无前缀。-long-m 后缀确保旧分页版 pdf/{id}.pdf 与桌面宽 -long.pdf 都不会被命中。
    assert.equal(reportPdfKey('abc123'), 'pdf/abc123-long-m.pdf');
  });
