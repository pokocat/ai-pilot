...
test('本地案卷导入：无案卷时导入军令与回填；已有活跃案卷则幂等跳过', async () => {
  // 用「昨天」保证落在 casefileView 的 14 天窗口内（写死日期会随时间腐化断言）。
  const y = new Date(Date.now() - 86400_000);
  const recentDate = `${y.getFullYear()}-${`${y.getMonth() + 1}`.padStart(2, '0')}-${`${y.getDate()}`.padStart(2, '0')}`;
  const localDossier = {
    title: '本地迁移案卷',
    sourceAgent: '军师',
    judgment: '先修信任链路',
    risks: ['不要扩渠道'],
    orders: [
      { text: '发布一条观点短视频', from: 'IP 军师', tag: '军令 · IP', date: recentDate, done: true },
      { text: '', date: recentDate }, // 空文本应被过滤
      { text: '坏日期', date: '07/01' }, // 非法日期应被过滤
    ],
    backfill: { [recentDate]: { leads: '8', consults: '2', deals: '0' } },
  };
  // other 用户无案卷 → 导入成功
  ...
  assert.equal(r.body.casefile.backfill[recentDate].leads, '8');
...
test('能力直达打标（v1.3 §五）：含「拍一支企业宣传片」的军令 accept 后 capabilityKey=promo；手动军令不打标', async () => {
  const capUser = await login(uniquePhone(), '打标用户');
  const CAP_PLAN = deliverable('品牌传播方案', [
    { h: '现状判断', b: '品牌声量不足，先立信任资产。' },
    { h: '30 天行动清单', list: ['拍一支企业宣传片，讲清客户的改变', '本周整理 3 组客户证言', '设计一张主视觉海报用于门店物料'] },
  ]);
  const r = await api('POST', '/api/casefile/accept', { token: capUser, body: { deliverable: CAP_PLAN, agentName: '军师' } });
  assert.equal(r.status, 200);
  const orders = r.body.casefile.orders as { text: string; capabilityKey: string | null }[];
  const promo = orders.find((o) => /宣传片/.test(o.text));
  assert.equal(promo?.capabilityKey, 'promo');
  const poster = orders.find((o) => /海报|主视觉/.test(o.text));
  assert.equal(poster?.capabilityKey, 'poster');
  const plain = orders.find((o) => /客户证言/.test(o.text));
  assert.equal(plain?.capabilityKey, null, '未命中关键词的军令不打标');

  // 手动自定军令（fromAgent='我'）保持 null，即使文本含关键词
  const manual = await api('POST', '/api/casefile/orders', { token: capUser, body: { text: '自己剪一条短视频发抖音' } });
  assert.equal(manual.status, 200);
  const manualOrder = (manual.body.casefile.orders as { from: string; text: string; capabilityKey: string | null }[])
    .find((o) => o.from === '我');
  assert.equal(manualOrder?.capabilityKey, null, '手动军令不打标');
});
