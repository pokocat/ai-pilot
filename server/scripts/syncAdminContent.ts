...
//
// ── 为什么默认不覆盖提示词 ──
// 2026-07-28 复核：早先把 UTF-8 字节数与字符数混在一起，误报成「漂移 2.85 倍」。同一口径下，
// `general` 线上是 49,094 字节 / 19,486 字符，仓库是 44,959 字节 / 17,232 字符，实际差异
// 为 +9.2%（字节）/ +13.1%（字符）；其余 9 个专业 agent 当时逐字节一致。提示词仍然必须视为
// 运营资产：`general` 的真实字符比 17,232 / 19,486 = 0.884，高于 0.8 护栏，误用 force 会直接写入。
//
// 原实现的 update 分支无条件写 `systemPrompt: a.systemPrompt`，无 diff、无确认。运行时读的是
// AgentVersion 已发布快照，所以同步不会立刻生效——但草稿被换成旧版后，之后任何一次「发布」
// 都会生成错误的新版本并切给 C 端。旧版本仍可回滚，但草稿里的线上增量要靠变更前备份恢复。
//
