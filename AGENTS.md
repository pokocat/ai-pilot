...
7. **登录态失效必须显式打断，游客未登录不得误伤（全局铁律）**：`app/src/services/api.ts` 的 `request()` 与文件上传会在发请求前捕获 `tokenAtRequest`。收到 401 时，**请求原本带 token 且当前 storage 仍是同一个 token**，才由第一条响应触发全局 `onAuthLost`（`store.ts` 清登录态 + 提示「登录态已失效，请重新登录」+ `reLaunch` 回 `pages/sessions`）；同一旧 token 的其余并发 401 标 `staleAuth`，页面 catch 不得再次清状态、提示或跳路由，旧请求晚于一次新登录返回时也绝不能清掉新 token。**请求原本无 token**只抛 `{ code:'UNAUTHORIZED', hadToken:false }`，由动作级登录门在原页面承接，不提示“登录态失效”、不清状态、不跳路由。页面 `.catch` 只负责本地非鉴权兜底；请求层已标 `authHandled/staleAuth` 的 401 只收口错误，不重复执行全局退出；新增鉴权调用优先经 `request()`，面向用户的错误走 `store.handleApiError`。**历史坑**：军师记忆库 / 完整履历页曾吞 401；游客态上线后若把无 token 401 也全局打断，则会把正常浏览误报成掉登录；「今日」tab 并发拉取曾让同一失效 token 连续多次 reLaunch，看起来像小程序闪退。
8. **小程序改动先查约束清单**：凡改 `app/` 的微信小程序页面、tabbar、弹层、登录、键盘、网络请求、路由分包或项目配置，先对照 **§7.2 小程序工程约束清单**；不确定时按清单保守实现，避免回退真机已修复问题。
9. **运营后台 UI 改动守设计系统**：凡改 `admin/` 前端（`.tsx`/`admin.css`），必须对齐 **`admin/DESIGN.md`「Engineering Compliance」**——颜色只用 `:root` token（禁硬编码 hex/rgb）、只用已定义的组件类（禁裸 class 与一次性 inline 控件样式）、z 轴只用 `--z-*`。提交前跑 `cd admin && npm run lint:ui`（`scripts/audit-admin-ui.mjs`，**递归扫描** `admin/src/**/*.tsx`，已接入 `build`）保持全绿。另外五条硬约束：① 新页面先在 `admin/src/nav.ts` 登记（归属 7 个场景组之一 + 一句话 hint + 命令面板别名），页面本身不再手写顶层 `sec-h`，标题走 `PageHead k="<section>"`；② 取数一律 `useResource` + `ViewState`，**禁止 `.catch(() => {})`**——请求失败必须与「确实没数据」区分开且可重试；写操作的 `catch` 必须透出 `e.message`（**禁止盖成固定文案**），否则 403「需要 owner 权限」会被说成「保存失败」，见 §9 的 401/403 分流；③ 破坏性/资金动作用 `ConfirmDialog`（回显对象 + 必要时手打确认词），**禁止 `window.confirm/prompt/alert`**；④ 开关必须用共享 `Switch`，弹层必须用 `useDialogFocus` 管焦点/Tab/Escape/焦点归还，禁止 `div.sw` 与按钮内嵌交互节点；⑤ 业务视图放 `admin/src/views/<组>.tsx`，`App.tsx` 只留外壳。
...
- **快出片独立产品规划 SSOT（2026-08-22）**：`docs/VIDEO_STANDALONE_MINIAPP_PLAN_2026-08-19.md` 是当前唯一产品事实源；`docs/prototypes/VIDEO_STANDALONE_MOBILE_FLOW_V1.svg` 与 `V2.svg` 均为未采纳探索稿，不得作为产品评审、设计验收或开发依据，原型迭代暂停。定位不再是“军师拆出的 AI 视频工具”，而是**独立运营、模板驱动、数字资产驱动、可被多 APP 调用的视频内容生产平台**；军师只是首个 Connector。首期只做“数字人口播/叙事”和“数字人带货”两条模板主链，独立端一级导航为“创作｜资料库｜作品”，并把商品、剧本、数字人/声音、媒体、模板、项目/任务/作品及 `CreationRequest` 作为核心对象。全片以低成本模板混剪为主：底层 `TimelineSlot` 可由多个 `SlotItem` 填充，优先用户/官方/图片动效/数字人素材，付费 AI 视频只补高价值片段和明确缺口；模板时长默认 `min/ideal/max` 弹性适配。这些底层选择不得平铺给手机用户，但“简单”也不得牺牲结果知情权。预览成本不能等同于生成成本：默认由手机按真实时间线即时连播并叠加字幕/声音，后台以自有 FFmpeg 自动升级低清代理；尚未生产的数字人/AI 槽位使用等时长示意并按秒披露，只有用户主动查看真实效果才提前付费生成，已生成 `PreparedClip` 必须用于最终成片。确认任务仍强校验 `PreviewRender.timelineHash`：确定性装配保持一致，含示意时只生产已披露缺失片段。此条目前是方案决策和文档化交互规格，尚未授权据此改契约或实现；后续相关讨论和落地必须持续回填该文档。
- **快出片闸门协议与出口判据（2026-08-24）**：`docs/KUAICHUPIAN_GATE_PROTOCOL_2026-08-24.md` 是方案 §11.0 两道前置闸门的执行细则（机型清单、网络档位、冷热缓存口径、样本量、P95 算法、证据链、「无明显黑场」的可判定定义），以及 P0-a 五条出口判据的可验证版。两道闸门本身就是开发量（闸门 A 约 6～8 人日、B 约 4～6 人日），方案原文「各半天到一天」已作废。文档 §2.5 与 §3.3 两条判据修订建议、§4 的出口判据可验证版**都是提案，待拍板后才回填方案正文**，不得直接当已决定引用。
- **快出片可点击原型（2026-08-24）**：`docs/prototypes/v3-canvas/`（`*.dc.html` + `canvas.json`，13 块画板）是方案的可视化，**不是独立事实源**，与方案冲突以方案为准。所有模板数字必须取 `packages/video/catalog.js` 的 `listBuiltInTemplates()` 运行时真值（`ct_shiti`：163 秒 / 22 段 / 数字人 32 秒即第 1、7、13、21 段 / 35 钻石），文件里手写的 162 / 38 / 68 是设计稿残留，会被 `templateMeta()` 重算覆盖。界面里不得出现任何 T2V/I2V 形态（AI 空镜、AI 生成画面、AI 补 N 秒）——已整块移出 P0。`seed-canvas.mjs` 打出的 2.4MB 自包含包已 gitignore，不入库。
- **快出片声音训练模型口径（2026-08-18）**：声音相关 clone 请求由军师 BFF 向 AIStar 明确传 `model: "2.0"`；照片分身不创建声音，因此不传该字段。原有“V2 speaker/TTS”表述是历史协议简称，实际训练模型字段以 `2.0` 为准。
- **快出片成片下载**：成片详情页保存必须直接调用军师同源 `/api/video/works/:id/file`，由 BFF 每次刷新 AIStar 短签名；不得先用详情响应里的 `videoUrl` 做前置门槛，因为它可能缺失或已过期。
...
- **「快出片」本人素材上传与成片保存（2026-08-18）**：此前“禁止 direct”针对把军师 JWT/服务 token 发给 AIStar 或第三方；唯一允许的直传是军师签发、AIStar OSS 接收的短时 PostObject V4 票据，必须精确限制单一 object key、字节数、MIME 和最长 10 分钟，长期密钥/JWT/service token 均不得下发。客户端只上传一次，后续按 uploadId 查询 `issued|uploaded|processing|accepted|failed`；owner + clientRequestId 是幂等真源，超时和重复点击不得换号重传。AIStar 对 HEVC/H.265 形象视频异步转 H.264/AAC 后再深检。成片保存必须走鉴权同源 `/api/video/works/:id/file` 刷新上游签名并流式下载，检查 HTTP 状态、真实进度与 `scope.writePhotosAlbum` 拒绝/恢复，禁止继续直接把 OSS 短签名交给 `wx.downloadFile`。
- **「快出片」出片前的三层预览（2026-08-18）**：预览被拆成三条互不依赖的链路，**不许用低层替高层签字**：①画面顺序 = 端上素材连播（静音、无字幕、以 `bindplay` 为计时起点）；②声音 = 声音训练完成时固化的样例 TTS（`DapVoice.demoAudioCdnKey`）；③口型与构图 = 形象训练完成时固化的带声音短片（`DapAvatar.demoVideoCdnKey`）。两份固化产物由 `ClipDemoWorker` 扫表补齐生成，**不得改成在 `engineStatus` 变 ready 那一刻直接生成** —— 置 ready 的地方有五处，逐个挂钩必漏，漏掉的那条路径上的用户就永远看不到样例。生成失败一律不阻断训练成功，`demoAttempts` 到 `MAX_ATTEMPTS` 即放弃；「形象还没有可用声音」返回 false 但**不消耗次数**。样例文案 `ClipDemoService.DEMO_TEXT` 与端上 `voice-preview` 的 `SAMPLE` 必须一致，否则端上快路径判定失效、每次都白走一趟合成。端上规则：样例句且已固化 → 直接播固化产物（零等待、零供应商调用）；改过一个字 → 落回 `POST /me/clip/voices/{id}/preview` 按需合成。mock 态**故意不给假 URL**，为的是让「没固化好 → 回落合成」这条真实分支被验到。**`dap_voice` / `dap_avatar` 不在 Flyway 基线里**，它们的列一律由 `ddl-auto=update` 从实体定义补齐；给这两张表写迁移文件会在新库上撞「表不存在」（Flyway 跑在 Hibernate 建表之前），生产 MySQL 还不认 `ADD COLUMN IF NOT EXISTS` 与 partial index。石榴 `createVideoByText` 是**异步**的（恒返回 `processing`，任务号在 `task.id()`），样例短片必须存 `demoVideoTaskRef` 再轮询 `query()`，attempts 只在提交时消耗；写成一步同步取结果 = 每轮重新提交、烧满次数还拿不到片子。认领必须走 `ClipDemoClaims` 的 `REQUIRES_NEW` 独立事务，与供应商调用同事务会被异常回滚掉。分身关联声音的真值是 `avatar.voiceName`（voice id），`voice.avatarId` 只在「从形象视频提原声」时有值，只看后者会让所有复用共享声音的分身永远拿不到样例。重训声音 / 换形象 / 改关联声音 / 删除分身都必须作废并清理旧样例产物 —— 旧 key 非空会让 worker 以为「已经有了」，用户看到的是一个看起来正确的错误样例。全片预览里的封面只在 `cover.enabled === true` 时出现，并必须写明它在成片里只占第一帧、不占正片时长（`COVER_DURATION_SEC` 仍是 0.04，不得为了「看得见」去改它）。
- **「快出片」幂等、状态机与试听（2026-08-18）**：请求标识只有在服务端明确判定「这一单已结束」时才允许换新。`CLIP_RENDER_CREATING`、`CLIP_CLONE_CREATING`、`CLIP_CLONE_ACCEPTING` 一律保留原标识——换号等于再建一笔 hold 和一个上游任务，克隆还会让用户重传整个文件；纯网络失败（无 statusCode）同样保留。出片确认页的 `renderRequestId` 必须在拿到可换号的判决后重新生成，禁止再回到「onLoad 生成一次、失败后永不更新」。`ClipProject.status` 表达的是「还能不能编辑」而不是任务成败：worker 失败与用户取消都必须经 `ClipRenderService.releaseProject` 回落 `draft`，且只有该项目当前最新的一条 job 才有权重置（旧 job 不得抢走已被新 job 占用的项目）。`VideoCreditHold` 状态单向：`refunded` 是终点，任何路径都不得把它写回 `settled`。出片完成通知走 `clip` 场景并复用「服务进度通知」模板（不申请新模板），推送幂等靠 hold 上 `submitted → notifying` 的一次原子认领；该场景必须由端上 `requestSubscribeMessage` + `recordSubscribeChoice({scene:'clip'})` 申请，`avatar` 的额度不能被 `clip` 命中；H5 的 `ACCEPT_TOAST` 必须继续以 `Record<WechatSubscribeScene, string>` 穷举全部场景，`clip` 成功口径为「已订阅一次成片完成提醒」。声音试听是 `POST /me/clip/voices/{id}/preview`，不依赖 project、不扣钻石，限流 5 次 / 5 分钟 + 80 字；端上统一用分包内 `components/voice-preview`，必须销毁 `innerAudioContext` 并用请求代次作废在途结果。素材连播只核对画面顺序：静音、无字幕、以 `bindplay` 为计时起点，禁止拿它验收字幕或冒充成片预览。
- **「快出片」模糊受理与下载代理（2026-08-19）**：军师预扣后向 AIStar 提交出片，只有上游明确拒绝才退款；连接超时、响应中断或本地保存 jobId 前失败一律把 hold 置 `unknown`，保留原 `clientRequestId`，先查 `GET /me/clip/jobs/by-request/{clientRequestId}`，查不到也只按同号重试，绝不换号或直接退款。清理器同样对账后才迁移到 `accepted/rejected`。成片同源下载代理默认 120s、500MB（`VIDEO_DOWNLOAD_PROXY_TIMEOUT_MS/MAX_BYTES`），校验上游 `Content-Length` 并实时计数，客户端断连或超时必须取消上游请求。
...
- **「快出片」成片页三态铁律（2026-08-27）**：`work/index` 必须把 **载入中 / 读失败 / 读到了** 分成三个互斥分支，读到之后再按 **`done`（`status` 为 `done` 或 `published`）** 决定说什么。三条踩过的坑，别再回退：
  ...
  ④ **不给必然失败的动作，不等于什么都不给**。403/404 时藏掉「重试」是对的，但只藏不换就是死路；同理成片取不到播放地址时 `retryLoad()` 早就存在，兜底块却只说「过一会儿再来」——有解法不给。两处都补了出路（「看我的作品」「刷新试试」）。
  ⑤ **未登录也不能说成「你还没有」**。游客态后端根本没查过这个账号，标题却写「还没有数字分身」、按钮写「去创建」而实际弹的是登录浮层。副文早就分了 `guest`，标题和动作没分。
  ⑥ **状态色要按状态映射，不能只把成功态摘出去让其余全落红**。训练中是正常等待态，穿告警红会被读成出错、诱发重复创建——和把「读失败」说成「你还没有」是同一类误导，只是走视觉这条路。注意图标是按 `name-tone.svg` 找文件的，tone 写错直接 404 开天窗，改完要确认对应 svg 存在。
  这些与首页「分身读失败不能说成你还没有」是同一条原则：**不能把取不到、没好、没查过显示成正常**。回归由 `app/scripts/video-chain.test.mjs` 守（JS 状态与视图收口两侧都断言），**版面另由 `video-e2e.mjs` 在真机量**（成片页三态共 10 条：兜底块不得撑出播放器、重试点击区 ≥ 44px、生成中不得出现保存/代发）——兜底块是绝对定位 + 左右锚死 + 自带 padding，少一个 `box-sizing` 就比播放器宽 44px，这种只有量得出来、看代码看不出来。另外**兜底文案必须跟着 `done` 走**：对一条 `done` 的片子说「还没有准备好」，就和头部的「生成完成」自相矛盾——已出好是「取不到播放地址」，没出好才是「还在生成」。
...
- **邀请归因与关系链契约（2026-08-18，2026-08-19 可信捕获修复）**：`LoginAttribution` 的 `inviteCode?` / `referralToken?` 挂在三条**建号**通道的请求体上（`/auth/login`、`/auth/wechat-phone`、`/auth/carrier-onetap`）；`referralToken` 只能由游客落地时调用公开 `POST /auth/referral-capture` 获取，时间与来源由服务端签名，登录链**绝不再信客户端自报的 `inviteCodeAt`**（旧字段即使伪造成未来一年也被丢弃）。任何脏值不得让登录失败：带码但 token 缺失/损坏/码不一致，乃至生产签名密钥暂缺时，验签都 fail-closed 为 `no_timestamp` 且不建边；签发接口本身在生产缺密钥时返回 503。首次注册因此失败或配置暂不可用时，响应 `referralOutcome`，客户端保留凭证，后续仅允许“同账号 + 同一码 + 已有 `no_timestamp|config_unavailable` 留痕”恢复，普通老账号仍禁止追认。IP 只取 Fastify 按可信代理链解析的 `req.ip`，禁止直接读 XFF 首段；来源按 `src=friend|timeline` / scene 固化成 `share_friend|share_timeline|poster_qr`。归因窗口天数归运营配置 `referral-window`（读取时新键优先、回退旧 `referral.window`，缺省 30 天）。注销时只删本人边与直接指向本人的失效边，后代仍有效的直邀边保留并重算 lv2/lv3，独占/多人租户两条分支同口径。`ReferralSummary` 挂 `/me.referral`：`directCount` / `activatedCount` 只数直接下级；整个字段为 `null` 表示本次没读到，与确实为 0 必须区分。
...
- **Token / 生产 fail-fast（2026-08-19）**：本地与测试可保留历史 `token=userId` 兼容；`NODE_ENV=production` 必须同时满足高强度 `APP_JWT_SECRET`（至少 32 字节、非示例值）、`APP_JWT_REQUIRED=true`，且至少一个登录通道完整可用（短信需阿里云短信配置并强制验码；微信需 AppID/Secret），否则进程启动即失败。生产请求只接受 HS256 JWT，登录通道关闭时返回 503，不得回退 userId token 或免验证码登录。
- **进程级致命异常（2026-08-19）**：`uncaughtException` 与 `unhandledRejection` 都视为进程状态不可再信任；记录日志后启动 15 秒上限的优雅关闭（停 scheduler、`app.close()`、断 DB），最终以非零状态退出，交给 systemd `Restart=always` 拉起。禁止注册只记日志不退出的 handler。
- **账号注销（2026-08-19，保留期口径）**：`DELETE /me` 是账户安全操作，始终绕过商业套餐写权限门。注销不是立即物理删除：账号马上写入 `deletedAt/purgeAfter`、禁止旧 token 与重新登录，公开报告立即撤销，套餐/续费停止；业务、订单与审计数据默认隔离保留至少 30 天（`ACCOUNT_DELETION_RETENTION_DAYS` 可设 30–365），供客服在保留期内处理申诉/恢复。到期由 `account-erasure-sweep` scheduler 按统一清单删除或匿名化本地个人数据、OSS/PDF 与 AIStar owner 数据；财务/同意审计按法定义务保留去标识记录。接口返回 `erasureJobId/retentionUntil`，不得再宣称请求成功时已“彻底删除全部数据”；H5、原生端与隐私政策必须同步展示“立即停用 + 至少 30 天隔离保留 + 到期清除/匿名化”。
...
- **退出登录**：「我的」页底部。
- 端到端隔离已验证（见 §11）。短信验证码与微信登录已接入；生产 JWT/通道完整性由启动 fail-fast 强制保证，路由隔离逻辑不变。
...
底栏顺序（2026-08-18 IA 回归定稿）：**问策 · 战局 · 今日 · 图籍 · 主公**（五个平铺 tab，无中间凸起按钮；索引 0..4 按此顺序，唯一定义在 `services/tabbar.js`）。图标：问策 counsel / 战局 sandtable / 今日 muster（打勾名册）/ 图籍 codex（两册线装书）/ 主公 lord。新增底栏字形要同时改 `src/components/Icon/index.tsx` 的 PATHS 与构建脚本的 `CUSTOM_TAB_ICONS`（抽不到即构建失败，不静默回退 lucide）。战局 `pages/home` 只做判断；今日 `pages/execution` 承载军令/打卡/回填/复盘；锦囊 `pages/pouch` 保留完整作品与手艺内容但降为今日页内普通入口，分发规则「第一次归军师，第二次起归锦囊」不变。`pages/studio` 继续作为老分享卡兼容跳转页，统一落今日的今天段。启动页仍为 `pages/sessions`。
...
| `pages/home` | 战局 | 先看清，再行动 | 势 | 有军令→执行；有判断无军令→战局内出令后进执行；无判断→问策 |
| `pages/execution` | 今日 | 今天该做的都在这 | 今 | 今天/近七日两段；页底锦囊入口；21:00 未复盘顶部提醒 |
| `pages/pouch` | 锦囊（普通页） | 军师替你出的妙计 | 计 | 返回栈为空时 `gotoExecution('today')`，不可 `switchTab` |
...
| 战局 | `pages/home` | **2026-08-18 起回归纯判断面**：`TabHeader`（战局 / 先看清，再行动 / 势）+ 军师判断 hero + 三势合参 + 天时条 +「现在不能做」+ 判断依据抽屉。页尾桥接判据固定为「今日有军令 > 有有效判断 > 无有效判断」；核心读取失败时只给重试，不得猜 CTA。军令/兵器/回填/复盘接口与 UI 已迁出，不得再塞回本页。|
| 今日 | `pages/execution` | **独立日频 tab**：`今天 / 近七日` 两段（时间维度一致，避免 tab 与段名同叫“今日”），segment sticky 且页面实例保留段与滚动位置；今天=军令卡+任务内兵器+富余兵器+手动加令+当日海报战果+经营结果轻入口+复盘，近七日=军令完成四态+按天记录，复盘 streak 单列不与执行完成混算。固定「线索/咨询/成交」三栏不常驻主流，只在复盘抽屉顶部作为可选的今日经营结果填写，且必须独立于复盘详情 loading/error；本周行业化数据明确叫「本周关键指标」，不与每日结果混成一套重复表单。状态机分为 core、今日战果、复盘抽屉三组；复盘详情只在打开抽屉时加载。执行页严禁调用 `videoWorks()`，今日战果失败显示局部重试且不冒充空态。导航统一 `gotoExecution(today/week)` 一次性内存 intent；显式点今日 tab 进今天，无 intent 返回时保留原段。|
| 锦囊 | `pages/pouch` | **普通作品页（非 tab）**：业务内容保留——最近作品三路聚合 + 手艺两列宫格 + 启用链路；页面壳必须有返回按钮，无栈兜底执行 tab，不得 `syncTabBar`、渲染 coach marks、底栏渐隐或 tab 留白。三路作品通过 `services/works-cache.js` 的 90 秒模块级 TTL + single-flight 缓存；缓存按当前 token 隔离且旧身份慢回包不得覆盖新身份，执行今日战果与锦囊共用 posters 槽。游客态不取数。|
...
- **小程序工程约束清单（先读）**：
  ...
  - **微信桌面端 / 大窗口适配（2026-08-18）**：原生真源 `app.json` 同时开启官方 `resizable:true` 与 `frameset:true`：Windows 微信 3.3+ 可自动左右分栏，其他不支持 frameset 的客户端保持单栏；PC 大窗口可自由拉伸/最大化，iPad 可随屏幕旋转。五个主 Tab 必须通过官方页面生命周期 `Page.onResize(res)` 调 `services/page.js#syncViewport`，按**当前栏**宽度而非设备型号更新 `viewport-compact / medium / expanded`、窗口 CSS 变量与胶囊几何；不要改回全局 `wx.onWindowResize`（官方不推荐）。断点当前为 720 / 1100px，宽屏正文最多 920/1080px，问策正文与输入浮岛同轴、底栏与 sheet 收窄，图籍目录/军师列表在 expanded 档提高列数；分栏内单栏不足断点时自然保持紧凑布局。分栏下 `wx.navigateTo` 的相对路径会以最新打开页为基准，本项目所有新增路由继续写 `/pages/...` 或 `/packages/...` 绝对路径；不要为了桌面态复制 `/pc/` React 工作台逻辑。
  ...
  - **原生 tabbar 与图标**：`weapp-native/custom-tab-bar` 直接使用微信 custom tabBar，不得调用 `Taro.showTabBar` 或引入 Taro。五个自绘底栏键为 counsel / sandtable / muster / codex / lord，功能图标统一由 Lucide 构建主题色 SVG，禁止用 Unicode/emoji 冒充。`reviewDue` 红点固定在执行 index 2，常规底栏与问策终态浮岛两处同位；冷启动问策页必须调用 `store.loadReviewBadge()` 产出状态。全屏页按原生路由自然不显示 tabbar；组件式全屏层（登录、解锁、全文预览、执行复盘/目标编辑）打开时必须通过 `store.setOverlay(open,key)` 同步隐藏自定义底栏。`components/agent-unlock` 必须随 `agent` 属性成对开关 overlay 并在 `detached` 释放。
  ...
  - **fixed 底部抽屉内的原生输入框禁止系统二次顶页**：`pages/execution` 复盘抽屉的每日经营结果与本周关键指标 input 必须统一 `always-embed={{true}} + adjust-position={{false}} + bindkeyboardheightchange`；键盘高度只用于“抽屉等量上移、最大高度等量缩短”，保持抽屉顶部稳定并让内部 `ScrollView` 滚到当前稳定 id。关闭抽屉必须主动 `hideKeyboard()` 并把高度/锚点归零。不得恢复默认 `adjust-position`，否则 fixed 抽屉和系统避让叠加，输入区会飘到页面顶部。
...
- **报告标题与分享图**：`emit_deliverable.title/cover.title` 必须是完整短语，不以「与/和/及/或/·」等连接词或分隔符结尾；服务端 `normalizeDeliverableTitle` 在落库前收口模型脏标题。原生报告分享图不得按固定字数 `slice` 标题，必须经 `utils/canvas-text.js` 按真实画布宽度换行并自适应字号，长标题也要完整展示。
- **资料上传格式与失败态**：知识上传支持 PDF、Word（DOCX；旧 `.doc` 仅尽力解析）、Excel（XLSX）、PPTX、CSV、Markdown、HTML、TXT，单份上限 20MB；旧二进制 `.xls/.ppt` 不解析，端上应明确引导另存为现代格式。智库批量上传必须在选择后先校验格式/体积，服务端失败要在卡片内保留错误状态并弹出逐文件原因，禁止最终统一改写成「已送达」。所有用户文档统一在资源受限 worker 中解析（20s、独立 V8 内存上限）；XLSX 限工作表/行/单元格数，PPTX 限 ZIP 条目、解压总量和单页 XML，`docParse.ts` 的 PPTX 只按页提取 `slide*.xml` 文本，不把图片、动画或备注冒充正文。生产 Nginx 不得用全站 10MB 默认值误挡业务允许的文件：`/api/knowledge/upload` 单独设 25MB（业务仍验 20MB），`/api/video/(avatar/(consent|clone)|assets)` 单独设 120MB（形象视频业务仍验 100MB），并把网关 413 返回为带 `KNOWLEDGE_FILE_TOO_LARGE` / `CLIP_CAPTURE_TOO_LARGE` 的 JSON；普通 API 继续保持 10MB，禁止全站放宽。
...
- **tabBar 页的清理必须写在 `onHide`，不能只写 `onUnload`（2026-08-28 真机事故）**：微信 tabBar 页切走时**只触发 `onHide`，页面被缓存、`onUnload` 根本不跑**。问策页把 chat-core 的全部拆解绑在 `onUnload` 上，于是进过一次流式对话后，那个每 180ms 一次的自动滚底 `setInterval` 再也停不下来，在看不见的页面上持续 `toBottom()` + `setData`，来回切 tab 不断累积——**真机上一路吃内存到被系统杀掉（表现为"点 tab 小程序直接退出"），开发者工具内存宽裕看不出来**。
  修法不是在 `onHide` 里调 `chatCoreUnload()`——那是彻底拆解（abort 流式 + 递增 epoch 作废在途请求），会把 bug 换成「切个 tab 对话就断」。正确做法是一对 `chatCoreHide()` / `chatCoreShow()`：切走只停「看不见时做了也白做」的部分（自动滚底），流式本身继续收，切回来内容连着；还在打字就把滚底接上。
  ⚠️ **用 `reLaunch` 做的自动化验证复现不了这个问题**——`reLaunch` 走 `onUnload`，把该清的都清了。当时"开发者工具五个 tab 全正常"这条证据是无效的。
  回归由 `native-weapp.test.mjs` 的「tabBar 页的清理不能只写在 onUnload」守：扫所有 tabBar 页，`onUnload` 里每处清理动作，`onHide` 必须有对应收口。
...
- **归因链路**：`services/invite.js` 在 `app.js` 的 `onLaunch(options)` 与 `onShow(options)` 两处捕获，只认本仓邀请码形状；覆盖口径是末次触点，捕获绝不触发跳转或登录弹层。好友路径加 `src=friend`、朋友圈 query 加 `src=timeline`、scene 固定按 `poster_qr`；捕获后异步换 `/auth/referral-capture` 的服务端签名 token，登录前最多短等 1.5s，超时仍照常登录。`store.afterLogin` 仅在终态清码，`no_timestamp|config_unavailable` 必须保留以便受限恢复；若上次签名请求已失败且当前仍只有 raw code，下一次登录会按已存来源主动重签，禁止永远重复提交裸码；本地 `local-*` 假登录同样不清。冷启动 onLaunch/onShow 回响只抵消埋点与重复签发，不得清掉刚拿到的 token。
- **邀请漏斗四段（`分享曝光 → 落地打开 → 注册 → 首开通`）的口径与落点**：
  ① `share_expose` 由 `services/share.js` 上报，**埋在 `withShare` 的 wrapper 里而不是两个 mixin 里**——那 4 个成果型分享页整体覆盖了 mixin，埋在 mixin 里它们一条都不报（与 `imageUrl` 兜底同一个理由的另一面），埋在 wrapper 里则 54 页统一一条且不双报。props 只有 `channel`（`friend`/`timeline`）与 `poster`（当日素材序号）：**不带页面路径、页面内容、邀请码**，否则「谁在账本页点了转发」就进了埋点库，与「内容与页面解耦」自相矛盾。
  ② `invite_landing` 由 `services/invite.js` 在 `captureInvite` 捕获成功处上报，props 只有 `channel`（`query`=分享卡 / `scene`=小程序码）。**每次真落地都报，但冷启动的那次回响要抵掉**：小程序被销毁后点分享卡进来时，微信把同一份启动参数先给 `onLaunch` 再给紧随其后的首次 `onShow`，两处都报就会变成冷启动两条、暖启动一条，**漏斗分母按启动形态失真**。做法是 onLaunch 上报后置一个带时效的一次性标记，被紧邻的下一次调用消费掉、且只在同码时抑制；用户运行期间再点一张卡（换码或同码再进）照报——那确实是新的一次落地，去重交给取数侧。
  ①b `qr_provision`（2026-08-20，静态传播链）由 `packages/work/invite` 上报，props 为 `slot`（物料位）+ `act`（`view`/`image`/`save`）。**它不是「曝光」而是「物料投放」**：二维码的曝光发生在端外（名片被看到、台卡被扫），小程序里没有任何时机能捕获，硬凑一个只会让分母失真；端上真实可捕获的是「用户取了这张码去印物料」。**只在真拿到码时报**——降级态（没码、只能手输）不报，否则漏斗里混进一批根本贴不出去的「投放」。取数时**与 `share_expose` 不可相加**当「总曝光」：一次投放会带来 N 次线下扫码，量级不同。
  ③ 注册段**不占事件名**：从 `ReferralAttribution` 算（每次带码进线一行、**八种** outcome 全留痕：bound / self / cycle / unknown_code / expired / already_bound / config_unavailable / no_timestamp），分母/分子都比客户端埋点准；再补一份端上埋点只会出现「端报了、库里没有」的对不上账。
  ④ 首开通段从 `ActivationEvent(source='invite')` 算，支付权益事务同时 upsert `InviteActivationOutbox`，提交后立即异步处理、`pay-reconcile-sweep` 定时续扫并退避重试；因此不阻断微信支付响应，也不再依赖“进程没退出”或“微信恰好重投”。写入挂在 `markPaidAndApply`（真金入账唯一收口）而不是 `applyPlanPurchase`，避免把注册测试期开通 / 演示 / 运营手工开通算成付费转化；同一用户仍以 advisory lock + 查重只记首次。
  - **`source` 冲突口径（务必看懂再改）**：`prescription/catalog/market` 回答「从哪个位子成交的」（值来自下单请求，随 `PaymentOrder.attrSource` 存档），`invite` 回答「这个人是被谁带来的」——**两个维度，不是一组互斥枚举**。所以 invite **另落一行**，绝不覆盖位子归因（覆盖会让运营后台处方位开通数凭空变少）；代价是按 source 分组的各桶**重叠、不可相加**，这条口径写在 `admin/src/views/revenue.tsx` 的读数侧。invite 行**按人去重**（一人最多一条 = 首次付费开通，因为漏斗问的是人数不是订单数），靠 `activation:invite:{userId}` advisory lock 串行化 check-then-insert（`ActivationEvent` 上没有唯一约束）。推荐人 id 不冗余进 `refId`——`Referral` 是不可变更账本，按 userId join 即得。前端请求体里写 `source:'invite'` 一律被 `parseAttribution` 打回 `catalog`（否则谁都能刷漏斗）。
  - **不阻断主链路的实现方式**：支付事务只持久化一行轻量 outbox，不在回调响应前执行 Referral/ActivationEvent 的额外查询；真正补记在事务提交后跑，失败留待 scheduler。端上两条埋点仍全走 `api.track`（裸 `wx.request` + 空回调）：分享回调同步返回，埋点不 await 且整段 try 住；`invite.js` 对 `./api` 保持函数体内懒 require，避免 CJS 环拿到半初始化 exports。
- **建边的并发口径：有序 advisory lock（2026-09-02，原 §13 的 TOCTOU 项已由此关掉）**。`services/referral.ts` 的 `bindReferral`（`bindOnRegister` 与运营补绑 `bindManually` **共用的那一份**）在「self 判定之后、归因窗口与查环之前」按 **`[userId, referrerId]` 字典序排序**依次取 `pg_advisory_xact_lock(hashtext('referral:' + id))`，拿到锁后**重读一次** `Referral` 行再往下走。三条不许改的理由：① **两个 id 都要锁**——环的成立条件涉及双方，只锁被邀人挡不住反向那条边（A 用 B 的码、B 用 A 的码同时进线时，两侧的 `wouldFormCycle` 都看不到对方未提交的边，于是建出二元环，而 `Referral` 是不可变更账本、环落库就修不回来）；② **必须排序**——各自「先自己后对方」就是教科书死锁；③ **拿锁后必须重读**——否则往下会撞主键，而唯一约束一失败整个事务即 aborted，连 attribution 留痕一起丢。锁是事务级的，只在 `prisma.$transaction` 内调才有效（传裸 `prisma` 时每条语句自成事务 = 无保护）。守卫用例：`test/adminInvites.test.ts`「并发互邀：恰好一条成功…且库里没有环」+ `test/referral.test.ts`「并发绑定走生产形态（事务内）」。
- **运营补绑（2026-09-02，`POST /admin/invites/manual-bind`，requireSuper）**：**只给尚无推荐人的用户建边，不提供改绑/解绑**——关系不可变更公理不破，已绑的照样回 `already_bound` 且原关系一字不动。判定完全走上面那份 `bindReferral`（同一套 self / cycle / unknown_code / already_bound + 同一把锁），**绝不复制查环逻辑**；`source='manual'` 同时是**免归因窗口**的开关（运营手上没有签名捕获凭证，`no_timestamp` 那道闸对 manual 放行），新鲜度由 reason 与审计承担、不由窗口判。失败的补绑尝试**同样落一行 `ReferralAttribution` 与一条审计**（`admin.invite.manual_bind`，payload 带 userId/inviteCode/referrerId/outcome/reason/by）——运营敲错码这件事必须在归因日志里看得见。`clientIp` 取运营这次请求的 `req.ip`，运营的 UA **不入库**（后台浏览器指纹与「新号从哪台设备进线」无关）。回归见 `test/adminInvites.test.ts`。
...
| `PUT /profile/bazi` · `GET /profile/chart` | 八字采集（→排盘引擎落库；believe=false=不信命理只存偏好；出生地仅作档案记录） · 我的命盘读取 | 是 |
...
| `POST /events` | 客户端埋点：**鉴权可选**（无 token=游客、userId 空；带 token 必须有效，无效仍 401，不静默降级成游客事件）。`name` 走白名单（现 18 项：问策入口八项 `wence_enter/proactive_show/chip_tap/hint_tap/first_message_send/drawer_open/attach_open/tab_switch`、今日执行与锦囊七项 `execution_enter/order_complete/backfill_save/review_start/pouch_entry_view/pouch_entry_click/weapon_click`、邀请漏斗三项 `share_expose/invite_landing/qr_provision`；**白名单在 `routes/wence.ts` 的 `EVENT_NAMES` 与 `shared/contracts.d.ts` 的 `ClientEventName` 两处，必须同时改**），非白名单 400 且不写库；`props` 序列化超 2KB 截断（事件本身不丢）。只写不查，无读端点；已加入 `app.ts` 禁写闸放行前缀（生产新号默认无套餐，拦了就把要观测的人从分母里切掉） | 可选 |
...
| `GET /admin/invites` · `GET /admin/invites/attributions` · `GET /admin/invites/export` · `POST /admin/invites/manual-bind` · `GET /admin/invites/chain/:userId` | 「增长」组邀请关系（`routes/adminInvites.ts`，自挂 requireAdmin）：关系账本分页（两侧姓名+掩码手机/码/来源/被邀人 planGate 开通态/首笔付费，`days` **缺省全量**——关系永久，不该被默认窗口吃掉；`q` 同时匹配两侧姓名·手机前缀·邀请码·userId）· 归因日志分页（八种 outcome 全留痕；**下发 clientIp、不下发 userAgent**）· CSV（筛选与列表共用同一份条件，手机掩码，文件名 `invites-<days\|all>.csv`）· 运营补绑（**只给尚无推荐人的用户建边，不提供改绑/解绑**；判定与有序 advisory lock 全在 `services/referral.ts` 的 `bindManually`，`source='manual'` 免归因窗口，落归因 + 审计 `admin.invite.manual_bind` 带 reason/outcome）· 邀请链（`upline` 沿 referrerId 递归到根、hop 64、撞顶如实回 `uplineTruncated`；`downline` 与 `/admin/referral/tree` **共用 `buildReferralSubtrees`**；`team` 三级人数/开通/GMV（已退款不计）+ 代理各级佣金净额） | 管理员（导出与补绑=owner） |
| `GET /admin/distribution/config` · `GET/POST/PATCH/DELETE /admin/distribution/tiers[/:id]` · `PUT /admin/distribution/tiers/:id/rules` · `GET/POST /admin/distribution/distributors` · `GET/PATCH /admin/distribution/distributors/:id` · `GET /admin/distribution/commissions` · `GET /admin/distribution/settlements` · `POST /admin/distribution/settlements/generate` · `POST /admin/distribution/settlements/:id/{approve,paid,void}` | 「增长」组代理分销（`routes/adminDistribution.ts`，自挂 requireAdmin；计提逻辑全在 `services/commission.ts`，本文件只做读投影 + 状态机写）：配置只读（`enabled`/`holdDays`/`holdConfigured`/`flagKeys`，`fresh` 读；总开关与冻结期归「功能开关」页，一个数字只有一个入口）· 等级目录（**禁止 seed、空目录合法**；同名 409；DELETE 仅在无代理挂靠时允许，否则 409 `TIER_IN_USE`）· 比例矩阵**整体替换**（level 1..3 × itemType plan\|sku\|all × rateBp 0..10000，(level,itemType) 重复 400；不在列表里的组合视为删除）· 代理名册（每人 lv1/lv2/lv3 人数与累计计提/待结/已结，**groupBy 批量，禁 N+1**；登记即 `active`，用户不存在 404、已是代理 409；`terminated` 后档案只读 409）· 佣金流水（`summary` 与 `items` 吃同一个 where，`daily` 按北京时间自然日）· 结算单（generate 纳入 `confirmed` 且未挂单且 `createdAt ∈ [start,end)` 的行含 clawback、**零行不生成**、按代理 advisory lock 串行；approve 不碰流水；paid 必填 `paidRef` 并同事务把 `confirmed→settled`；void 仅 draft/approved，解绑行但**保留单上原金额作凭据**）。手机号一律 `maskAuditPhone`；分页 `pageSize` 夹 1..200；写操作全部 `requireSuper` + 审计 `admin.distribution.*`（payload 带 before/after 与 `by`） | 管理员（写=owner） |
...
- `services/scheduler.ts`（M1 定时任务框架，2026-08-19 多实例选主，2026-08-29 连接池防饿死）：任务注册制 + 进程内周期扫描；**同一进程所有 job 进入一条严格 FIFO 串行队列，同名任务已排队/运行时直接复用该轮，不再堆第二份**。这是连接池安全约束：生产 Prisma 池只有 5 个连接，旧实现若同一秒并发启动多轮，每轮先占一个 advisory-lock 事务连接、`job.run()` 又向全局 Prisma 申请第二个连接，会让锁持有者彼此饿死并在 10 秒后集体超时。串行后单轮最多占一个选主连接和任务自身连接，同时给用户请求留出容量。每个生产 job 仍先取得 PostgreSQL `pg_try_advisory_xact_lock(hashtext('junshi:scheduler:<name>'))`，锁覆盖本轮执行，因此同一数据库上的多实例每轮至多一个执行者；`NODE_ENV=test` 不自启并绕过锁，测试直接 `runJob/scan*` 驱动。`SCHEDULER_ENABLED=false` 仍可让纯 API 实例完全不扫描，但不再依赖人工“只能开一个 true”。每个任务均输出周期、启用态、健康锚点、最近成功/失败/耗时与结果计数，通用 `JunshiSchedulerJobStalled` 在超过 `max(3 个周期, 15 分钟)` 未成功时告警；支付业务告警只有库里确有待收敛订单才触发，不能再拿“当前没有支付”冒充资损。任务彼此隔离（单任务崩不影响其它）。已挂：`account-erasure-sweep`（到期注销数据清理）、`video-maintenance-sweep`（视频模糊提交/克隆卡死对账）、`casefile-idle-recall`、`daily-review-reminder`、`review-gap-reminder`、`prophecy-due-scan` 等。视频维护不得在每个路由实例另起 timer。
...
- `services/paipan.ts`（★ M1 排盘引擎 `paipan-v6`）：确定性命理/历法计算——干支历/八字/大运用 `lunar-typescript`，紫微命宫/身宫主星用 `iztro`；产出四柱十神/月令取格/日主强弱与喜用/大运时间线/年度逐月攻守。全端统一采集出生证明上的准确 `HH:mm`，直接按法定钟表时间排盘，不根据出生地/经度换算真太阳时；出生地区只作档案记录。换日固定 `sect 1`：`22:59` 仍是当日亥时，`23:00–23:59` 自动计为第二天子时，次日 `00:00` 为该日早子；紫微每次立盘也显式恢复 `iztro dayDivide=forward`，不得依赖库默认值或进程内全局配置。存量只有时辰的记录按原档位中点解释（`00:30` / `23:30`，其他档位的小时本就是中点），报告标「旧记录分钟待确认」。新排/主动重排写 v6；存量 v3/v4/v5 首次读取按原始钟表时间惰性重算并落 v6，清除经度计算标记；v1/v2 原盘照旧读取。**铁律：算→存（`NatalChart.engineVersion`）→拼指令，AI 只做比喻翻译**。
...
- `services/generationWorker.ts` 调度（2026-08-17）：**有界滚动并发 + 套餐优先级**。生产入口是 `pumpGenerationWorker()`（`startGenerationWorker` 的 300ms interval 驱动）：`inFlight < GENERATION_WORKER_CONCURRENCY`（默认 4，设 1 = 回到旧串行）时循环领单，任务本体 `void` 出去、收尾 `.finally` 立即再 pump 补位——不是 Promise.all 批（一单可跑 300s，批处理会队头阻塞）。`tickGenerationWorker()` 保留「领一单、await 跑完」的串行语义，**它是约 10 个测试文件的精确驱动器，不许改**；两条路径共用 DB 原子 claim（SKIP LOCKED + 租约），并发安全在数据库层。`stopGenerationWorker` 置 `stopped` 标志同时挡 interval / pump 循环 / `.finally` 再触发三个入口；保证强度是**至多再领一单**（stop 恰落在进行中的 claim await 里时，那单已带租约回来，照常跑完是最小伤害）。**领单顺序不再是纯 FIFO**：`GenerationJob.priority`（0-9，入队时按 `Plan.tierRank` 快照、封顶 9，映射归运营后台，不进幂等指纹）+ 虚拟到达时间调度——`createdAt - priority × GENERATION_PRIORITY_AGING_SECONDS`（默认 30s/级，封顶 9 级 = 高档最多插队 4.5 分钟，杜绝低档饥饿；设 0 = 严格优先级）。刻意用虚拟到达时间而不是「等待时长 FLOOR 折算」——后者的阶梯会让相对次序随时钟边界来回翻转。排位透出：`countQueuedAhead` 与 claim 共用 `schedulingKeySql`（改一处必须改另一处），GET `/generations/:id` 与 SSE 快照在 queued 期间带 `queue.ahead`（SSE 每 4 拍约 1.4s 刷一次 COUNT；status/phase 变化独立触发推送，因为 claim 不递增 snapshotVersion）；小程序 `generationProgressText` 显示「排队中·前面还有 N 位」。**llmGate 刻意不做优先级队列**：worker 并发 ≤ 主车道槽位时生成调用到闸门必有空槽，优先级在 claim 层已生效，闸门保持纯 FIFO 的上游保护阀角色。回归：`test/generationPriority.test.ts`、`test/generationWorkerConcurrency.test.ts`。
...
- **worker**：`FOR UPDATE SKIP LOCKED` 抢占，2s 轮询，`setInterval+unref`，`NODE_ENV=test` 不自启（测试直接调 `tickCreativeWorker()`）。worker 本身依靠数据库行锁支持多进程并发消费，与 scheduler 的 advisory lock 选主是两套不同机制。一轮最多串行处理 `TICK_BATCH_SIZE=2` 单（不是并发，见下文）。`sweepCreativeJobs`（挂 scheduler，5min）：running 超 `STALE_RUNNING_MS`（10 分钟）→ 回 pending 或 failed+退款，外加「已扣未退的终态任务」兜底重扫。
...
- **`admin/src/useResource.ts` + `components.tsx` 的 `ViewState`**：统一 loading（骨架屏）/ error（带服务端原文 + 重试）/ empty 三态，并在 `PageHead` 显示数据新鲜度。替掉旧版散落 32 处的 `.catch(() => {})`——接口 500 时页面渲染成「近 30 天暂无订单」，运营会把它当业务结论上报。**React 18 StrictMode 约束（2026-09-02）**：取数有效性必须由每一轮 effect 自己的 `cancelled` cleanup 管理，不能用组件级 `alive` ref 只在卸载时置 `false`；StrictMode 的 mount → cleanup → mount 会把后者永久烧死，开发环境所有后台页都会卡在骨架屏。跨请求只接受最后一次响应仍由递增 `reqId` 负责；回归见 `admin/src/useResource.test.ts`。
- **交互完整性（2026-08-17）**：共享 `Switch` 统一开关的键盘/读屏语义；`useDialogFocus` 统一弹层首次聚焦、Tab 圈定、Escape、关闭焦点归还与长内容视口内滚动。登录状态探测、账号/密钥提交、命令面板找人、技能目录和已学记忆治理均将网络失败与真空结果分开，并给出原文/重试；账户菜单改为同级 trigger + `role=menu`，toast 用 `role=status`。主动作白字统一落 `--accent-deep`，次级说明灰提升到可读对比度；`lint:ui` 额外阻断 `:root` 外 hex、字面量 z-index、`div.sw` 和空 catch。
...
- **观测信息人话化与审计降噪**：调用诊断用 `LlmTrace.userId/tenantId/sessionId/agentKey` 回填来源用户、手机号、租户与智能体中文名；`model/endpointId/endpointLabel` 保存实际命中的模型及端点快照，端点改名或退出池后历史记录仍可辨认。界面主信息显示「对话回复/方案生成」等业务标签与实际端点，`general/deliverable/ip` 原始 key 只留排障标识。`/api/metrics` 是 Prometheus 机器抓取，不写 `AuditLog`；修复前历史抓取默认过滤，只有显式 `includeMetrics=true`/「含监控抓取」才展示，避免淹没用户行为。**2026-08-20 完整性重构**：`GET /admin/sessions` + `GET /admin/sessions/:id` 提供跨用户会话检索、消息游标、生成任务与 trace 下钻；`GET /admin/audit-view` 提供服务端日期/状态/关键词筛选与精确分页，并正式返回 `requestId/sessionId/operator` 供“同一请求链 → 关联会话/用户”跳转。`services/adminRange.ts` 是概览、Token、trace、会话和审计的统一时间解析器，自定义日期按北京时间自然日、最长 3660 天；概览比较所选区间与前一等长区间，不再同屏混用固定 7/30 天。**滚动预设的 `toExclusive` 必须略过当前时刻（当前为 60 秒）**，否则数据库 `now()` 与 Node 同毫秒新建的 trace / 审计会被严格 `lt` 上界排掉；对外 `view` 仍截至真实现在、天数不含余量。自定义日期上界不加余量，避免运营选「截至昨天」却查进今天；无显式上界的 trace 查询不应凭空添加 `lt now`。详细审计见 `docs/ADMIN_CONSOLE_REFACTOR_2026-08-20.md`。

页面/接口：概览看板、**注册用户管理**（小程序注册用户、微信绑定、租户/套餐、最后会话、会话/成果数、算力余额，并可点进用户详情为其开通/取消 `unlock` 智能体）、**会话工作台**（跨用户搜索、消息/生成/trace 同屏下钻）、**算力消耗**（按用户汇总赠送/消耗/余额、30 天活跃、成果数）、**审计日志**（服务端日期/关键词/状态筛选与分页；默认过滤 `admin.*` 和 metrics 噪声；单行展示用户 API、登录尝试、业务动作、用户/租户、摘要、方法/路径/状态码、IP/UA，详情展示 request/session/operator 与原始 payload；`adminAuditView.ts` 的原生 SQL 时间边界必须经 `utcTimestamp` 与 Prisma 的 UTC naive `DateTime` 对齐，禁止裸绑 `Date`，否则北京时间当天会漏日志）、每日献策库（增删改启停）、智能体/功能配置（新增智能体、基础信息、`free/unlock/metered` 定价、System 提示词、对外问候语、专属理解提示 `memText/learnText` + Agent Memory 策略 + **上架/下架**，前台 `/agents` 默认只展示已上架功能）、**技能库**（新增/编辑自定义 HTTP 工具，复用后台统一组件语汇）、**模型配置（凭证 / 接入点 / 六用途路由，含直测、深度探活、分流池与用途预算，即时生效）**、建档问卷、套餐编辑。所有 `/api/admin/*` 路由由 `services/adminAuth.ts` 保护；入口 `admin/src/App.tsx` + `admin/src/views/*` + `AgentDetailPanel.tsx`，API `admin/src/api.ts`（类型来自 SSOT）。智能体详情保存采用真正的 dirty-field PATCH，后端继续统一审计并重算 `draftDirty`；数据库草稿/已发布版本才是运行时事实来源。Agent Memory 开关保存到 `Agent.memoryConfig` 并由后端真实读取；`LlmTrace.contextJson` 保留可排障但不含记忆正文的上下文快照。开发期 Vite 代理 `/api → localhost:4000`。后台全屏无边框，桌面左栏 + 移动 8 组底栏（flex 均分、永不横滚；375px / 8 ≈ 46px 一格，故组名保持 2-3 字）+ 组内 segmented；样式必须保持视口安全收缩、横向隐藏、长文本断行、键盘可达与弹层焦点完整。

- **「增长」组与 shadcn 模块（2026-09-02）**：新开第 8 组 `growth 增长`，四个分区 `invites 邀请关系` / `chain 邀请链` / `distribution 代理分销` / `referral 邀请增长`。为什么另立一组而不塞进「经营」：本轮补的是邀请体系的**运营面**（查关系账本、看一个人的上下游链路、登记代理与结算佣金），三页里两页带写操作，按「主导动词决定归属」既不属只读的「经营」也不属平台级的「配置」；只读的「邀请增长」一并搬进来，运营查邀请的动线本来就该收在一处（搬完 `revenue` 回到 4 项，两组都在「单组不超过 8 项」内）。路由 `#/invites`、`#/chain[/<userId>]`、`#/distribution[/<distributorId>]`——**`chain` 的 `route.id` 是 userId，`distribution` 的是 distributorId，别混**。跨屏直达：邀请关系行内「被邀人链 / 邀请人链」、用户详情面板「查邀请链」、代理名册「查邀请链」、邀请链上的「代理档案」。写操作只有补绑（`invites`）与代理/规则/结算（`distribution`），全部 `isSuper` 才渲染入口 + `catch` 透出 `e.message`；总开关与冻结期只显示并链到 `#/flags`（一个数字只有一个写口）。
- **shadcn 模块（只进「增长」组）**：这三页是仓里**唯一**用 Tailwind v4 + shadcn/Radix 的地方，边界是目录——`admin/src/growth/**`、`admin/src/components/ui/**`、`admin/src/lib/**`。**旧页面一行没改，也不许 import `@/components/ui`**；新模块不许用 admin.css 的组件类（`PageHead` 输出的旧类除外——admin.css 无作用域，页头因此全站一致）。四条不许回退的约束：① **不引 preflight**（会重置旧页面的 `button`/`h1..h6`/`table`/`input`），缺的三样自己补——`.sc` 的底色文字、`.sc *` 的 `border-color`、Radix 浮层用 `container` prop 挂进 `.sc` 内的挂载点（`src/lib/scPortal.tsx`）；② `utilities.css` 用 `source(none)` + `@source` 只扫那三个目录，**白名单不许放宽到 `admin/src`**（旧页面里的 `block`/`full`/`on`/`fixed` 与工具类同名，扫进去会按 layer 顺序盖掉旧样式）；③ shadcn 变量一律 `--sc-` 前缀（admin.css 的 `--accent` 是品牌金，shadcn 的 `--accent` 是悬停底，同名互相污染），色板全部映射到既有纸/墨/金 token，**One Command Color 不变**；④ **不做暗色模式**——Tailwind v4 的 `dark:` 默认等于 `prefers-color-scheme: dark`，`shadcn.css` 用 `@custom-variant dark` 把它重定义成永不命中的选择器（别手删组件里的 `dark:`，下次 `shadcn add` 会覆盖回来）。`scripts/audit-admin-ui.mjs` 只加一条豁免：那三个目录**跳过规则 1**（class 必须在 admin.css 里定义），inline 硬编码颜色 / 控件一次性 inline style / `div.sw` / 空 catch 照旧阻断；admin.css 一行未改。逻辑层两边共用（`useResource` / `PageHead` / `router` / `api.ts`），三态与空态在 `src/growth/ui.tsx` 有一套 shadcn 皮肤但**语义与 `ViewState` 完全一致**（loading/error/empty 不许互相冒充、error 带服务端原文 + 重试、403 不给重试按钮）。归因结果 / 来源 / 短 id / 人物标签的人话表抽到 `src/growth/labels.ts`，`views/referral.tsx` 改成 import 同一份（两屏对同一个 `outcome` 说两种话，运营会当成两件事）。映射表与禁令全文见 `admin/DESIGN.md`「§7 shadcn 模块（增长组）」。

### 9.1 邀请增长（2026-08-18，P3；`admin/src/views/referral.tsx` + `server/src/routes/adminReferral.ts`）

「经营 · 邀请增长」一页四个 tab，**一份数据三个投影 + 一个留位**，全只读。端点四个：
`GET /admin/referral/{overview,tree,risk,tenants}`——`tenants` 单独一个是为了让「租户筛选项没加载出来」
能显示成可重试的错误，而不是伪装成「暂无带邀请关系的租户」（业务空态与读失败不许混）。
四个 tab **各自持有取数、切到哪个才挂载**：只看一张静态 UML 说明图不该触发树聚合与风控扫描。

- **视图选型有理由，不是审美**：① 本体 Schema 用 **UML 类图**（看字段与基数），**刻意不用力导向图**——实例一多就是毛球，且每次刷新布局都不一样、没有阅读逻辑；② 邀请关系树用**从左到右的层级树**（邀请关系天然有方向与层级），节点大小=直邀人数、底色=开通状态、红环=风控标记，点节点展开/折叠；③ 风控用 **IP ↔ 新号二部图**——正常用户分散在各自 IP、刷号聚集成一个 IP 连一串新号的扇形，这个形状要一眼可见；**响应不下发 `userAgent`**（前端一处不展示，把 IP + 用户 id + 手机号 + 设备指纹一起端给所有后台账号是没必要的隐私扩散），手机号一律走 `services/audit.ts` 的 `maskAuditPhone` 掩码（树节点同理——树上挂着风控红环，与风控屏属同一关联面）；④ **转化漏斗留位未做**，卡在 `ClientEvent` 没有聚合读端点（§13 ③），不是卡在画图。
- **零图表依赖**：`admin` 的 dependencies 只有 react/react-dom，三张图全是手写 SVG。**颜色一律在 `admin.css` 的 `.rf-*` 组件类里用 token 定义，`.tsx` 里只算几何**（x/y/r/width）——这是 `lint:ui` 的硬约束，也让三张图的配色只有一处可改。树的布局是**每个可见节点占一行 + 直角折线**：可预测（同一份数据每次打开完全一样）、展开只在下面插入行，且**天然不会重叠**（同一行不会有两个节点）；最后一列右侧必须留够标签宽度（`T_TAIL`），否则 L3 的元信息会被 viewBox 直接裁掉。宽图一律裹在 `.rf-scroll` 里自己横滚，页面主体永不横滚。
- **端点单独成文件**：`routes/adminReferral.ts` 自挂同一把 `requireAdmin`（admin.ts 已 2900+ 行，三个投影各要一次成形）。树接口用**物化路径一条 `OR`（lv1/lv2/lv3 各有索引）取全三级子树**，配合一次 `groupBy(lv1)` 拿到任意深度都准的直邀数 + 一次 `findMany` 补名字/套餐，**没有逐层展开的 N+1**；根 = 直邀最多的前 N 人（投影不是全景，排序稳定可预测）。风控接口把聚集判定**整体下推到 SQL**：一条 `GROUP BY clientIp HAVING COUNT(DISTINCT newUserId) >= 阈值` 直接出组（并在同一条里带出 `group_total`），名单用 `DISTINCT ON (clientIp,newUserId)` 取每人最早那条、再 `ROW_NUMBER() OVER (PARTITION BY clientIp)` **按组**限量。**行级上限已彻底删除**——初版是「先全局 `take: 5000` 再在内存里去重判阈值」，那会让某个 IP 的 5000 条重复旧留痕把另一个真实聚集组整组挤没、页面显示「没有聚集」（2026-08-18 复核判为阻断：风控视图唯一的价值就是看见聚集，这种漏报比没有这块屏更糟）。截断只发生在组数上且**如实透出 `groupTotal`**；`scannedIps` / `scannedAttributions` 由一行 `COUNT(DISTINCT clientIp), COUNT(*)` 给出，让**「扫了 200 个 IP 没有聚集」与「一条数据都没有」在界面上分得开**。树的红环调**同一个** `riskGroups()`（同窗口、同阈值、同组数上限）解析，所以红环集合恒等于风控屏本次返回的组——否则会出现「树上标红、风控屏里找不到那个 IP」。
- **阈值归运营，不写死**：聚集阈值是功能开关 `referral-risk` 的 `payload.ipMin`（默认 5，区间 2~200），区间与默认值由 `routes/adminReferral.ts` 导出、`FEATURE_FLAG_CATALOG` 引用，两处不会漂移。**刻意另立一个 flag id 而不是复用 `referral`**：`PATCH /admin/flags/:id` 的 number 分支是整块 payload 覆盖写，两个数值挤一个 flag 会互相抹掉。读阈值走 `featureFlagPayload(..., { fresh: true })`，运营改完当场能验证；**读失败照抛 5xx，绝不用兜底默认值算出一份看着正常的名单**。
- **口径同源**：「已开通付费」复用 `planGate` 的判定（`planId` 存在且 `isExpired(planExpiresAt, now())` 为假，时钟走 `clock.now()`），与「我的邀请」的 `activatedCount`、与套餐页是同一个答案；树上的风控标记与二部图**同一批超阈值 IP 组**，两屏不会各说一套。**公理 5「风控预警不阻断」**：这一页没有任何封禁/拉黑/停发奖写操作，端点也只有 GET。
- 回归：`server/test/adminReferralViews.test.ts`（物化路径分层、状态着色含到期回落、租户可筛、只回超阈值组、改运营配置就改结果、去重口径、树与二部图同源、无写方法、空态与读失败可区分）。
...
- `Referral`（邀请关系，`userId` 主键 = **单推荐人公理**，绑定后不可变更）+ `ReferralAttribution`（**每次带码进线都落一行，成功与失败都留痕**，含 `clientIp`/`userAgent` 作风控原料）。物化路径 `lv1/lv2/lv3` **从第一天就写全**（运营侧邀请树要看完整链路），但对用户呈现与将来的激励口径**只看一级**。判环**必须沿 `referrerId` 递归上溯**（物化只到三级，A→B→C→D 这种链上只比 lv1/lv2/lv3 会漏判并建出环），带 hop 上限兜底。`outcome ∈ bound|self|cycle|unknown_code|expired|already_bound|config_unavailable|no_timestamp`（后三个是复核时补的：`cycle` 与 `self` 分开是因为运营看到 `self` 会以为用户在拿自己的码、排查方向不同；`config_unavailable` / `no_timestamp` 都表示「判不了就不建这条改不回来的关系」）。
...
- **代理分销六表（2026-09-02）**：`Distributor` + `DistributorTier` + `DistributionRule` + `CommissionEntry` + `CommissionSettlement` + `CommissionOutbox`。
  代理 = **运营在后台登记的签约渠道合作方（B2B）**，佣金线下打款、后台只回填凭证号；小程序端本期零暴露（不触碰「小程序内现金裂变返利」这条审核红线）。**佣金只发代理、不发普通邀请人**——一笔订单沿买家 `Referral.lv1/lv2/lv3` 上溯，某一级祖先是 `status='active'` 的代理且其等级在该层级有 `rateBp>0` 的启用规则才计提；三级是上限（与物化路径同深，不是另定规则）。
  · `Distributor.status`：`pending` 待审（留给将来的自助申请，本期无入口）→ `active` 生效 ↔ `suspended` **暂停计提** → `terminated` 终止（**终态，档案只读**）。**`suspended` 不是延后计提**：暂停期间成交的订单永久不产生佣金行，解除暂停也不补算——否则运营一解除就会冒出一批没人核过的对外负债。
  · `DistributorTier` / `DistributionRule` 是**运营目录**：**禁止 seed，空目录/空规则合法且等于不计提**（代码里没有任何默认比例）。因此二者**不进** `prisma/resetBusinessData.ts`（同 `WenceTemplate` 的理由：进了就会在预发 seed 时清掉运营录入的比例），测试自己建自己清。规则维度 = 等级 × 层级(1..3) × 商品类型(`plan|sku|all`)，**精确 itemType 优先于 `all`**，`enabled=false` 与 `rateBp=0` 都等于没配。
  · `CommissionEntry` 是**不可变账本**：只追加，落行后只改 `status`/`settlementId`/`reversedAt`。`kind`：`accrual` 计提（正额）| `clawback` 追回（负额，**已结算后才退款**时另落一行，绝不改写已打款的那行）。`status`：`pending` 冻结期内 → `confirmed` 可结算 → `settled` 已随结算单打款；`reversed` = **结算前**退款作废（不进任何结算单）。**幂等键 `(outTradeNo, beneficiaryUserId, level, kind)`**——outbox 重试、微信重复回调、scheduler 续扫全靠它兜住（层级进 key 不是因为同一人可能占两级，而是幂等键不该依赖别处的无环公理成立）。`ruleSnapshotJson` 存等级名/规则 id/比例/冻结期，运营改规则历史行不漂。
  · `CommissionSettlement`：`draft` → `approved` → `paid`（必填 `paidRef` 线下凭证号，同事务把关联行 `confirmed→settled`）；`void` 仅 draft/approved 可作废（解绑行放回待结池，**单上 `entryCount/totalAmount` 保留原值作凭据**）。净额可为负（同期追回多于计提），这种单照样走流程 = 下期抵扣的凭据。银行账户资料**不入库**，线下财务保管。
  · `CommissionOutbox(outTradeNo, kind)`：与 `InviteActivationOutbox` 同一套路、同一理由——支付/退款**事务内** upsert 一行（意图随真金一起提交），提交后异步派发一次，失败由 scheduler `pay-reconcile-sweep` 的 `scanCommissionOutbox` 续扫。**绝不 await、绝不往支付响应上抛错**：把「查 Referral + 查代理 + 查规则」塞进支付事务，等于让一次查询抖动回滚已经算成功的入账。挂点就在 `markPaidAndApply` 里 `enqueueInviteActivation` 的**同样两处** + `refundWechatOrder`/`markRefundNotified` 置 `refunded` 的两处。
  · 冻结期归运营（功能开关 `distribution-hold.days`，兜底 7 天、区间 0..90）：`holdUntil = 支付时刻 + days`，scheduler 新增 `commission-mature`（30 min）调 `confirmMatured()`——`pending` 且到期且**订单未退款**才转 `confirmed`。总开关 `distribution` **默认关**，且**开启后不追溯历史订单**（不计提的 outbox 行照常置完成，一次开关操作不该凭空产生一笔没人核过的负债）。
  · 注销政策：`Distributor` / `CommissionEntry` / `CommissionSettlement` 全归 **retain**（财务与签约凭据，去标识保留）；purge 时只抹 `Distributor` 的 `displayName`/`contactPhone`/`remark`，**不动 userId 与状态**（改了 userId 就再也说不清那些已结算的钱是给谁的；终止合同是商务动作，不该由注销替运营决定）。`CommissionOutbox` 无 userId/tenantId，不需登记。
...
```
**改「快出片」界面文案的固定口径（2026-08-28）**：用 `shuorenhua` skill（`~/.claude/skills/shuorenhua/`）做「去 AI 味」时，按这套判定走，别每次重新拍：

- 场景 `chat`（界面微文案 = 系统对用户的即时回应），档位 `minimal` 为主、翻译腔与自造口语升 `standard`，
  **scope 固定 `in-place`** —— 界面文案一句一个位置，删了就是开天窗，不许删整句、不许并句。
- ⚠️ **项目口径压过 skill**（skill 自己也这么规定）。最容易打架的一条：方案 §3.7.2 B 类要求
  「标签 / 按钮 / 状态 / 分类一律用行业通用词」，而「去 AI 味」的惯性是往口语走。
  **去 AI 味不等于改得更土**。实测这一轮 agent 就把状态词改成了「已经开始生成了 / 出好了 /
  这次没算出来 / 没下载完」，全部退回 §3.7.2 的映射（`做好了 → 已完成`、`没做成 → 生成失败`）。
- **术语 sweep 要么全落要么整族退出**，不能只改一半 —— 同一组件里出现两种叫法比不改更糟。
  实测这一轮就漏出过「在『我的数字分身』里随时删除」，而那个页面已经叫「我的数字人」，
  等于把用户指向一个不存在的页面。
- **「出镜」有两种含义，不能一刀切**：用户本人出镜（「不用出镜」「上传你的出镜视频」）保留本义；
  指数字人时长的必须限定成「数字人出镜 N 秒」—— 光秃秃的「出镜 32 秒」会被店主读成
  「要我自己讲 32 秒」，与首页承诺正面冲突。
- **protected spans 里额外要护住的**：法律授权声明正文（`mock.js` 的 `consentText`）、
  文档正式名（《数字分身素材使用说明》）、朗读脚本（`READING_SCRIPT`）、
  以及 AGENTS 已固定的全局文案（如 §0 第 7 条的「登录态已失效，请重新登录」）。
- **完成态不能前移**：`onPhase('processing')` 时还没拿到 `accepted`，就不能说「正在安排训练」。
- 落地方式：改写产物走 `FILE/FROM/TO` 精确替换，**对不上要报错而不是静默跳过**
  （静默跳过的话最后看到「大部分改了」，根本发现不了漏哪条）；解析 `TO:   xxx` 时注意别把
  对齐空格吃成文案内容，实测污染过 20 多处前导空格。改完由 `npm test` + `e2e:weapp` 当闸门。
...
- **e2e 的夹具本身也会说谎**（2026-08-27 实测两次）：① 拍「不可重试」那屏时只翻了 `canRetry` 没换 `loadError`，截出「404 却说网络没通」这种生产里跑不出来的组合，走查照着它得出了一条假 P0；② gate 三态只 `setData({avatarState})` 从不登录，而 `home` 未登录时强制 `avatarState:'missing'`，于是三张图全是游客态——`01-home-guest.png` 与 `02-gate-missing.png` **md5 逐字节相同**；③ 采集页未登录会被全局登录浮层盖住，而**路由断言照样通过**，截出来的是「AI 军师」登录页。**教训：注入的假数据必须是该状态真会出现的值；断言路由不等于断言进对了页。**
...
- Node 运行时统一固定 **24.x**：仓库根 `.nvmrc/.node-version` 与三端 `package.json#engines` 均为 `>=24 <25`；CI 同样使用 Node 24。不要用 Node 22 跑后端全量测试（`unref` 定时器会产生 cancelled），也不要把未验证的 Node 25 当作发布基线。
...
- [ ] **邀请体系的后续四件事（2026-08-18 全站分享 + 关系链落地时明确延后）**：
  ...
  ② **海报素材池服务端下发**——前端 `services/share.js` 的 `posterPool()` 已留读取点（读 `/me` 的 `sharePosters`，拿不到用内置四套兜底），服务端下发与运营后台维护界面未做，**契约里也还没有该字段**，实现时要先补 `shared/contracts.d.ts`。素材图现在是**内置品牌底图**（`app/src/assets/share/card-friend.png` / `card-timeline.png`，各 2KB 出头，随 `src/assets` 进产物）——**不允许改回空串**，留空会让微信截当前页、把私密页内容带进分享卡。运营出图后可换成自己的图（本地或网络 URL 皆可，注意主包体积上限）。
  ③ **漏斗取数侧**——四段埋点本身**已完成**（2026-08-18，口径与落点见 §7.6 末条：`share_expose` / `invite_landing` 两个事件名 + 注册段读 `ReferralAttribution` + 首开通段由 `recordInviteActivation` 写 `ActivationEvent(source='invite')`）。**仍未做的是把四段拼成一条可查的漏斗**：`ClientEvent` 至今**只写不查、没有任何读端点**（§8.1），所以「分享曝光 → 落地打开」这前两段现在只能直接查库；要在后台看漏斗，得先补一个按 `name + createdAt` 聚合的查询（注意 `ClientEvent` 只有 `@@index([name, createdAt])` 与 `@@index([userId])`，按 props 里的 `channel` 下钻要么加索引要么先物化）。另外 `ClientEvent` 仍无留存策略，放量后这两条事件会是最大的写入源（游客照发、每次 onShow 重复进入都报），应与清理/分区任务一并规划。
  ④ **运营后台「邀请增长」四视图**——~~本体 Schema~~ / ~~邀请层级树（物化 lv1/lv2/lv3 正是为它写全的）~~ / 转化漏斗 / ~~IP 风控二部图~~：**三个已落（2026-08-18，见 §9.1）**，只剩**转化漏斗**未做，且它卡在 ③ 的读数侧（`ClientEvent` 只写不查、没有聚合读端点），不是卡在画图。页面上已为它留了第四个 tab 与说明，落地时补在那里、**不要另起一屏**；开通来源那一段本来就在「处方漏斗」页（按 `ActivationEvent.source` 分组），别做第二份。方案见 `docs/[OPUS5]WEAPP_SHARE_REFERRAL_PLAN_2026-08-18.md` §4.3。
- [ ] **开通事件写在支付事务内 + `.catch(() => {})` 是假安全（既有隐患，本次只订正注释未动支付链路）**：`services/wechatPay.ts:732,752` 把 `recordActivation` 传进支付 `tx` 再 catch。Postgres 里那条 insert 一失败，事务即 aborted，catch 吞掉的只是 JS 异常——后续语句连带失败，**整笔支付回滚**。概率低（只有 activation_event 写失败才触发），但后果是真金损失，不是丢一条统计。正确形状见同文件的 `recordInviteActivation`：跑在事务**提交之后**、自带小事务、内部吞掉全部异常。改它要动支付回调的验证面，故单独排期，别顺手改。
- [ ] **建号与归因之间仍有一道提交缝隙**：账号在建号事务里提交，关系绑定在其后的独立小事务里做。2026-08-19 已让 `no_timestamp|config_unavailable` 留痕可在后续登录用同一码签名凭证恢复；但若进程恰在“账号提交完成、绑定函数尚未开始”之间退出，连 attribution 都来不及写，仍无恢复资格。彻底解法是在建号事务里落 pending/outbox，再由后台推进；开放人工补绑或对账修复时应一并实现。**2026-09-02 更新**：人工补绑已开（§7.6 末条 + §8.1 `/admin/invites/manual-bind`），并发建环那半已由有序 advisory lock 关掉；这道**提交缝隙**仍在，且补绑正是它现在的人工兜底手段（运营按 `ReferralAttribution` 的留痕补），不是它的替代品。
- [ ] **邀请相关测试仍有缺口**：`/auth/wechat-phone` 与 `/auth/carrier-onetap` 两个入口只验到「归因字段不改变既有响应」（测试环境到不了微信、运营商入口本身仍返回 501）；`config_unavailable` 分支、恰好 64 节点的无环链边界、绑定事务中断后的恢复都没有用例。小程序侧「朋友圈白名单必须游客友好」与「自定义落地页」两条守卫仍是源码正则，没有实例化页面执行 `withShare` 后的最终回调与游客进页生命周期。
...
- **运营后台剩余 P2 键盘语义债（2026-08-17，2026-08-20 更新）**：会话/调用诊断/审计主行与详情已改为原生按钮 + `useDialogFocus`；仍有少量旧 `div onClick` 行（评测历史、订单复制、商品/基准编辑卡及两处内嵌详情遮罩）主要依赖鼠标。它们不阻断现有运营主链，但下一轮改对应页面时必须拆成显式按钮或可聚焦行，并补 Enter/Space、可见 focus 与详情关闭焦点归还；禁止继续扩散新的 `div onClick`。
- **历史审计链不可逆缺口（2026-08-20）**：新 `admin.http` 已把后台操作者写入 `payload.auth.admin`，审计工作台也正式展示 `requestId/sessionId/operator` 并支持同请求/会话/用户下钻；但修复前的业务审计事件若当时没写 request ID、session ID 或 `by`，无法可靠反向推断，必须如实显示 `-`，禁止按时间邻近硬绑。要让未来每条业务事件与 HTTP 请求严格一对一，需把 request context 贯穿 `recordAudit`（或统一 `recordAdminAudit(req, ...)`）并逐个迁移写点，再补跨请求并发回归。
- **Notion 工程变更日志待补 2026-08-15 宿主机事故闭环 + 2026-08-20 运营后台完整性重构**：前者已完成 Dify 下线、AIStar Clip `/tmp` JAR 泄漏修复、军师预发原子 release 与资源 cgroup，并以真实预发 8925cc2 验收生产/预发/AIStar 三服务健康；后者新增会话工作台、精确审计链与任意时间区间。产品可读结论均在 `docs/CHANGELOG.md` 顶部。当前会话无已安装的 Notion 连接器，无法同步「军师 · 工程变更日志（持续更新）」；连接恢复后应补到页面顶部，完成后删除本条。
...
- **五 tab IA 回归的遗留项（2026-08-18）**：① `/modules`、`enableModule`、`patchModule` 在小程序端仍无 UI 消费方；「已启用的兵器」后续应改读 `/modules` 真数据。② `pages/studio` 默认订阅落地已改到 `pages/execution/index`，但历史分享卡仍可能访问；删除前还需路径访问数据或明确兼容期限。③ 锦囊仍是客户端三路聚合（posters/video works/reports）；模块级缓存只解决重复与并发，不替代量大后的服务端统一作品索引。④ 军令兵器继续只认拆令时落下的 `toolKey`，任务级作品回挂尚未做：需另批定义 `orderId/works[]` 契约、用户/租户归属、跨服务重试与一件作品多军令规则。⑤ H5 tab 配置未随改，随 H5 移除处理。⑥ coach 已升 v3，锦囊入口有一次性「搬到这了」提示；需观察锦囊到达率，掉穿后再评估提权。⑦ `PATCH /modules/:key`（隐藏/排序）全端无调用方。⑧ 今日战果 v1 只收海报；成片需服务端快速聚合/缓存，报告需可靠 createdAt 后再接入。⑨ H5/PC tab 分叉继续登记在 `docs/H5_REMOVAL_TODO_2026-08-10.md`。⑩ 每日 `CasefileMetric` 仍固定为线索/咨询/成交，而每周 `BizMetricWeekly` 已按行业模板动态化；本批只收拢 UI 并保留下游兼容，后续需单独设计动态日指标/数据源自动采集及日→周聚合，确认迁移和旧客户端兼容后再合并两套模型。
...
- 自有登录态支持 JWT（`services/userToken.ts`，HS256）：开发/测试可兼容历史 userId token；production 已改为 fail-fast，强制高强度 secret、`APP_JWT_REQUIRED=true` 与至少一个完整登录通道，缺项直接拒绝启动。短信通道启用时强制真实供应商与验证码校验，不能仅靠运维“记得打开”。
...
- **时序知识图谱**（Graphiti 式，2026-08-19 并发/跨项目收口）已落首版：`GraphEntity/GraphRelation`（关系带有效时间窗）+ `services/knowledgeGraph.ts`（实体按租户去重、新事实软失效旧事实、as-of 查询）+ `routes/graph.ts`（抽取/实体/关系查询）。写入事务用租户 advisory lock 串行，实体 upsert 与关系替换原子化；关系只替换当前项目，项目实体列表同时按实体历史 `projectId` 或本项目有效关系归属解析，因此同一租户实体跨项目复用不会在过滤中消失。抽取依赖真实模型（mock 返回空）。仍可增强：对话汇总/知识入库时自动触发抽取、图谱可视化前端。
...
- **5-tab 设计还原（2026-07）· 前端已跑通但缺后端建模的能力（gap 清单，按优先级）**：
  ...
  5. **排盘引擎 `paipan-v6` 已知边界**（`services/paipan.ts` 头注同步）：称骨暂缓（60 干支年表需可靠来源核对后再上，防带错表）；格局仅月令取格（不处理从格/化格）；身强弱/喜用仍为加权启发式；时间统一取出生证明法定钟表时间，不做真太阳时换算，出生地仅留档；阴历闰月后端支持（负 month）但前端采集 UI 暂未提供闰月选项。存量 v3/v4/v5 首读惰性升级 v6，其中旧记录缺分钟时按原时辰档位中点重算且报告要求用户确认；v1/v2 照旧。以后再升级必须提 `PAIPAN_ENGINE_VERSION`，不得悄改历史命盘。
