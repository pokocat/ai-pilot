...
- **「快出片」分包（2026-08-10）**：原生端 `packages/video` 只允许经自包含 `host.js` 访问宿主，数据源只有 `mock / bff`；真实请求始终同源到军师 `/api/video/**`，禁止恢复会把军师 JWT 发往 AIStar/第三方的 direct 模式。军师 BFF 用固定 service token + `externalOwnerId` 调 AIStar clip 域，积分只在军师侧按权威报价做幂等 hold/settle/refund。端内体验模板以 `catalog.js` 为唯一事实源，列表、详情和 mock 项目骨架必须共用它；当前三套内置模板各有独立文案/镜头结构，接口失败或返回空列表时仍可浏览内置目录。纯 `api.isMock()` 会话使用分包的 200 点演示额度打通确认/出片交互；附身 JWT 与 server 模式必须返回真实军师余额，禁止拿演示额度越过真实扣费。分包自定义导航必须复用 `navInset/navTop/navRowHeight/navRightInset`，标题行绝对定位在胶囊同一层，禁止重新叠加一条 50px 标题栏。本人授权必须录制前置摄像头视频并由 BFF 机器审核后转交石榴 `/authVideo/create`，不得用前端布尔值直通；石榴授权/克隆/TTS/出片/轮询已在隔离预发接真，但真实视频/音频审核、完整多段 ffmpeg 总装或平台发布未配置时仍必须失败关闭，mock 标识不得进入生产成片。当前状态与外部门槛见 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md`。
...
- **「快出片」尚未具备生产上线条件（2026-08-10）**：代码已完成分包、军师 BFF、AIStar clip 域、任务租约/stale reaper、mock 闭环，并已按石榴官方契约把授权/克隆/TTS/出片/轮询部署到隔离预发；不得据此提审或放量。仍需：① 微信确认军师现有 appid 增加数字人口播后的服务类目、重新提审范围和深度合成/生成式 AI 备案；② 用本人合规素材完成方案 §3.2 质量/时延/一致性/规格/成本实测及商务/授权 7 项拍板；③ 接入真实图片/视频/音频机器审核，当前预发/生产上传会 fail-closed；④ 实现并验证多段 avatar/b-roll/尾片 ffmpeg 总装、私有 OSS/CDN 镜像与成片内 AI 标识；⑤ 四平台逐一接真实发布，仅抖音二次短信验证可沿现有选择器复用，其余不得假成功；⑥ DevTools、iOS、Android 真机完成权限拒绝/恢复、100MB 上传、原生 video 层级、下载相册、长任务恢复与扣费幂等验收。里程碑 Notion 工程变更日志本会话无可用连接器，待恢复后同步本条与 CHANGELOG 的产品可读结论。
