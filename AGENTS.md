...
- **「快出片」分包（2026-08-10，2026-08-11 机审与总装更新）**：原生端 `packages/video` 只允许经自包含 `host.js` 访问宿主，数据源只有 `mock / bff`；真实请求始终同源到军师 `/api/video/**`，禁止恢复会把军师 JWT 发往 AIStar/第三方的 direct 模式。军师 BFF 用固定 service token + `externalOwnerId` 调 AIStar clip 域，积分只在军师侧按权威报价做幂等 hold/settle/refund。端内体验模板以 `catalog.js` 为唯一事实源，列表、详情和 mock 项目骨架必须共用它；当前三套内置模板各有独立文案/镜头结构，接口失败或返回空列表时仍可浏览内置目录。纯 `api.isMock()` 会话使用分包的 200 点演示额度打通确认/出片交互；附身 JWT 与 server 模式必须返回真实军师余额，禁止拿演示额度越过真实扣费。分包自定义导航必须复用 `navInset/navTop/navRowHeight/navRightInset`，标题行绝对定位在胶囊同一层，禁止重新叠加一条 50px 标题栏。本人授权必须录制前置摄像头视频并由 BFF 机器审核后转交石榴 `/authVideo/create`，不得用前端布尔值直通。BFF 的 `services/video/aliyunGreenMedia.ts` 已按阿里云内容安全增强版实现图片同步、视频/语音异步审核，只放行 `none/low`；配置、权限、欠费、超时和 `medium/high` 均失败关闭。AIStar v0.113 已完成逐段标准化、多段 ffmpeg 总装、字幕、全程 AI 标识、三套运行时固定品牌尾卡、最终音轨归一、平均亮度/综合响度/真峰值失败关闭和缩略图；预发宿主 180 秒合成探针已通过。当前预发仍因目标阿里云账号尚未开通内容安全且 Chrome 登录 RAM 身份缺 `yundun-greenweb:GetAuditUserConf` 权限而保持 `provider=none`；本人真实素材实测、授权群像尾片/运营 preset 与平台发布未完成时也不得宣称生产闭环。当前状态与外部门槛见 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md`。
