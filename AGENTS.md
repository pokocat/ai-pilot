...
- **邀请漏斗四段（`分享曝光 → 落地打开 → 注册 → 首开通`）的口径与落点**：
  ...
  ② `invite_landing` 由 `services/invite.js` 在 `captureInvite` 捕获成功处上报，props 只有 `channel`（`query`=分享卡 / `scene`=小程序码）。**每次捕获都报**，包含 `onShow` 那次重复进入——同一个人被同一张卡拉回来几次本身就是漏斗要看的数据，去重交给取数侧。
