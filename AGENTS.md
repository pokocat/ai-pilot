...
- **「快出片」当前预发基线（2026-08-11）**：AIStar clip 为 `dcf5f37d-20260812T023005Z`（v0.128、force-mock=false、active/running、`NRestarts=0`），AI 水印缺省关闭且测试演示标识独立保留；军师预发后端为 `9285a9b`（active/running、`NRestarts=0`，公网 health 与 pgvector 2/2 正常），API 固定 `https://wxapi.aibuzz.cn/api_preprod`，同 SHA 原生 server 包已 auto-preview。后续回归不得拿 mock 包冒充本人素材验收。
