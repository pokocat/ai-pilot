...
- **AI 排版引擎（第 3 档，2026-07-29；`layoutEngine='ai'` 是默认值 = 部署即切换）**：模型**自己写整张海报的 HTML/CSS**，而不是往三套模板里填空。动因是真机那张图的画质问题（撞色 + 被画成占位卡片的"负空间" + 六维度哲学只被消费了 palette），而上游 `canvas-design` 根本不用图片模型——「哲学长文 → 用代码在画布上创作 → 强制二次打磨」。链路：`manifesto.ts`（LLM #1：中文宣言 4–6 段 + 色板 + 隐性主题）→ `canvasEngine.ts`（LLM #2：540×720 整页 HTML，纯 CSS/SVG 作画，**不调图片供应商**）→ `canvasSanitize.ts` 静态审计 → 占位符替换 + AI 标识兜底注入 → `renderer.renderCanvasPoster`（加固渲染）+ `canvasMeasure.ts` 量测 → **无条件打磨一轮** → 最多再修一轮。
  ...
  - **轮次与预算**：HTML 相关 LLM 调用 ≤4 次（`MAX_HTML_CALLS`），看图评审 ≤2 次（`MAX_VISION_CALLS`，独立计数），整段 ≤300s（`AI_ENGINE_BUDGET_MS`；仍小于 `STALE_RUNNING_MS`，别把它调过 10 分钟）。每轮都要跑一次真实渲染，成本与时延都在这里。**单次渲染超时取 `min(后台 timeoutMs, 引擎剩余预算)`**（下限 15s）——没有这一层的话，运营把渲染超时配到上限 480s 时一次渲染就能顶穿 sweep 的 10 分钟，同一单被跑两遍。
  - **看图打磨闭环（2026-08-12）**：此前整条链路**没有任何一个环节在评审审美**——量测器量的是越界/重叠/字号这类事故，审美只由模型盲写那一次决定，之后再没人看过。这是「军师出的图不如 Claude Code 里出的图」的第一位原因。现在补上：`visualCritique.ts` 把**渲染出来的 PNG** 交回模型做艺术总监评审（构图/层级/留白/色彩/排印/收口），产出「判定 + 最多 5 条具体意见」，意见逐条回喂给打磨轮；**打磨轮本身也带着那张图发出去**（作者要看见自己画成了什么样，而不是对着自己写的 HTML 凭空想象）。
    ...
    - **指标 `junshi_creative_critique_total{verdict=pass|revise|unavailable}`**：`unavailable` 的斜率就是「看图打磨在生产悄悄失效」的告警信号（与 `template_fallback` 同类：这条链路失效时画面不报错，只是悄悄变回没人看过的样子）。留痕另见 `resultJson.visualCritiques` / `critiquePassed`。
  ...
  - **创作轮开思考、产出预算 12k**：`allowThinking` 与 `images` 是 2026-08-12 给 `gateway.completeText`/`rawText` 加的两个可选参数，**缺省关**（既有辅助抽取行为一字未变）。海报创作显式打开：上游 canvas-design 在 Claude Code 里就是长思考之后才动笔。HTML 的产出格式容错（`<!DOCTYPE` 起始 + 围栏剥离兜底），不怕 thinking 把格式约定带偏——**依赖尾部格式约定的调用方（ask 块那类）绝不能开**。`HTML_MAX_TOKENS` 6000 → 12000：一页有密度的手写 CSS 光样式就能写掉 4k，6000 的顶只够交「结构清楚但细节稀」的版面。
  - **高级海报档（`tier`，2026-08-12）**：用户在确认页选的**档位**，不是运营的全局开关。`standard`=模型纯 CSS/SVG 写整页（原行为）；`premium`=先由图片大模型出**全幅无文字主视觉**，再由渲染器把中文/Logo/二维码排上去。**高级档不是「让图片模型把整张海报连字画出来」**——中文仍确定性排版（§4.1 原则未变：模型写的中文不可校验，主标题里一个错字就是信任事故，而它恰恰最难被自动发现）。
    - **不可用一律 422，绝不静默降标准**（`PREMIUM_UNAVAILABLE`）。可用 = 供应商已配启用 **且** `aiMode !== 'graphic'`（那是影像路线的熔断闸）。判断只有一个实现 `premiumTierAvailable()`，同时供 `/creative/status` 的 `premiumAvailable`（前端据此**整块隐藏**高级档选项）与建单闸门用——两处各写一遍就会重演「前端露着入口、后端一律 422」。
    - **高级档 + 本人照片 → 建单即 422**（`PREMIUM_PORTRAIT_CONFLICT`）：模型画的人与用户的照片必然打架，而这件事建单时就知道，不该先扣 25 钻再退。
    - **失败不降级交付**：photo 链走不通 / 路线没落到 photo / AI 引擎未产出 → **整单失败 + 全额退款**（复用既有幂等退款，不造部分退款），`PREMIUM_VISUAL_FAILED`。这与标准档的三层回落链**刻意相反**，别以「统一行为」为由抹平：标准档承诺「给你一张海报」，高级档承诺的是「顶级图片模型出的主视觉」。重试次数未耗尽时仍先回队重试。
    - **定价独立**（`premiumPricePerPoster`，默认 25 钻）不是倍率——倍率会在改标准价时把高级价一起带偏。`priceForTier()` 是唯一定价口径，create 与 regenerate 共用；revise 不调供应商、不计费、不设门禁。
