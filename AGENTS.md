...
- Claude 模型（`provider=claude` 或 OpenAI 兼容模型名含 `claude`）可在后台配置 `thinkingMode=disabled|enabled|adaptive`；`enabled` 的 `thinkingBudget` 限 1024–7000（业务普通聊天 `max_tokens=8000`，必须留出正文预算）。开启 `enabled/adaptive` 后 temperature 自动锁为 `1`；关闭时七牛等第三方网关显式发送 `thinking.type=disabled`，且**不得携带 `budget_tokens`**（七牛仅在 `enabled` 时接受预算字段，`disabled + budget_tokens:0` 会返回 400），避免网关默认开启思考；Anthropic 官方直连则按官方协议省略 thinking。后台“测试连接”与普通聊天必须携带同一 Thinking/temperature 配置。结构化成果及多轮工具调用显式关闭 Thinking：Anthropic Thinking 只允许 `tool_choice=auto/none` 且要求跨轮保留 thinking block，与现有强制 `emit_deliverable` 收口不兼容，不能为开关破坏成果链路。
