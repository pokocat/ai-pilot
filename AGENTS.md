...
- **小程序上传鉴权（2026-07-25 现状）**：本机 DevTools CLI 服务端口已开启，但微信登录会话已退出，上传报 `code 10`，需在开发者工具右上扫码恢复登录；本机备用密钥 `/Users/donis/dev/aliyun/private.wx810ebe6dfef8e75f.key` 可被 `miniprogram-ci` 正常读取、编译和打包，但当前出口 IP `120.204.218.229` 未加入小程序后台上传密钥白名单，微信接口返回 `-10008 invalid ip`。两条路任选其一恢复即可重试：扫码登录走 DevTools CLI，或在 mp 后台把当前出口 IP 加白后走 `npm run upload:weapp`；云端执行环境另有 `servicewechat.com Host not in allowlist` 的历史限制。
