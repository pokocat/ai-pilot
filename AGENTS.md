...
- `services/scheduler.ts`（M1 定时任务框架，2026-08-19 多实例选主）：任务注册制 + 进程内周期扫描；每个生产 job 先在独立事务中取得 PostgreSQL `pg_try_advisory_xact_lock(hashtext('junshi:scheduler:<name>'))`，锁覆盖本轮执行，因此同一数据库上的多实例每轮至多一个执行者；`NODE_ENV=test` 不自启并绕过锁，测试直接 `runJob/scan*` 驱动。`SCHEDULER_ENABLED=false` 仍可让纯 API 实例完全不扫描，但不再依赖人工“只能开一个 true”。任务彼此隔离（单任务崩不影响其它）。已挂：`account-erasure-sweep`（到期注销数据清理）、`video-maintenance-sweep`（视频模糊提交/克隆卡死对账）、`casefile-idle-recall`、`daily-review-reminder`、`review-gap-reminder`、`prophecy-due-scan` 等。视频维护不得在每个路由实例另起 timer。
