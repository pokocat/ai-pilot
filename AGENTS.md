...
- **邀请归因与关系链契约（2026-08-18）**：`LoginAttribution` 的 `inviteCode?` / `inviteCodeAt?`（ms epoch）挂在三条**建号**通道的请求体上（`/auth/login`、`/auth/wechat-phone`、`/auth/carrier-onetap`），两个字段都可选；**任何脏值都不得让登录 400**，但也不等于「当没传」：带了码而形状/类型不对（含 `inviteCode: 123`）仍落一条 `unknown_code` 留痕，`inviteCodeAt` 缺失或非正整数则落 `no_timestamp` 且**不建关系**（判不了新鲜度就不建一条改不回来的边）。归因窗口的判定权在服务端（天数归运营配置，开关目录里的 **`referral-window`**；它与奖励键分属两个 flag，因为 `setFeatureFlagPayload` 是整块覆盖写、共用会互相抹掉。读取时新键优先、**回退旧 `referral.window`** 以兼容搬迁前的存量配置，缺省 30 天），客户端只如实上报「码」和「什么时候拿到的」，不写死业务天数。`ReferralSummary` 挂 `/me.referral`：`directCount` / `activatedCount` **只数直接下级**（关系链物化存三级，但对用户呈现与激励口径只看一级）；整个字段为 `null` 表示**本次没读到**，与「读到了但是 0」必须能区分开（把读失败画成 0 会让人以为邀请没被记上）。
...
- **邀请漏斗四段（`分享曝光 → 落地打开 → 注册 → 首开通`）的口径与落点**：
  ...
  ④ 首开通段从 `ActivationEvent(source='invite')` 算，写入方是 `services/activation.ts` 的 `recordInviteActivation`，挂在 **`markPaidAndApply`**（真金入账唯一收口）而**不是** `applyPlanPurchase`——后者还被注册测试期自动开通 / 演示购买 / 运营手工开通三条**非付费**路径共用，挂那儿会把免费白发算成邀请开通，开着注册自动开通时被邀人「注册当场即首开通」，这一段永远 100%；而且那条路径跑在建号事务内，关系是建号提交后才绑的，那一刻查推荐人必然为空。
  ...
  - **不阻断主链路的实现方式**：`recordInviteActivation` 在支付事务**提交之后**跑、自带小事务、内部吞掉全部异常（永不 reject，只记 warn）。塞进支付事务里是错的：Postgres 事务内任一语句失败即整体 aborted，`.catch(() => {})` 也救不回来（`services/referral.ts` 的 P2002 分支踩过一次），一次 DB 抖动就能回滚一笔已成功的入账。端上两条埋点全走 `api.track`（裸 `wx.request` + 空回调，见 §7.2）：分享回调必须**同步返回**给微信，所以埋点不 await 且整段 try 住，连 `require('./api')` 都在 try 里；`invite.js` 里刻意是**函数体内懒 require**——`services/api.js` 顶层就 `require('./invite')`，顶部引用直接成环，CJS 环不报错但会拿到半初始化 exports、症状是静默变哑。
...
- [ ] **invite 归因补记没有运营可点的补救入口**：修好的是「微信自己重投」这条路（`markPaidAndApply` 的 `already_applied` 分支也会派发一次补记，函数幂等）。但 `reconcileOrder`（查单对账 / 运营手工补账）在开头就 `if (order.appliedAt) return 'already_applied'` 短路，**根本走不到 `markPaidAndApply`**。所以万一微信的重试次数在补记持续失败期间耗尽，该用户就永久缺 `source='invite'`，运营手上没有任何按钮能重跑。要补需在 `reconcileOrder` 或 admin 侧单开一个「重跑归因补记」动作（只补 ActivationEvent，不动权益发放）。
...
- [ ] **建号与归因之间仍有一道提交缝隙（本期不做，如实记账）**：账号在建号事务里提交，关系绑定在其后的独立小事务里做。若进程恰在两者之间崩溃、或绑定事务超时，会出现「账号建成、关系没有、连 attribution 也没写进去」的黑洞；而该用户下次登录走 `existing` 分支、**不追认**，于是永久丢失归因。当前缓解：绑定整体失败时由 `bindReferralAfterRegister` 在事务外补一条留痕（至少「有人带着这个码进来过」可查）。彻底解法是**在建号同事务里落一条 pending/outbox 行**，再由后台重试推进——涉及新表与重试 worker，本期不做。开放人工补绑或对账修复时应一并实现。
