...
- **「快出片」真实供应商链已具备本人真机验收条件，但尚未具备生产上线条件（2026-08-11）**：代码已完成分包、军师 BFF、AIStar clip 域、石榴官方采集/训练/生成契约、可选 `authId`、V2 统一音频驱动、多段 ffmpeg 总装、字幕/AI 标识/缩略图和任务恢复；自动化只跑桩，不消耗供应商点数。隔离预发部署当前仍是军师 `f6cb58d` / AIStar `b5140a8a-20260811T132756Z`，v0.118 发布后须回填新版本；两服务 active、`NRestarts=0`，AIStar `AEP_CLIP_FORCE_MOCK=false`。默认创建已改为“声音 → 形象 → 训练”，不再要求另录授权视频；requirements 应返回 `authorizationVideoRequired=false`、形象硬门 5 秒、声音服务端 `>2s`/端上 3 秒。测试预发仍旁路媒体审核，production 硬拒绝。仍需：① 微信类目、重新提审与深度合成/生成式 AI 备案；② 阿里云内容安全正式授权并关闭旁路；③ 用户用本人合规素材完成音色、口型、训练/出片耗时、规格与成本实测，并拍板商务/素材条款；④ 运营 preset 与真实长视频压力验收；⑤ 四平台真实发布；⑥ iOS/Android 真机完成权限拒绝/恢复、100MB 上传、原生 video 层级、下载相册、长任务恢复与扣费幂等验收。里程碑 Notion 工程变更日志本会话无可用连接器，待恢复后同步本条与 CHANGELOG 的产品可读结论。
