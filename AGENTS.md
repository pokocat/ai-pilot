...
- **「快出片」尚未具备生产上线条件（2026-08-11）**：代码已完成分包、军师 BFF、阿里云多模态机审适配器、AIStar clip 域、任务租约/stale reaper、逐段标准化、多段 ffmpeg 总装、字幕/全程 AI 标识/缩略图和 mock 闭环，并已按石榴官方契约把授权/克隆/TTS/出片/轮询部署到隔离预发；不得据此提审或放量。仍需：① 微信确认军师现有 appid 增加数字人口播后的服务类目、重新提审范围和深度合成/生成式 AI 备案；② 正确阿里云主账号开通内容安全增强版、给专用 RAM 身份授权并落预发凭据，随后用本人合规素材完成图片/视频/语音真实审核与方案 §3.2 质量/时延/一致性/规格/成本实测及商务/授权 7 项拍板；③ 固定正式尾片素材并补亮度/响度阈值与真实长视频压力验收；④ 四平台逐一接真实发布，仅抖音二次短信验证可沿现有选择器复用，其余不得假成功；⑤ DevTools、iOS、Android 真机完成权限拒绝/恢复、100MB 上传、原生 video 层级、下载相册、长任务恢复与扣费幂等验收。里程碑 Notion 工程变更日志本会话无可用连接器，待恢复后同步本条与 CHANGELOG 的产品可读结论。
