...
- **邀请归因与关系链契约（2026-08-18）**：`LoginAttribution` 的 `inviteCode?` / `inviteCodeAt?`（ms epoch）挂在三条**建号**通道的请求体上（`/auth/login`、`/auth/wechat-phone`、`/auth/carrier-onetap`），两个字段都可选；**形状不合法的码一律当没传，绝不返回 400**——分享链路不能因为一个脏参数把登录拦下来。归因窗口的判定权在服务端（天数归运营配置 `FeatureFlag.referral.window`，缺省 30 天），客户端只如实上报「码」和「什么时候拿到的」，不写死业务天数。`ReferralSummary` 挂 `/me.referral`：`directCount` / `activatedCount` **只数直接下级**（关系链物化存三级，但对用户呈现与激励口径只看一级）；整个字段为 `null` 表示**本次没读到**，与「读到了但是 0」必须能区分开（把读失败画成 0 会让人以为邀请没被记上）。
...
- **邀请漏斗四段（`分享曝光 → 落地打开 → 注册 → 首开通`）的口径与落点**：
  ...
  ③ 注册段**不占事件名**：从 `ReferralAttribution` 算（每次带码进线一行、六种 outcome 全留痕），分母/分子都比客户端埋点准；再补一份端上埋点只会出现「端报了、库里没有」的对不上账。
...
### 9.1 邀请增长三视图（2026-08-18，P3；`admin/src/views/referral.tsx` + `server/src/routes/adminReferral.ts`）

「经营 · 邀请增长」一页四个 tab，**一份数据三个投影 + 一个留位**，全只读：

- **视图选型有理由，不是审美**：① 本体 Schema 用 **UML 类图**（看字段与基数），**刻意不用力导向图**——实例一多就是毛球，且每次刷新布局都不一样、没有阅读逻辑；② 邀请关系树用**从左到右的层级树**（邀请关系天然有方向与层级），节点大小=直邀人数、底色=开通状态、红环=风控标记，点节点展开/折叠；③ 风控用 **IP ↔ 新号二部图**——正常用户分散在各自 IP、刷号聚集成一个 IP 连一串新号的扇形，这个形状要一眼可见；④ **转化漏斗留位未做**，卡在 `ClientEvent` 没有聚合读端点（§13 ③），不是卡在画图。
...
- **端点单独成文件**：`routes/adminReferral.ts` 自挂同一把 `requireAdmin`（admin.ts 已 2900+ 行，三个投影各要一次成形）。树接口用**物化路径一条 `OR`（lv1/lv2/lv3 各有索引）取全三级子树**，配合一次 `groupBy(lv1)` 拿到任意深度都准的直邀数 + 一次 `findMany` 补名字/套餐，**没有逐层展开的 N+1**；根 = 直邀最多的前 N 人（投影不是全景，排序稳定可预测）。风控接口两步走：`groupBy(clientIp)` 拿候选（记录数 ≥ 阈值是去重新号数的安全超集，不会漏判）→ 只对候选拉明细在内存里按 `newUserId` 去重，顺带回 `scannedIps`，让**「扫了 200 个 IP 没有聚集」与「一条数据都没有」在界面上分得开**。
