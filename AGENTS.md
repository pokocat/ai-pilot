...
- **「快出片」分包（2026-08-10，2026-08-11 AI 共创/多句镜头/机审与总装更新）**：原生端 `packages/video` 只允许经自包含 `host.js` 访问宿主，数据源只有 `mock / bff`；真实请求始终同源到军师 `/api/video/**`，禁止恢复会把军师 JWT 发往 AIStar/第三方的 direct 模式。军师 BFF 用固定 service token + `externalOwnerId` 调 AIStar clip 域，积分只在军师侧按权威报价做幂等 hold/settle/refund。文案页的 AI 对话走军师现有 LLM 路由并把 `scriptChat` 随项目持久化，不得挪用石榴做文案生成；`segments` 只负责逐段改稿，视觉编排统一写 `shots[{startNo,endNo,role,assetId}]`，配画面允许连续多句共用一个镜头。旧草稿无 shots 时按相邻 b-roll 每 3 句防御性生成；报价、preflight 与 AIStar worker/总装必须消费同一聚合投影，禁止只在端上“看起来合并”而后端仍逐句切片。端内体验模板以 `catalog.js` 为唯一事实源，列表、详情和 mock 项目骨架必须共用它；当前三套内置模板各有独立文案/镜头结构，接口失败或返回空列表时仍可浏览内置目录。纯 `api.isMock()` 会话使用分包的 200 点演示额度打通确认/出片交互；附身 JWT 与 server 模式必须返回真实军师余额，禁止拿演示额度越过真实扣费。分包自定义导航必须复用 `navInset/navTop/navRowHeight/navRightInset`，标题行绝对定位在胶囊同一层，禁止重新叠加一条 50px 标题栏。本人授权必须录制前置摄像头视频并由 BFF 机器审核后转交石榴 `/authVideo/create`，不得用前端布尔值直通。BFF 的 `services/video/aliyunGreenMedia.ts` 已按阿里云内容安全增强版实现图片同步、视频/语音异步审核，只放行 `none/low`；配置、权限、欠费、超时和 `medium/high` 均失败关闭。AIStar v0.115 已完成多句镜头投影、逐段标准化、多段 ffmpeg 总装、字幕、全程 AI 标识、三套运行时固定品牌尾卡、最终音轨归一、平均亮度/综合响度/真峰值失败关闭和缩略图；预发宿主 180 秒合成探针已通过。当前预发仍因目标阿里云账号尚未开通内容安全且 Chrome 登录 RAM 身份缺 `yundun-greenweb:GetAuditUserConf` 权限而保持 `provider=none`；本人真实素材实测、授权群像尾片/运营 preset 与平台发布未完成时也不得宣称生产闭环。当前状态与外部门槛见 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md`。
...
- **「快出片」测试工程链已可闭环，但尚未具备生产上线条件（2026-08-11）**：代码已完成分包、军师 BFF、阿里云多模态机审适配器、AIStar clip 域、任务租约/stale reaper、逐段标准化、多段 ffmpeg 总装、字幕/全程 AI 标识/缩略图；隔离预发可显式旁路媒体审核并生成永久带「测试演示」的真实可播放 MP4，仍走积分、总装、质检、封面和存储链。`CLIP_MEDIA_MODERATION_BYPASS` 只允许非生产并留审计，production 启动硬拒绝；AIStar force-mock 同样禁止 production/mysql。仍需：① 微信确认军师现有 appid 增加数字人口播后的服务类目、重新提审范围和深度合成/生成式 AI 备案；② 正确阿里云主账号开通内容安全增强版、给专用 RAM 身份授权并落预发凭据，随后用本人合规素材完成图片/视频/语音真实审核与方案 §3.2 质量/时延/一致性/规格/成本实测及商务/授权 7 项拍板；③ 授权群像尾片或运营 preset 与真实长视频压力验收；④ 四平台逐一接真实发布；⑤ DevTools、iOS、Android 真机完成权限拒绝/恢复、100MB 上传、原生 video 层级、下载相册、长任务恢复与扣费幂等验收。里程碑 Notion 工程变更日志本会话无可用连接器，待恢复后同步本条与 CHANGELOG 的产品可读结论。
