...
- `Referral`（邀请关系，`userId` 主键 = **单推荐人公理**，绑定后不可变更）+ `ReferralAttribution`（**每次带码进线都落一行，成功与失败都留痕**，含 `clientIp`/`userAgent` 作风控原料）。物化路径 `lv1/lv2/lv3` **从第一天就写全**（运营侧邀请树要看完整链路），但对用户呈现与将来的激励口径**只看一级**。判环**必须沿 `referrerId` 递归上溯**（物化只到三级，A→B→C→D 这种链上只比 lv1/lv2/lv3 会漏判并建出环），带 hop 上限兜底。`outcome ∈ bound|self|unknown_code|expired|already_bound`。
