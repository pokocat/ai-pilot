...
- **登录身份核对契约（2026-08-14）**：`LoginResult.phoneBinding?: LoginPhoneBinding` 只由手机号快捷登录返回，区分 `matched / placeholder_upgraded / mismatch`。`mismatch` 表示登录已经成功进入原微信身份对应账号，客户端只做非阻断说明；两个手机号字段均为脱敏展示值，不得据此改写账号归属。
...
- **账号归属与换号铁律（2026-08-14）**：账号归属只认不可变 `User.id`，同一 openid/unionid 必须进入同一账号。`POST /auth/wechat-phone` 只有在原手机号为历史 `wx_<openid>` 占位值时才可自动升级；已有真实绑定号与本次授权号不一致时，登录仍成功进入原账号，但服务端不改手机号并返回脱敏 `phoneBinding.status=mismatch`，端上在登录完成后非阻断提示去设置换绑。设置中的显式换绑继续用新号码短信验证码（`scene=bind`）确认，老号失效也不能把用户锁死；跨账号占用返回 `PHONE_TAKEN`，微信凭证唯一键冲突返回 `WECHAT_ACCOUNT_CONFLICT`，数据库故障不得吞成登录成功。
...
| `POST /auth/wechat-phone` | 小程序手机号快捷登录：同微信身份优先复用原账号；仅 `wx_` 占位号自动升级，真实号不一致时返回脱敏提示且不静默换绑 | 否 |
