...
- **「快出片」分包（2026-08-10，2026-08-11 AI 共创/多句镜头/石榴 v0.116 更新）**：原生端 `packages/video` 只允许经自包含 `host.js` 访问宿主，数据源只有 `mock / bff`；真实请求始终同源到军师 `/api/video/**`，禁止恢复会把军师 JWT 发往 AIStar/第三方的 direct 模式。军师 BFF 用固定 service token + `externalOwnerId` 调 AIStar clip 域，积分只在军师侧按权威报价做幂等 hold/settle/refund。文案页 AI 对话走军师 LLM 并持久化 `scriptChat`；视觉编排统一写 `shots[{startNo,endNo,role,assetId}]`，报价、preflight、worker 与总装消费同一聚合投影。端内三套体验模板以 `catalog.js` 为唯一事实源。本人授权、形象、声音采集必须先读 `/api/video/avatar/requirements`：官方限制与产品质量门分开呈现，端上做时长/大小前检，BFF 校验 MIME/大小，AIStar 再用 ffprobe 验真实时长、H.264/分辨率/音轨；禁止只信客户端元数据。授权口播必须使用服务端下发文本，`authId` 只表示石榴受理授权声明，不得展示为独立实名认证。声音固定 V2.0；所有非尾段先生成同一 speaker 的段音频，avatar 走 `/video/createByVoiceV2`、b-roll 复用同一音频，禁止恢复 `createByText` 与独立 TTS 混用。训练页只展示官方真实进度/失败原因，重新采集必须支持只重做失败项。BFF 媒体机审只放行 `none/low`；测试期旁路只允许非 production 并留审计。AIStar v0.116 已包含多段 ffmpeg 总装、字幕/AI 标识、固定品牌尾卡、音轨归一、亮度/响度/真峰值门和缩略图。本人真实素材质量/耗时/成本实测、授权群像尾片/运营 preset 与平台发布未完成时不得宣称生产闭环。当前状态见 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md`。
