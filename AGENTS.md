...
五个 tab 的页头统一由 `components/TabHeader` 渲染（对齐最新交互原型 `header`）：**一行小字用途（本命色，字距 .3em）+ 大字 tab 名（宋体 29px）+ 背景一枚大字（本命色 104px / 透明度 .09，贴右上、字脚被下方细线切齐）+ 底部细线**。**页头零按钮**——原型右侧那枚「行业 tag」是装饰，按定稿去掉；`TabHeader` 也不再提供 `right` 插槽，任何功能入口都必须回归它所属的内容区（见下表「原页头入口去向」）。三要素定义在各页调用处，勿散落到样式里：
...
| `pages/thinktank` | 锦囊 | 存你的家底 | 囊 | 本来就没有 |
| `pages/profile` | 老板 | 你自己 | 板 | 设置 → 「系统」菜单组首行 |
