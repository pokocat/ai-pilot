...
- [ ] **建号与归因之间仍有一道提交缝隙**：账号在建号事务里提交，关系绑定在其后的独立小事务里做。2026-08-19 已让 `no_timestamp|config_unavailable` 留痕可在后续登录用同一码签名凭证恢复；但若进程恰在“账号提交完成、绑定函数尚未开始”之间退出，连 attribution 都来不及写，仍无恢复资格。彻底解法是在建号事务里落 pending/outbox，再由后台推进；开放人工补绑或对账修复时应一并实现。
- [ ] **两个未绑定用户并发互邀的 TOCTOU 未加锁**：A 用 B 的码、B 用 A 的码同时注册时，两侧的无环检查都可能看不到对方的边，从而建出二元环。当前**不构成线上风险**：公开入口只在**新建账号**时绑定，而双方要拿到彼此的邀请码就得先都已存在，老用户又禁止追认；也还没有人工补绑入口。**但在开放人工补绑、归因重放或对账修复之前必须先加锁**（按 id 有序取双用户行锁 / advisory lock / 可重试的 Serializable 事务），否则那条路径会真的建出环。
