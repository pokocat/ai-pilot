...
- **观测信息人话化与审计降噪**：调用诊断用 `LlmTrace.userId/tenantId/sessionId/agentKey` 回填来源用户、手机号、租户与智能体中文名；界面主信息显示「对话回复/方案生成」等业务标签，`general/deliverable/ip` 原始 key 只留排障标识。`/api/metrics` 是 Prometheus 机器抓取，不写 `AuditLog`；修复前历史抓取默认过滤，只有显式 `includeMetrics=true`/「含监控抓取」才展示，避免淹没用户行为。
