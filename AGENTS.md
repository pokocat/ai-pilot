...
- **「快出片」真实供应商链已具备本人真机验收条件，但尚未具备生产上线条件（2026-08-11）**：代码已完成分包、军师 BFF、AIStar clip 域、石榴官方采集/授权/训练/生成契约、V2 统一音频驱动、多段 ffmpeg 总装、字幕/AI 标识/缩略图和任务恢复；自动化只跑桩，不消耗供应商点数。隔离预发当前部署军师 `9a1700b` / AIStar `458246e1-20260811T124030Z`，两服务 active、`NRestarts=0`，AIStar `AEP_CLIP_FORCE_MOCK=false`；石榴只读权益为 12000 点、0 分身、0 音色。真机包已指向 `https://wxapi.aibuzz.cn/api_preprod` 推到 AppID `wx810ebe6dfef8e75f`，后续用户本人提交才会扣点。测试预发仍旁路媒体审核，production 硬拒绝。仍需：① 微信类目、重新提审与深度合成/生成式 AI 备案；② 阿里云内容安全正式授权并关闭旁路；③ 用户用本人合规素材完成音色、口型、训练/出片耗时、规格与成本实测，并拍板商务/授权条款；④ 授权群像尾片或运营 preset 与真实长视频压力验收；⑤ 四平台真实发布；⑥ iOS/Android 真机完成权限拒绝/恢复、100MB 上传、原生 video 层级、下载相册、长任务恢复与扣费幂等验收。里程碑 Notion 工程变更日志本会话无可用连接器，待恢复后同步本条与 CHANGELOG 的产品可读结论。
