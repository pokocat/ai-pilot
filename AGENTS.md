...
**对话核心 `weapp-native/chat-core/`（主包，2026-08-08 抽取）**：分包可以引用主包，反向不行——所以问策对话的可复用部分必须住在主包。四件物：`behavior.js`（Page Behavior，基础库 2.9.2+；承载消息加载/规范化、发送、SSE 流式对接、生成态/停止/重试、键盘避让、粘贴归卷、asks 问答卡、成果卡闭环）、`message-list.wxml` 与 `composer.wxml`（`<template name>` 模板库，宿主页 `<import>` 后按各自 data 渲染）、`chat-core.scss`（可共享样式，宿主页 `@use`）。宿主页只留**页面专属**的四件事：导航参数解析（解析完交给 `chatCoreLoad(config)`）、页头、返回键、滚动容器（`scroll-into-view` 留在宿主页，避免流式 180ms 自动滚底把整张消息列表带进重算）。`packages/main/chat` 是第一个宿主，问策 tab 内嵌总军师对话是第二个。两条硬约束：① 模板里绑定的 handler 名必须与 behavior 提供的方法名一致，`npm test` 会逐个核对；② 模板自己没有 json，用到的 `native-icon / markdown-text / report-card / towxml` 由**宿主页**注册，测试逐标签核对。towxml（568K）仍留在 `packages/main/vendor/towxml` 分包，主包的 chat-core 不得反向 require 它——由同包宿主页 `useStreamRenderer({ setMdText, setStreamFinish, stopImmediatelyCb })` 注入，新宿主页不注入就没有流式打字机。
...
| `GET /wence/hints` | 问策提示问题 pill 词池：enabled 的 `kind='hint'` 模板（`id`+`text`，按 sort）。**空池是合法状态**回 `{hints:[]}`，端上回退本地兜底池。**不鉴权**——游客也要能看提示词（登录门不得前置） | 否 |
...
- 全程 mock 模型（确定性、可复现），无需真实 key/pgvector。**现状 1457 用例 / 242 套件（0 跳过）**；覆盖微信登录 openid 复登、运营后台鉴权、算力/套餐购买（含挂牌价/优惠价生效窗）、大模型接入配置、智能体权益、军师档案访谈、问策入口 WP1（分桶/主动消息/词池/埋点）与用户主路径。最近一次本地 PostgreSQL 测试库全量实跑为 2026-08-08（问策入口 WP1 落地后，独立测试库 `junshi_test_wence`），1457/1457 全过。
...
- **问策入口改版 WP1 之后未做的（2026-08-08，本包只交契约 + 服务端，端上一行未动）**：规格 `docs/[FABLE5]WENCE_ENTRY_INTERACTION_SPEC.md`。
  - **端上全部形态未实现**：合一页头、对话区进场序列（greet → 输入中 → 主动判断 → chips）、提示 pill、底部合体浮岛、军师团/历史抽屉、CoachMarks 第一步改文案，都还没写；`/me.features.wenceForm` 现在**下发了但没人读**。分桶开关默认关，所以线上无变化。
  ...
  - **埋点白名单少四个事件**：规格 §5 列了 `hint_show / advisor_open / attach_pick / drawer_seg`，本包白名单只放了八个高频项。补的时候改 `routes/wence.ts` 的 `EVENT_NAMES` **和** `shared/contracts.d.ts` 的 `ClientEventName` 两处。
