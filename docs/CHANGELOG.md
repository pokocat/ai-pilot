...
### 2026-09-02 · 修复后台 StrictMode 骨架屏与刚写入记录短暂不可见 · 影响面：运营后台取数、调用诊断、审计工作台、时间筛选回归

`useResource` 原先用组件级 `alive` ref 管理异步响应，只在卸载 cleanup 置为 `false`。React 18 StrictMode 在开发环境会执行 mount → cleanup → mount，第一次 cleanup 因而把该 ref 永久烧死，第二次挂载发出的请求无论成功、失败还是 finally 都被丢弃：数据始终为空、loading 永远为 true，所有后台页面只剩骨架屏。现改为每一轮 effect 的局部 `cancelled` 标记，cleanup 仅作废本轮请求；跨筛选请求仍以递增 `reqId` 保证旧响应不能覆盖新结果。新增真实 `react-dom/client + StrictMode` 回归，覆盖成功、失败、403、旧请求晚回与卸载后到货。

后台滚动时间筛选此前将查询上界严格钉在 Node 的 `new Date()`，而 `createdAt` 由数据库 `now()` 生成；两者同毫秒时，`lt` 会稳定排除刚落库的一行，运营刷新「调用诊断」或审计工作台会短暂看不到刚发生的记录。滚动预设现把仅供查询的开区间上界越过当前 60 秒，对外展示的结束时间和天数仍以真实现在为准；自定义日期范围不加余量，避免越界查入后一天。缺省 trace 查询不再多加 `lt now`。后端回归连续 12 次验证 trace / 审计「写入后立刻可见」。

### 2026-08-29 · 修复定时任务连接池饿死并清理支付/注册噪音告警 · 影响面：后端调度器、Prometheus 规则、Grafana 业务大盘、运营功能开关

生产只读证据确认，「支付自动对账停止」不是支付资损：近 7 天没有支付订单、没有已付未发放订单，真正故障是多个定时任务在同一秒并发启动。每个任务先占一个 Prisma 连接持有 PostgreSQL advisory transaction lock，随后任务本体又通过全局 Prisma 申请第二个连接；生产池上限为 5 时，五个以上任务会把连接全部占成“拿着锁等连接”，最终一起报 `Timed out fetching a new connection`，还会短暂拖累健康检查。

调度器现改为进程内严格 FIFO 串行队列，同名任务已排队或运行时去重；跨实例 advisory lock 保留，因此既不会单机自锁，也不会多实例重复执行。新增统一任务指标（周期、启用态、健康锚点、最近成功/失败/耗时、在途和结果计数）与 `JunshiSchedulerJobStalled`，超过 `max(3 个周期, 15 分钟)` 未成功才提示。支付规则改成「确有待处理订单 + 对账连续未成功」才告警；没有订单时只看通用调度健康，不再冒充 P1 资损。

「72 小时零注册」增加默认关闭的 `monitor.public-launch` 阶段闸门，运营后台显示为「正式开放增长监控」；产品尚未完全开放时零注册只进看板，上线或开始投放后由 owner 显式开启才通知。真实故障信号（已付未发放、LLM 鉴权失败等）不降级、不隐藏。Grafana 支付卡同步改为显示对账距上次健康锚点的秒数。完整回归另暴露审计工作台原生 SQL 裸绑 `Date` 的既有时区缺陷：北京时间“今天”可能漏掉刚写入的日志，现统一改用 `utcTimestamp` 与 Prisma UTC naive 列对齐。验证：后端 TypeScript 构建通过，调度/指标/规则 41 条定向测试全绿，Prometheus 2.53 `promtool` 校验四组 58 条规则全部成功。
...
**最该记的一条是冲突怎么裁的。** skill 的「去 AI 味」和方案 §3.7.2 的「标签/按钮/状态/分类一律用行业通用词」会正面打架——skill 自己规定项目 style guide 优先，所以**去 AI 味不等于改得更土**。实测 agent 把状态词改成了「已经开始生成了 / 出好了 / 这次没算出来 / 封面没下载完」，全部退回 §3.7.2 的映射表（`做好了 → 已完成`、`没做成 → 生成失败`）。同理「登录已过期」退回 §0 第 7 条固定的「登录态已失效，请重新登录」——同一种鉴权失效出现两套口径，排障时会分不清。

**术语 sweep 只落一半比不改更糟。** 第一遍漏了 `clone/index.js` 与 `api.js`，还把「在『我的数字分身』里随时删除」写进了删除说明——而那个页面这一轮已经改叫「我的数字人」，等于把用户指向一个界面上不存在的页面。补扫后 wxml 里只剩文档正式名《数字分身素材使用说明》（受保护，不动）。

**「出镜」是两个意思，不能一刀切。** 用户本人出镜（「不用出镜」「上传你的出镜视频」）保留本义；指数字人时长的一律限定成「数字人出镜 N 秒」——光秃秃的「出镜 32 秒」会被店主读成「要我自己讲 32 秒」，恰好是首页承诺他不用做的事。

**Protected spans 全部保住**（Codex 逐项复核）：法律授权声明正文、文档正式名、朗读脚本、以及 AGENTS 已固定的全局文案。另修一处完成态前移——`onPhase('processing')` 时还没拿到 `accepted`，原改写说「军师正在安排训练」等于宣布已受理，改回「军师正在受理」。

改了 6 条测试断言，都是断言把旧文案当标记钉住（`/固定视频 · {{item.seconds}} 秒/`、`/关联声音会自动带入/`、e2e 的 `GATE_CASES` 等），已逐条注明它守的是什么、措辞可变；Codex 复核确认无一条是放宽守卫。验证：`npm test` 255 + 109 全绿，真机 e2e 46/46。

### 2026-08-27 · 快出片主链补页面行为测试；作品页把「没读到」说成了「已成片」 · 影响面：快出片分包成片页、前端测试基座、Swiper 安全补丁

**主链第一次被端到端跑通。** 此前快出片只有两层测试：纯函数（模板真值、报价、镜头切分）和开发者工具自动化（布局契约，且只走到模板详情、换机后因新 profile 无登录态跑不起来）。中间「页面 JS 的真实行为」这一层是空的——改文案、配画面、确认扣费、出片、作品这条用户唯一在意的路，没有任何测试走过。现补 `app/scripts/weapp-runtime.mjs`（最小微信运行时，让原生页面在 Node 里真跑）与 `video-chain.test.mjs`，逐页走通首页→模板专区→模板详情→改文案（真改一句并断言跨页存活）→配画面（走真实 `pickAsset` 入口，按文案选「从相册选」）→确认→出片→作品，覆盖取数、状态流转、按钮 handler、跳转参数与页面栈生命周期。跑源码而非 `dist-native`：构建器对 JS 是逐字节搬运，跑源码等于跑产物且改完立刻生效，也避免在并行测试里写共享产物抢文件。

**跑通链路当场抓到成片页三处状态谎报。** ① 作品详情读失败时 `work` 为 null、`loading` 关掉，而「生成完成 · 你的故事，已经成片」这块是无条件渲染的——一次网络失败、401 或作品不属于当前账号，都会被显示成出片成功，且没有任何重试入口（原来只有一个三秒消失的 toast）。现读失败是独立分支，说明原因并给「重试」。② 成片没有可播地址时只铺一块装饰底纹，用户对着「已经成片」和一个黑框自己猜；契约里 `videoUrl` 本就可空，后端下载口对同一状态给的是 `CLIP_WORK_NOT_READY「成片还没有准备好」`——**下载路径诚实、观看路径沉默**。现给出说明，并明确不在本页对扣费下结论（详情接口只透传作品、不做结算，结算在任务轮询那条路径上，本页无从判断这一单结没结）。③ 「生成完成 · 你的故事，已经成片」对任何取回来的作品都无条件渲染，而契约允许 `status: 'generating'`（深链或从作品列表点进来都会撞上，mock 里 `cw_mock_1` 就是），「字幕已生成」「已扣 N 积分」也跟着一起说出去——没出好的单子扣费未必已结算。现按 `done`（done / published）收口，生成中给自己的版式。三处都与首页「读失败不能说成你还没有」同一条原则：不能把取不到、没好显示成正常。另外读失败按原因分流。401 尤其不是一种而是三种——`services/request.js` 的 `unauthorized()` 会挂 `hadToken` / `authHandled` / `staleAuth` 正是为此：只有 `hadToken=false` 的游客 401 归本页弹登录门；`authHandled` 说明全局 `onAuthLost` 已经清态并准备 reLaunch，本页再弹一次就是第二次打扰；`staleAuth` 是旧请求晚到，用户可能**已经重新登录**，弹登录门等于把登录好的会话盖住（AGENTS §0.7 铁律）。403/404 不摆必然失败的「重试」，只有网络与 5xx 才给——与 `services/api-error.js` 的 `retryable` 分类一致。登录成功后自动重载本页。生成中同时收口保存与代发：`saveToAlbum()` / `publish()` 补完成态防御，「保存到相册」按钮与整个「下一步」区块（保存 + 代发）都按 `done` 隐藏——没出好的片子点下去只会拿到 `CLIP_WORK_NOT_READY`，一个点下去必然失败的主按钮本就不该摆在那儿。**真机走查又改出两处**：兜底文案原本不分上下文，对一条 `done` 的片子也说「成片还没有准备好」，与头部的「生成完成」当场打架——现按 `done` 分成「这会儿放不出来（片子已经出好了，只是取不到播放地址）」与「还没出好（还在生成）」；读失败的「重试」原本被父级拉成 286px 通栏橙条，一个错误恢复动作长得比主 CTA 还重，改为内容宽度居中（实测 84px）。这两处都是 40 条断言全绿之后、看截图才发现的——**断言能守住不塌，守不住难看和自相矛盾**。

**随后对 12 张真机截图做了四视角走查（文案 / 版面 / 状态一致 / 陌生用户），逐条对源码复核后又修出六处。**最讽刺的是走查排在第一位的那条「404 却说网络没通」是**假的**——它是 e2e 夹具的产物：拍那屏时只翻了 `canRetry` 没换 `loadError`，生产里跑不出这个组合。同类夹具失真还有两处：gate 三态从不登录（导致 `01-home-guest` 与 `02-gate-missing` md5 逐字节相同，「已登录但没分身」这一态从未被走查过），采集页未登录被登录浮层盖住而路由断言照样通过。三处都已修。

**真缺陷六条**：① 登录浮层对快出片用户说「军令」——`execute` 那句是军师案卷模型的词，而分包 26 处登录门全传 `execute`，等于这条链上每一次登录都说一遍；现加 `video` 档，`requireLogin` 默认值也改掉（漏传时会静默退回军令那句）。② 403/404 藏掉「重试」后什么都不剩，用「不给假动作」换来一条死路；补「看我的作品」。③ 成片取不到播放地址时 `retryLoad()` 早就存在，兜底块却只说「过一会儿再来」；补「刷新试试」。④ 游客态标题写「还没有数字分身」、按钮写「去创建」，而后端从没查过这个账号、按钮实际弹的是登录浮层；副文早分了 `guest`，标题和动作没分。⑤ 训练中的图标落在告警红（只有 ready 被单独覆写），正常等待态穿告警色会诱发重复创建。⑥ 兜底文案的地域口语（「这会儿」）改书面。e2e 同步扩到 46 条。

**Swiper 原型污染补丁已静默失效。** `patch-swiper-security.mjs` 写死两条嵌套路径 `node_modules/@tarojs/components{,-react}/node_modules/swiper/...`；npm 把 `swiper` 提升到顶层后这两条路径不存在，脚本一个文件都没打到——而它只在「一条都没找到」时才抛，那恰好也是它被跳过的方式。配套测试红成 ENOENT，看起来像陈旧路径而非护栏失效。实测顶层 `swiper@6.8.0` 中带修复模式的文件数为 **0**，而 Taro 解析到的正是它，即 GHSA-hmx5-qpq5-p643 的防护完全没有生效。现新增 `scripts/swiper-targets.mjs` 按 Taro 自己的解析规则定位，已回移修复 11 个文件；测试改为断言「解析到的那份里已无脆弱写法」而不是「某条路径存在」，npm 提升与否都不再影响。

### 2026-08-23 · 命盘统一法定钟表时间并保留 23:00 子初换日 · 影响面：排盘引擎、全端生辰录入与报告、存量命盘升级、提示词及边界测试

- 根据用户确认的统一流派，排盘引擎升 `paipan-v6`：直接使用出生证明上的法定钟表时间，保留准确分钟，不再按出生地/经度换算真太阳时；出生地区仅作档案信息，旧 `longitude` 与 `matchedCity` 字段只保留兼容、不得影响四柱。
- 换日规则固定为 `23:00–23:59` 自动按第二天子时排盘，`00:00` 已是新公历日的早子时；边界回归逐分钟锁定 `22:59` 当日亥时、`23:00/23:59` 次日子时、次日 `00:00` 同日早子。
- 开源源码交叉审查后补强紫微边界：每次调用 `iztro` 前显式固定 `dayDivide=forward`，并用“先污染为 current、再立盘”的回归测试锁定晚子盘与次日早子盘十二宫完全一致，避免依赖默认值或进程级全局配置漂移。
- 存量 v3/v4/v5 首次读取按原始出生钟表时间惰性重算为 v6，清除经度计算标记；v1/v2 历史快照继续保留。报告档头固定展示「法定钟表时间 / 子初换日 · 23:00」，不再暗示补出生地可提高时柱准确度。

### 2026-08-23 · 出生时间精确到分钟并修复真太阳时子初边界误排 · 影响面：全栈排盘契约、建档/命盘/天时/速写卡录入、存量命盘升级、报告档头与回归测试

- 用户反馈复盘确认根因不是 `23:00` 子初规则失效，而是四个前端入口只保存时辰小时、丢失真实分钟：瑞安 `1987-03-16 23:30` 被提交成 `23:00`，经经度与均时差校正后落到 `22:53`，因而被排成亥时和当日盘。
- 排盘引擎升 `paipan-v5`，全端改用系统时间选择器提交准确 `hour + minute`；该案例现按真太阳时约 `23:23` 排为 `1987-03-17`、农历二月十八、`乙丑`日、`丙子`时。准确 `23:00` 则仍按约 `22:53` 排为三月十六、`甲子`日、`乙亥`时，边界语义由回归测试锁定。
- 存量 v3/v4 若档案没有分钟，不再武断补 `:00`，而按原时辰档位中点重算并升级 v5；报告同时展示出生钟表时间、真太阳排盘时间、最终时辰，并标记「旧记录分钟待确认」，让用户可直接修改后重新立盘。v1/v2 历史快照继续保留。

### 2026-08-22 · 快出片暂停原型迭代，方案回归文档化交互规格 · 影响面：产品 SSOT、端到端交互、异常状态、资产关系、原型使用边界

- 根据用户评审，V1/V2 SVG 均降为未采纳探索稿：V1 过于像六步向导，V2 又用“三步口号”过度压缩复杂业务；两者不得作为产品评审、设计验收或开发依据。
- 方案升为 v4，新增七阶段端到端交互规格，逐阶段明确用户看到什么、最少动作、系统自动处理、什么情况下才打断，以及产生/更新哪些核心对象。
- 新增四层评审框架：用户任务主链、系统自动化链、异常与付费分支、数字资产链；以后不能再只用页面数量或按钮数量判断“是否简单”。
- 补充叙事口播与视频带货的确认内容差异，以及首次/已有资产/外部任务/恢复草稿、部分就绪、素材缺失、余额不足、模糊受理、单片段失败和回传失败等移动端状态基线。
- 视觉原型恢复的前置条件调整为：规格稳定后选择一个具体场景、具体资产状态和具体任务做页面级验证。本次仅更新文档边界，未修改现有代码、接口或数据契约，也未删除历史 SVG。

### 2026-08-22 · 快出片原型 V2 收敛为用户感知三步 · 影响面：移动端创作流、自动准备、素材适配、数字人选择、预览与计费表达

- 新增 `docs/prototypes/VIDEO_STANDALONE_MOBILE_FLOW_V2.svg`，将 V1 六屏向导改为“说需求 → 看预览 → 发出去”三步；取消独立等待页，自动准备只作为创作工作台里的后台进度。
- 首屏把外部任务、用户一句话、AI 自动提取结果和唯一待确认问题放进同一连续对话；商品、品牌、门店和来源 APP 已知信息自动引用，不再要求重复填写。
- 预览页成为唯一创作工作台：一个播放器承接效果示意、准确秒数、时间线当前段定位、文案/数字人/样式三个调整入口和最终报价，所有修改就地完成。
- 原型下半部明确七项系统后台动作，并补齐四类操作细节：长短素材自动处理、快速 AI 形象/固定场景分身两类选择、准确预览与效果示意边界、免费装配与生成费用一次汇总及片段复用。
- 方案文档升为 v3，并记录 V1 被替代的原因与 V2 真机验证目标。本次仅新增 SVG 和规划记录，未修改现有小程序实现或数据契约。

### 2026-08-22 · 快出片形成首版六屏移动端 SVG 原型 · 影响面：产品方案、移动端主流程、预览交互、资产归档表达

- 新增 `docs/prototypes/VIDEO_STANDALONE_MOBILE_FLOW_V1.svg`，以“创作首页 → AI 理解确认 → 自动准备 → 整片预览 → 当前段调整 → 作品完成”六个状态表现首期核心流程。
- 视觉沿用快出片暖橙人文纪实体系：橙色表示生成式片段、蓝色表示已有素材、绿色表示可准确预览；所有底层预览能力仍由一个播放器承接，不让用户选择技术模式。
- 原型突出“40 秒准确预览 · 5 秒效果示意”、自动准备阶段未调用付费生成、按成本排序的当前段解法，以及完成后的商品/剧本/数字人/素材自动归档。
- 方案文档新增原型入口、六状态说明与下一轮真机验证问题。本次仅新增 SVG 和规划记录，未修改现有小程序实现或数据契约。

### 2026-08-22 · 快出片预览改为“生成与装配解耦” · 影响面：产品方案、手机预览、代理合成、计费时点、最终复用

- 修正上一版“看整片前先生产全部数字人/AI片段”的高成本路径：默认先由手机按真实时间线连播已有素材并叠加字幕/声音，再由自有后端 FFmpeg 自动升级 360p/540p 代理；这两层不调用付费生成供应商。
- 尚未生产的数字人口播用固化形象样例等时长示意，AI b-roll 用参考图/分镜卡示意，并在播放器与扣费区按秒标记口型、动作或画面仍待生成；例如“40 秒准确预览 · 5 秒效果示意”。
- 高保真生成试片降为按段可选动作，用户也可基于已披露的装配预览直接确认最终生成；提前生成的 `PreparedClip` 强制复用于最终成片，未生成槽位只在首次最终任务中生产，避免重复生成和重复扣费。
- `PreviewRender.fidelity` 调整为 `device_timeline | assembly_proxy | exact_proxy`，规划补充占位槽、确定性 hash 与示意确认时间；手机端不承诺本机编码完整 MP4，后台代理承担稳定音画合成。本次仅更新方案文档，未修改现有实现或契约。

### 2026-08-22 · 快出片确立“试看片即质量控制界面” · 影响面：产品方案、预览语义、分段生成、计费复用、最终导出

- 吸收现有子应用试用反馈：静音素材连播、声音试听和数字人口型 demo 只能验零件，不能让用户预知字幕、裁剪、音乐、转场、片头片尾与最终时长；默认总览升级为可播放的低清成片试看片。
- 试看片必须消费实际时间轴与已准备片段，除分辨率/码率外与高清导出所见一致；仍含未生成占位时只能叫结构预览，禁止让用户直接签字导出。
- 新增方案对象 `PreparedClip / PreviewRender`：数字人和 AI 片段先生成一次并固化复用，装配型修改只重做低清代理，生成型修改只重做受影响片段；最终 `ExportJob` 强校验已批准的 `previewRenderId + timelineHash`，项目改动后必须刷新预览。
- 试看片代理总装默认不另收费或仅限合理次数；昂贵片段按真实供应商成本先报价并直接计入最终可复用素材，不能同时虚假承诺“完全免费、完全真实、片段又尚未生成”。首片补贴和免费生成型重做次数留待成本实测。本次仅更新规划文档，未修改现有三层预览实现或接口。

### 2026-08-22 · 快出片手机操作再次收敛：槽位退居底层、时长弹性适配 · 影响面：产品方案、默认制作页、异常处理、模板时长

- 将上一轮完整的槽位/补位决策从默认手机界面撤下：用户默认只检查“讲什么、谁来讲、用什么画面”三项并点击一次生成；只有事实不确定、关键资产缺失或新增付费生成才阻断。
- 引入“默认结果层 → 问题处理层 → 细节调整层”三级披露；系统无法处理时只显示一张问题卡、一个不增费推荐动作和一个“其他方式”，裁剪、原声、拆并段等收进主动调整。
- 模板秒数由硬槽位改为 `min/ideal/max` 弹性目标：例如约 45 秒可随素材和真实 TTS 自适应到合理区间，先重排叙事节拍、数字人口播和相邻素材，只有严格投放规格或无法安全适配时才让用户处理秒数。
- 普通用户不设置 AI 预算上限；多个付费缺口在生成前只做一次增量费用确认，预算上限仅保留给批量、企业和合作方配置。本次仅更新规划文档，未修改现有实现或契约。

### 2026-08-21 · 快出片明确低成本模板混剪与素材时长适配 · 影响面：产品方案、模板槽位、素材动线、成本交互

- 全片生产从“数字人 + AI b-roll”进一步收敛为低成本混剪：优先用户上传、资料库、官方素材、图片动效和已有数字人，昂贵的 AI 视频仅用于高价值镜头或明确素材缺口，且按槽位展示增量价格后再生成。
- 方案级镜头模型由单一 `shot → assetId` 升级为 `TimelineSlot → SlotItem[]`：10 秒槽位可由多段上传/数字人/生成/图文素材共同填满，并区分旁白绑定、保留原声和纯音乐三类时长真值。
- 明确长素材智能选取最佳时间窗、短素材按缺口分级适配；约 1 秒差值可由轻微调速/转场吸收，明显缺口优先推荐已有素材或低成本补位，人物动作和推镜默认禁止硬循环。
- 制作中可原地“AI 补 N 秒”，异步完成后回填当前槽位并作为项目衍生素材，可收藏进资料库；本次仅更新规划文档，未修改现有单素材镜头实现或接口契约。

### 2026-08-21 · 快出片独立产品方案升级为持续讨论 SSOT · 影响面：产品定位、移动交互、数字资产、多 APP 连接、模型策略

- 将 `docs/VIDEO_STANDALONE_MINIAPP_PLAN_2026-08-19.md` 从“军师快拍拆分 + 通用生成工具”重构为持续演进方案：快出片独立运营，军师只是首个 Connector，同时支持其他 APP 以中立 `CreationRequest` 接入。
- 首期范围收敛为“数字人口播/叙事”和“数字人带货”两条模板主链；移动端从五 Tab 收敛为“创作｜资料库｜作品”，输入改为语音/一句话、商品自动提取、外部任务带入和逐步确认。
- 补齐商品、剧本、数字人/声音、媒体、模板、项目/任务/作品、官方素材与授权预留的关系模型，以及合作方身份映射、费用/作品/素材归属和结果回传。
- 结合 Sora、Veo、Runway、Seedance 等公开 API 的单次时长约束，明确长片采用音频主时间线、稳定数字人口播主干、短 AI b-roll、多镜独立生成与统一总装；本次仅更新产品与工程方案，未修改接口或代码。
