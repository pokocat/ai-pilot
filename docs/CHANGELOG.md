...
- 依据 `mobile-app-ui-design` 的任务优先、单主动作、44px 点击区和 8 点间距重新组织页面：三步进度只显示真实任务，拍摄指南压为两列，素材使用权改为轻量声明且保留删除能力。自动化只走 HTTP 桩和本地媒体策略，没有创建真实 speaker/avatar/video 任务、没有消耗石榴点数；部署与真机 preview 版本待本轮发布后回填。
