...
- 自动化继续只用 HTTP 桩和本地媒体验证，禁止创建真实 speaker/avatar/video 任务，避免消耗石榴点数。已部署军师预发 `6ca6a64` 与 AIStar `e9e8e43c-20260811T153721Z`；两服务 active、`NRestarts=0`，requirements 仍为 `authorizationVideoRequired=false / avatarMin=5 / voiceMin=3`，微信 avatar 模板在线可见。预发 db push 再次短暂重建 `knowledge_chunk.embedding_vec`，随后 `pgvector.sql` 已从 JSON 回填，2/2 条向量恢复；生产未修改。
