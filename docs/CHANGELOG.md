...
真因不在凭据也不在页面：打的 URL 是 `wxa/getwxacode/unlimit`，**这条路径不存在**，
微信对任何入参（不传 page、scene 只给一个字符）都回 `{"errcode":40066,"errmsg":"invalid url"}`。
官方路径没有斜杠：`wxa/getwxacodeunlimit`。同 token 对照打 `wxa/getwxacode` 与
`cgi-bin/wxaapp/createwxaqrcode` 均正常返回 PNG，据此排掉了 appid / access_token / 未发布线上版本。

**为什么没人发现**：两处调用都遵循「失败一律降级成无码」的铁律——不抛错、不打日志，
页面只是安静地少一张码。所以 `miniCodeDataUri`（B 级卡片页脚回流码，早就在线上）大概率
**从未成功过一次**。这正是 SOP §5.3 那条「静默失败必须有 null 分支日志」的同类坑，
只是这次靠「上线后打真实链路」逮到的。

**改法**：两处收成 `wechat.ts` 的 `WXACODE_UNLIMIT_URL` 常量（这条路径要么全对要么全错，
不该有第二个写法），测试钉住确切字符串并显式排掉 `/unlimit` 写法（server 1931 → 1932）。

**线上验收**（生产 `dff9c45`、真凭据）：default/card/store 三个物料位各取到真实 PNG
（130 KB / 126 KB / 147 KB），同 `(code, slot)` 二次调用 0 ms 命中缓存；顺带验了卡片页脚码
也回 PNG（70 KB）。

### 2026-08-20 · 邀请二维码：静态传播链（转发到不了的地方靠它） · 影响面：小程序新页、主公页入口、server 新端点、契约

转发依赖微信会话（人传人、有社交成本）；二维码是**离线锚点**——名片、门店台卡、提案封底、
展会物料、课程讲义，转发到不了的地方它能到。归因链路本来就通（`invite.js` 早就认 `ic:` 前缀的
scene），缺的只是「生成并给用户这张码」。

**服务端**：新增 `services/inviteQrcode.ts` + `GET /invite/qrcode`。
不复用 `wechat.ts` 的 `miniCodeDataUri`，因为那个是给 B 级卡片页脚用的（scene 写来源标识、
刻意不传 page 落主页）。邀请链两点不同：① 必须带 `page=pages/sessions/index`——扫码的是**陌生游客**，
落主页再自己找一遍会掉转化，而问策 tab 是公开内容不撞登录门；② 必须缓存——`getwxacode/unlimit`
有频率限制而邀请页会被反复打开，同一 `(code, slot)` 的码恒定所以缓存天然安全（复用
`services/cache.ts`，TTL 12h，**失败不缓存**免得把一次限流抖动放大成 12 小时无码）。
另外 `line_color` 设墨色 `#16191D`（默认黑偏冷）、`width` 430（280 在名片尺寸下扫不动）。

**物料位**：`slot` ∈ default/card/store/deck/event，scene 形状 `ic:<码>[:<位>]`，
受微信 **32 字符硬限制**（`ic:JS2K7P:card` 仅 13 字符）。白名单外的脏值一律落回 default——
scene 是要印出去的，拼进未知字符串等于印一张归因不了的码。这也带来转发通道给不了的信息：
「这个客户从哪块物料来的」。

**端上**：新增 `packages/work/invite`（第 55 个路由），主公页邀请码那行变成可点入口。
主动作是**存图**而非转发（存下来要能进 PPT、发给设计做物料）；复用 gift 页现成的
`canvas.render/save/share`。出图刻意做了两件事：二维码四周留 34px **静区**（贴满边扫不动）、
邀请码用大字**重复一遍**（物料被拍照或印花时人还能手输）。

**降级**：服务端 `dataUri` 可能为 null（凭据未配/限流/测试环境），此时页面与出图都改成
「邀请码大字 + 说明可手输」，不给裂图。读数（已邀请/已开通）取不到显示「—」而不是 0。

**测试**：server +5（1926 → 1931），实测变异均红——去掉 `ic:` 前缀报「必须带 ic: 前缀」、
脏 slot 照单全收报「未知位必须落回 default」。覆盖 scene 形状与 32 字符上限（含码变长余量）、
slot 白名单、失败降级回 200 而非 5xx、未登录 401、邀请码惰性生成落库。

**顺带修**：`weapp-share` 里还有一处写死单张底图的断言（上一轮漏改，图池 3 套后随机会取到 B/C）；
两处路由数守卫 54 → 55（守卫本意就是「路由变了来审计分享覆盖」，按预期触发）；
新页与主公页的图标改用项目 Lucide 映射里的 `back` / `arrow`（`chevron-*` 不在表里，
仓库要求功能图标统一走 native-icon）。

### 2026-08-20 · 分享底图换成 AI 生成的三套暗调大片 + 文案改钩子式 · 影响面：分享底图、文案库

**底图**：codex imagegen（内置 image_gen）生成三套「有压迫感的东方质感」，各两幅（5:4 转发 / 1:1 朋友圈）：
- **A 墨落**：一滴浓墨落入砚池、在水中炸开成丝状卷须，逆光让墨丝透出金色光晕
- **B 金石**：深绿墨玉砚台局部特写，边缘一道锐利金高光，砚心一汪未干的墨反射一点光
- **C 破雾**：强光束斜切浓雾（丁达尔），照亮下方一小片深绿山脊轮廓

方向上刻意排除两类「炫」：赛博朋克霓虹 / 电路板 / 蓝紫渐变，以及廉价商务风的握手 / 上升箭头 / 金币。
配色守品牌：深绿 #143726 与墨黑 #16191D 占大面积，金色只作金属边缘光与光束。
**图上无任何文字**（标题由 onShareAppMessage 另配，AI 出中文字几乎必然错字），
主体极大（聊天流里只有约 210pt 宽，细密纹理会糊成一团）。

处理：按目标比例**居中裁切再缩放**（不拉伸变形），逐步降质压到 100KB 以内。
实际六张 21~50KB、合计 212KB，主包压缩后 0.96M（上限 2M）。
1.5M×6 的原始大图**不入库**（放在 weapp-native 下会被整目录拷进包）。

**文案改钩子式**（12 条）。原来那批只陈述痛点、不给「点开会得到什么」的悬念，
且句式雷同（清一色「先…」「值得…」），12 条扫过去像同一句。改用四种打法：
① 反常识（「客户越来越贵，多半不是因为你投得少」）② 具体数字（「10 分钟，拆成三步」）
③ 未完成（「同行悄悄在改的那件事」）④ 精准处境（「每天在救火，没人陪你看全局」）。
仍不承诺收益数字（合规）、不用盘面黑话。

### 2026-08-19 · 分享底图换成真实素材（Unsplash，可商用） · 影响面：分享底图池、分享守卫测试

原底图是构建期用代码画的纯色块（750×600 / 600×600，各 2~3KB），用户评价「很丑」。
codex 的 imagegen 撞额度、本机无 `OPENAI_API_KEY`，故改为从 Unsplash 取真实素材。

**三套六张**（转发 5:4 + 朋友圈 1:1 各一张，图上均无文字——文案由 `title` 承载）：

| 套 | 内容 | 出处 | License |
|---|---|---|---|
| A 墨山 | 浓墨山石、留白足 | Unsplash `PIiPQ8eEOOo`（The Walters Art Museum，Yoshizawa Setsuan 1876） | Unsplash License，可商用、无需署名 |
| B 竹林 | 竹影与远山，雅致清透 | Unsplash `photo-1684871430852` | 同上 |
| C 棋盘 | 落子前的停顿（唯一实物照，与两张水墨互补） | Unsplash `LcfVtKRnKT8`（Kyle Hinkson） | 同上 |

**统一后处理**：轻降饱和（0.82）+ 往米底 `#FBFAF6` 偏 6% + 微提对比，让三套看着像一个系列
而不是三张不相干的库图；按 5:4 与 1:1 分别裁切（不是同一张裁两次）；逐张压到 100KB 内
（50~84KB），因为本地图占主包额度。

**图池 1 套 → 3 套**，与 12 条文案独立随机 = 36 种组合。

**包体积**：分享图合计 423KB，主包 0.75M → **1.16M**（上限 2M，余量充足）。

**测试**：修了三处「写死单张图」的断言（图池变 3 套后随机会取到 B/C），改为验「取自图池」+
「埋点 art 序号对应本次实际用图」。另修正一条**测试自己的错误期望**：原用例只覆盖了
`onShareAppMessage` 就断言朋友圈走兜底，而 `onShareTimeline` 仍来自 mixin（自带随机图、
走不到兜底）——现在分别验 mixin 随机图与兜底图两条路径。weapp-share 34/34。

### 2026-08-19 · 修小程序主包顶穿 2MB 上限（H5 字体被误打进包） · 影响面：小程序构建脚本、包体积守卫

准备 `0.2.48` 审核包时发现**主包压缩后 2.54MB，超过微信 2MB 硬上限**，这个包传上去会被平台拒。

**根因**：`scripts/build-native-weapp.mjs` 用 `fs.cpSync(ASSET_ROOT, …, { recursive: true })`
把 `app/src/assets/` 整目录拷进包，连**给 H5 用的** 1.79MB 中文衬线字体
（`junshi-serif-{400,600}.woff2`）一起带上了。而小程序侧**从来不读包内字体**——
`services/font.js` 走 `wx.loadFontFace` 拉远程 `https://wxapi.aibuzz.cn/fonts`（微信自己缓存），
字体放仓库的本意只是让 H5 同源引用、躲开 CORS（见该目录 README）。
所以这 1.79MB 是纯白占，且占掉了主包 2.54MB 中的 70%。

**实测归因**（微信按压缩后算，用 zip 近似）：
| 包 | 修复前 | 修复后 |
|---|---|---|
| 主包 | **2.54M ✗** | **0.75M ✓** |
| packages/main | 0.16M | 0.16M |
| packages/work | 0.19M | 0.19M |
| packages/video | 0.19M | 0.19M |

顺带纠正一个误判：`du` 显示 `assets/native-icons` 占 2.5M，但那是文件系统块开销的假象
（628 个小 SVG），压缩后只有 0.22M，不是问题。**判包体积必须看压缩后，不能看 du。**

**改动**：构建时排除 `assets/fonts/`，留 `WEAPP_BUNDLE_FONTS=1` 逃生开关
（真机万一验出老基础库 `loadFontFace` 加载不上时可打回包里，但那时必须同步挪进分包或做更小子集）。

**新增守卫**（`scripts/weapp-build-meta.test.mjs`）：按主包/分包分别算压缩后体积，
阈值 = 上限的 85%（留余量）；另加「`assets/fonts` 不得进包」的定点断言。
这类问题只有上传到微信后台才会报错，本地 tsc 与单测都看不出来，所以值得钉死。
实测变异：`WEAPP_BUNDLE_FONTS=1` 重建 → 当场报「主包 2.51MB 超出预算」。

**验证**：远程字体两个 URL 均 200 且字节数与本地一致（930468 / 943552），加载路径通；
产物 `FONT_BASE` / `FONT_FAMILY` / `FONT_WEIGHTS` 配置正确；
weapp-build-meta 10/10、native-weapp 92/92、weapp-share 34/34。
⚠️ **字体是否在真机上仍正常渲染需要真机复验**——本次只验证了 URL 可达与配置正确。

### 2026-08-19 · 分享文案改文案库 + 每次随机（埋点带序号保可排查） · 影响面：小程序分享 mixin、分享埋点、分享守卫测试

- **文案库**：内置素材从 4 条扩到 **12 条**，并把「文案」与「底图」拆成两个独立池
  （`BUILTIN_COPY` / `BUILTIN_ART`）。运营可能只想换文案不换图，绑成一套就必须整套替换。
  两池独立随机，组合数是相乘的（12 × N）。文案分通用 2 / 获客 3 / 现金流 3 / 用人 2 / 方向 2，
  每条带 `title`（递给你看的语气）与 `timelineTitle`（广而告之的语气）。
- **改随机**：原先按本地自然日轮动（同一天全站同一套）。产品要求改随机——同一个人一天分享
  几次，卡片一模一样会显得很假。`dayIndex()` 与旧 `posterIndex()` 一并删除。
- **可排查性没有丢**：`share_expose` 埋点改报本次选中的 `copy` / `art` 两个序号，
  线上照样能还原「他当时看到的是哪条配哪张」。**随机与可追溯并不冲突，只要选择结果被记下来。**
- **埋点必须报「本次真正用掉」的序号**（这次改动最容易写错的一处）：旧写法是在埋点函数里
  再算一次序号——按日轮动时期那样还算对（同一天恒等），改随机后就变成「埋点自己又摇了一次」，
  报上去的不是用户看到的那张，等于埋点说谎。现在序号由回调结果经内部字段 `__pick` 透上来，
  且**返回微信前必须 delete**（分享回调返回值有固定契约，不该夹带内部实现）。
- **wrapper 时序改为「先执行回调、再上报」**：序号只有回调跑完才知道。原来「先报」图的是
  「回调抛错也不丢曝光」，这条保证没丢——`catch` 里照样报一次（只是没有序号），且不吞页面异常。
- 测试 33 → 34，三条断言方向随口径反转后重写，并**实测变异均红**：
  ① 埋点自己再摇序号 → 报「埋点序号必须对应本次实际用的文案」；
  ② 退回确定性选择 → 报「文案必须是随机的，80 次只出现了 1 种」；
  ③ 不删 `__pick` → 报「必须在返回微信前删掉」。
  另新增文案库口径守卫（条数、双语气、不出现盘面黑话与品牌红线）。
- **底图仍是待办**：`card-friend.png` / `card-timeline.png` 目前还是构建期用代码画的纯色块，
  用户已明确「很丑」要换。codex imagegen 撞额度、本机无 `OPENAI_API_KEY`，
  现定方案是找开源图（只用 Unsplash / Pexels / CC0，商业用途不能用 CC BY-NC），
  换图时把出处与 License 记在这里。交接见 `docs/[HANDOFF]SHARE_ART_AND_COPY_POOL_2026-08-19.md`。
...
### 2026-08-18 · 微信桌面端自动分栏与大窗口响应式适配 · 影响面：原生小程序五个主 Tab、桌面微信、iPad

- 按微信官方能力在原生 `app.json` 开启 `resizable:true` 与 `frameset:true`：Windows 微信 3.3+ 可把首栏与后续页面自动左右展开，PC 窗口支持拉伸/最大化，iPad 支持旋转；不支持分栏的客户端继续单栏运行。
- 五个主 Tab 统一接入官方推荐的 `Page.onResize`，实时重算当前栏宽高、`compact / medium / expanded` 布局档位与胶囊安全区；没有使用官方不推荐的全局 `wx.onWindowResize`。分栏中的每一栏按自己的真实宽度判断，不会把半屏误当整块桌面。
- 大窗口不再把手机版等比例拉宽：正文限制舒适行长，问策页头/消息/输入浮岛同轴，军师列表在超宽栏展开双列，战局核心判断和三势放大，今日提高任务与作品密度，图籍目录展开三列，底栏与半屏 sheet 居中收窄。手机端断点以下的结构、底栏安全区、键盘与 overlay 行为保持不变。
- 新增构建回归，锁定官方配置、五个主 Tab 的 `onResize` 接入、窗口变量和宽屏样式，防止后续只保留视觉 CSS 却丢掉运行时尺寸同步。

### 2026-08-18 · 微信开发版切回生产接口 · 影响面：微信后台开发版、内部真机验收环境
...
- 原生与 H5 请求层现在只允许“当前 storage 仍等于该请求的 `tokenAtRequest`”的第一条 401 触发全局退出；其余旧响应标记为 `staleAuth`。页面错误收口识别 `authHandled/staleAuth`，不再重复清状态或跳路由。
- 同时堵住更危险的竞态：旧请求若在用户重新登录后才返回 401，不得清掉刚写入的新 token。原生与 H5 回归测试都覆盖同 token 双并发 401 与旧响应晚到两种时序。
- 补齐 H5 微信订阅 `clip` 场景的成功文案「已订阅一次成片完成提醒」，保持 `WechatSubscribeScene` 穷举映射与共享契约一致，解除完整类型检查阻塞。
- 微信开发版 `0.2.46` 已上传 AppID `wx810ebe6dfef8e75f`，构建身份为 `native-weapp / server / https://wxapi.aibuzz.cn/api_preprod`，用于预发真机回归；尚未转体验版、提交审核或发布。

### 2026-08-18 · 快出片「先看看」三件套：预览解耦为画面/声音/出镜三条独立链路 · 影响面：AIStar clip、DAP 模型、契约、原生小程序

产品口径（用户拍板）：不做按需试镜，改成**训练完成时预生成 + 固化**，三件事互相解耦，任一失败不影响其余两件。

| 预览什么 | 形态 | 何时生成 |
|---|---|---|
| 画面顺序、节奏 | 纯画面拼接，静音无字幕 | 端上实时，零后端 |
| 声音像不像本人 | 固定样例文案的 TTS | 声音训练完成时固化一次 |
| 口型、构图 | 带声音的短片 | 形象训练完成时固化一次 |

- **为什么固化而不是按需**：点开就响（不用等三五秒合成）、成本可预测（每条声音/每个形象各一次，我们承担，不必靠限流防薅）、口径一致（所有人听到的是同一句，好横向比较）。按需合成仍保留给「我想听自己那句难念的」，两者并存：端上样例句直接播固化产物，改过一个字才落回合成。
- **为什么是后台扫表补齐而不是在 ready 那一刻生成**：`engineStatus` 变 `ready` 的地方有五处（形象刷新、声音刷新、克隆回调、重训回调…），逐个挂钩必漏一个，而漏掉的那条路径上的用户就永远看不到样例。`ClipDemoWorker` 只有一个入口，天然幂等，失败下一轮还能再来，也不拖慢任何请求路径。每轮限 3 条，绝不和用户正在等的出片抢供应商配额。
- **失败不阻断**：样例是锦上添花，形象 ready 就是 ready。`demoAttempts` 到上限后放弃，避免对着一条坏素材无限重试白烧点数；「还没有可用声音」不消耗次数，用户之后补录声音会自然补上。
- **数字人样例短片是砍掉试镜后唯一能验口型的地方**：形象预览帧只能证明「是我」，证明不了「说起话来自不自然」。分身管理页新增「看出镜效果」，用真实 9:16 容器。
- **全片预览补封面**：连播里给封面 2 秒并明说「成片里只占第一帧，用作平台缩略图，不占正片时长」；只在真开启时才放，别让「没设置」看起来像「设置了但没生效」。尾卡本来就在镜头列表里，已覆盖。
- **顺手修**：上一批把 `ClipRenderWorkerState` 构造函数加了参数却只跑了 `mvnw compile`，测试编译是断的；已补齐并纳入 `mvnw test`。
- **codex 第三轮审阅修掉的 7 条**（逐条回代码核实成立）：
  - **迁移 V21 整个删掉**。`dap_voice`/`dap_avatar` 根本不在 Flyway 基线里，一直由 `ddl-auto=update` 管；Flyway 跑在 Hibernate 建表之前，ALTER 必然撞「表不存在」（新库 51 个 context error 即此）。而且生产是 MySQL，`ADD COLUMN IF NOT EXISTS` 和 partial index 都是 PostgreSQL 语法。新字段随实体定义由 `ddl-auto` 补齐，与这两张表的既有列同一路径。
  - **样例短片改两阶段**。`HttpShiliuGateway.createVideoByText` **恒返回 `processing` + `outputRef=null`**，任务号在 `task.id()`。原实现既不存任务号也不轮询，等于每轮重新提交、烧三份点数还永远拿不到成片。现在提交时存 `demoVideoTaskRef`，下一轮 `query()` 取结果；attempts 只在提交时消耗，轮询不消耗。
  - **认领挪出业务事务**。CAS 与供应商调用同在一个 `@Transactional` 里时，供应商异常会把 attempts+1 一起回滚，退避形同虚设。拆出 `ClipDemoClaims`（`REQUIRES_NEW`）——不能写成同类私有方法，Spring 事务是代理织入的，类内自调用不走代理。
  - **关联声音查错了字段**。分身关联的声音 id 存在 `avatar.voiceName`，不是 `voice.avatarId`；后者只在「从形象视频提原声」那条路径上有值。原实现让**所有复用已有声音的分身**都被判成「没有可用声音」，永远拿不到样例。口径已对齐 `ClipAvatarService.linkedVoice`。
  - **素材变了要作废旧样例**。重训声音、更换形象、改关联声音之后，旧 key 仍非空 → worker 认为「已经有了」不再生成 → 用户一直听到旧音色、看到旧形象。比没有样例更糟，因为它看起来是对的。三个入口都挂上 `invalidateXxxDemo`。
  - **删除分身要清样例文件**。样例短片也是用户本人的肖像和声音，不能删了分身还留在 OSS。
  - **`clone` 页的 `voiceDemoUrl` 是个从没定义过的死绑定**，快路径在那一屏形同虚设。已接上数据来源；同时说明白：轮询到 ready 就停，固化多半还没好，那一屏走按需合成是**正确行为**。
  - **签名 URL 一小时过期**：页面开着超过一小时再点，播的是过期地址。改成静默回落按需合成（只回落一次），而不是甩一句「播放失败」。
- **验证**：AIStar 编译 + 全量测试通过（含 `ClipDemoServiceTest` 12 项）、`check:api-contract` 通过；小程序构建 + 187 项测试通过。
...
- **上传治本**：本人形象/声音/照片先向军师领取精确限制的 OSS V4 短票据，再由手机一次直传；AIStar 用持久化 uploadId HEAD 核验后异步深检/受理，军师按同一 clientRequestId 幂等预扣与结算。弱网超时、OSS 成功响应丢失及 180 秒仍在受理都保留原请求号，禁止重复上传或重复创建供应商任务；域名配置未生效时仅在文件确定未离开手机的错误上兼容回退旧链路。
- **iPhone 编码兼容**：AIStar 对 HEVC/H.265 形象视频先转 H.264/AAC，再进入既有 ffprobe、预览帧和石榴训练，避免用户上传完成后才因编码被打回。
- **保存相册**：新增鉴权同源 `/api/video/works/:id/file` 流式下载，刷新上游短签名并避开 OSS downloadFile 合法域名问题；端上检查 HTTP 状态、展示下载进度，并分别处理相册权限拒绝、视频格式与存储空间错误。

### 2026-08-18 · 第三 Tab 定名「今日」并收拢经营结果填写 · 影响面：原生小程序底栏/页头/分段/复盘抽屉/每日战报、服务端复盘口径

- 第三 Tab 用户可见名称由「执行」统一为「今日」，页头同步改为「今日 / 今」；为避免 Tab 与首段重名，页内时间段按既定候选规则改为「今天 / 近七日」。工程路由继续使用 `pages/execution` 与 `gotoExecution(today/week)`，不改接口和历史落地兼容。
- 今日主流移除常驻的「线索/咨询/成交」三栏表单，改为一行「今日经营结果」状态入口；表单收进复盘抽屉顶部，并独立于复盘详情接口的加载/失败，用户有结果再记、不强迫每日填 0。本周行业化填报正名为「本周关键指标」；每日战报与复盘上下文同步由「数据回填」改称「经营结果」。底层 `CasefileMetric` 和既有每日战报/复盘/处方归因链路保持兼容。
- 修复真机点击经营输入框时 fixed 复盘抽屉被微信默认键盘避让二次上推、输入区飘到页面顶部：每日三项与本周动态指标统一关闭 `adjust-position`，由 `keyboardheightchange` 驱动抽屉等量上移和缩短，内部滚动区只滚到当前输入；关闭抽屉主动收键盘并清高度。

### 2026-08-18 · 原生小程序恢复独立执行 Tab，锦囊降为伴随入口 · 影响面：weapp-native IA/战局/军令/锦囊/导航/徽标/埋点、订阅落地

- 五 tab 改为「问策 / 战局 / 执行 / 图籍 / 主公」：新增 `pages/execution`，迁入今日军令、兵器、打卡、经营回填、近七日执行与复盘；战局回归纯判断，并按「今日有令 / 有判断无令 / 无判断 / 读取失败」桥接行动。
- 执行页按产品完整度拆成 core、今日战果、复盘抽屉三个独立状态机；今日战果 v1 只读当日海报，失败局部重试、缺图回插画，严禁触发慢 `videoWorks()`。锦囊三路作品缓存提升为模块级 90 秒 TTL + single-flight，与执行共享 posters 槽；缓存按当前 token 隔离，旧账号慢回包不能覆盖新账号缓存。
- 锦囊保留业务内容但摘掉 tab 身份：补返回与无栈执行兜底，移除 syncTabBar/coach/底栏留白；全仓禁止 `switchTab` 到 pouch。新增一次性「锦囊搬到这了」提示，studio 老路径与订阅消息默认落地统一转执行。
- `gotoExecution(today/week)` 使用一次性内存 intent；显式点击执行进今日，无 intent 返回保留原段与滚动位置。reviewDue 红点移到 index 2，并在常规底栏、问策浮岛和冷启动生产链保持同位。
- 新增执行到达、军令完成、回填、复盘、兵器、锦囊曝光/点击事件（先更新 `shared/contracts.d.ts` 再同步服务端白名单）；coach 升 v3。原生守护测试更新到 54 路由并补执行慢接口、缓存、路由、失败态与红点回归。

### 2026-08-17 · 修复生产大文件被 Nginx 误挡并补清晰 413 提示 · 影响面：生产 Nginx、数字人视频上传、知识资料上传、原生/H5 错误提示

- 生产实证：同一用户约 31.6MB 的数字人视频在 13:24–13:28 连续 7 次被 `wxapi.aibuzz.cn` 全站 `client_max_body_size 10m` 直接返回 413；请求未进入军师 API、未调用 AIStar、未产生 `VideoCloneHold`。客户端/后端允许 100MB 与生产网关 10MB 不一致是根因，不是训练引擎失败。
- Nginx 模板改为路由级放宽：知识上传 25MB（业务上限仍 20MB），数字人 `avatar/consent|clone` 与素材上传 120MB（形象视频业务上限仍 100MB），普通 API 保持 10MB；413 由 HTML 改为带业务码的 JSON，避免全站放大攻击面。
- 原生/H5 错误映射新增数字人 100MB、资料 20MB 专属提示；即使旧环境仍返回纯 HTML 413，也会明确说明「文件太大」并给出压缩/拆分动作，不再显示模糊的「上传未能完成」。
- 生产已备份 `/etc/nginx/conf.d/junshi.conf.bak-codex-20260818-upload-limits`，`nginx -t` 通过后平滑 reload；31.6MB 无效鉴权探针已穿过网关到达 API 401，121MB 视频与 26MB 资料分别返回专属 JSON 413，公网 health 正常，Nginx/军师 API 均 active、`NRestarts=0`。

### 2026-08-17 · 报告标题完整展示与 PPTX 上传失败闭环 · 影响面：server 报告生成/文档解析、原生小程序报告分享图/智库上传、H5/PC 格式入口

- 报告生成新增标题收口：模型标题统一去掉悬空连接词、换行和超长解释，工具提示同步要求标题为完整短语；分享图移除固定 14 字截断，按画布实测宽度自动换行和缩字号，长标题完整绘制。
- 文档解析新增 PPTX：复用现有 OOXML ZIP 读取能力，按 `slide*.xml` 页序提取文本，智库批次类型标为「演示文稿」；原生、H5、PC 的上传格式入口同步加入 `.pptx`，旧版 `.ppt` 明确引导另存新版格式。
- 智库批量上传补前置格式/20MB 校验、文件选择器失败提示与逐文件错误汇总；全失败/部分失败都保留红色状态并弹出原因，不再被末尾「已送达」覆盖。新增报告标题、分享长标题、PPTX 文本提取及上传失败提示回归。
...
- **只在开发版显示**：读取微信 `getAccountInfoSync().miniProgram.envVersion`，仅 `develop` 渲染；`trial / release` 与身份读取失败均安全隐藏，体验版和正式版不会露出调试角标。
- **保留 mock 验收能力**：`MOCK` 角标仍可点按切换「经营中 / 空态」，server 环境角标只作标识、点击不产生数据副作用。补齐四种地址分类、版本显隐和五 Tab 接入回归。

### 2026-08-17 · 对话增加安全的流式「思路摘要」 · 影响面：模型提示词、LLM 流式网关、ChatReply 契约、原生小程序/H5/PC 对话 UI

- **澄清旧状态**：「正在梳理上下文」仍是读取上下文、等待 provider 与收尾阶段的状态提示，不再把它包装成模型真实思维过程。
- **公开摘要独立成流**：普通对话允许模型先在可见输出通道撰写 1–3 行公开思路摘要；网关可跨任意 token 切分识别 `<public_thought>` 块，向客户端下发独立 `thought` SSE，正文不再出现内部标签。非原生流、mock 降级和最终完整回复沿用同一结构。
- **安全边界**：公开摘要只写事实核对、判断维度或回答路线；供应商隐藏的 `thinking_delta` / `reasoning_content` 不转发，系统提示、工具参数、密钥、安全规则和隐藏 chain-of-thought 均不进入客户端。
- **端上体验**：原生小程序、Taro H5 与 PC 在生成中展开摘要并显示呼吸状态，正文完成后折叠成「思路摘要」；`ChatReply.thoughtSummary` 随消息落库，退出重进仍可展开。旧客户端忽略新 SSE 事件也能继续消费正文和完整 `chat` 事件。
- **回归**：新增完整抽取、标签跨块解析、无协议不吞字、提示词安全边界与历史回复字段守卫；服务端构建、原生构建、app typecheck 与 app 全量测试通过。

### 2026-08-16 · 命盘地区改为三级滚轮并统一 23:00 子初换日 · 影响面：原生小程序/H5 生辰录入、排盘引擎、存量 v3 命盘、命盘报告与天势注入

- **地区录入**：命盘页的自由文本出生城市改为省/市/区三级滚轮，整行 48px 点击区、明确未选择态；选中后提交完整行政区文本，服务端长度上限由 20 调整为 40，仍按保守城市表识别经度，未命中会明确提示按北京时间排盘。
- **子初换日**：排盘引擎升 `paipan-v4`，`lunar-typescript` 固定 `sect 1`。真太阳时校正后的 23:00–23:59 统一按次日排日柱，报告公历、农历与日柱共用同一有效日期；命盘档头改显「子初换日 · 23:00」，时辰选项改为「子正 0-1 / 子初 23-24（换日）」。
- **例证锁定**：公历 2025-03-16 23:30 固定输出排盘公历 2025-03-17、农历二月十八、乙酉日、丙子时，与 3 月 17 日 00:30 同柱。
- **存量一致性**：命盘报告本就按当前引擎现算；为避免对话/天时仍读旧快照，存量 `paipan-v3` 首次 `loadChart` 时惰性重算并落 v4，v1/v2 继续保留原盘。服务端构建、原生小程序构建、前端 typecheck 及 22 项排盘/命盘报告回归全绿。
...
- **模型清单不是健康硬门**：生产首轮验收确认 qnaigc 的聊天与 Thinking 均成功，但可选 `GET /models` 返回 404；现将 404/405/501 记为 skipped，不再反推端点不可用，鉴权/限流/5xx 仍失败。修复前已持久化的此类假失败不会恢复进 latest gauge，而会在下一轮立即按新语义重测覆盖。
...
- **污染源与消费方双保险**：`structuredBilling` 在 `after` 清理 AI v2 四表；全程 mock 的 `integration.test.ts` 在套件开始/结束也显式复位 AI v2 配置，因此旧测试异常退出留下脏库时，单跑 TC-L 仍可自愈。TC-L 失败断言现在直接打印请求序号、status、code 和错误，不再只给“全部应成功”。AI v2 是运营配置，不能塞进生产 seed 共用的 `resetBusinessData()` 清理范围。
- **并发旁路一并修正**：8 路首次对话还暴露 `UserJourney.upsert()` 的首次建行唯一键竞争；改为 PostgreSQL `createMany(skipDuplicates)` 后再读取，所有并发事件都能继续迁移，不再依赖 best-effort 吞掉 P2002。补 16 路首次事件回归。

### 2026-08-14 · 真实用户作品域与内部海报夹具硬隔离 · 影响面：CreativeJob 数据模型、C 端作品/详情/资产/派生动作、方向样例、运营任务台、原生 mock 与回归测试

- **不再把测试内容当用户作品**：`CreativeJob.audience=user|internal` 成为可见性真源；C 端列表、详情、改文字、换方向、取消和资产文件只认 user，内部任务统一 404。运营任务台显示归属，并允许 owner 把 E2E/人工验收显式切 internal 或恢复 user，全部留审计，不再靠标题和 id 前缀猜测试数据；方向样例来源另标 `sampleSource`，在样例仍存在时禁止恢复 user。
- **方向样例必须先归类再复制**：只有已在任务台显式标 internal 的成功任务可以生成公开样例；未归类的真实用户作品返回 `SOURCE_JOB_NOT_INTERNAL`，服务端绝不在建样例时自动隐藏客户作品。字段上线前已经存在的样例来源通过 `CreativeDirectionSample.sourceJobId` 在列表、详情、派生动作与资产文件继续兼容拦截。
- **原生 mock 不再自动塞两张海报**：删除经营中档案的两张预置成品，读时清理已有 storage 的 `mock-poster-seed-*`；mock 作品库只显示当前 mock 账号亲手创建的任务，真实 JWT 仍强制走 server。生产只读核查没有活跃用户海报；预发明确 E2E/人工验收任务待部署后由运营标 internal，本次未修改线上数据。

### 2026-08-14 · 手机号快捷登录停止静默换绑，账号归属回到不可变用户 ID · 影响面：登录契约、服务端身份关联、原生微信/H5 提示、审计与回归测试

- **根因与边界**：旧 `/auth/wechat-phone` 在 openid 已命中账号、但本次授权手机号不同且未被占用时，直接覆盖 `User.phone`，导致同一 `userId` 在不同手机号登录下看到完全相同的快出片与海报内容；这不是作品串租户，而是两个号码先后进入了同一个账号。只读生产审计在 8 个手机号快捷登录用户中发现 2 个账号有多个脱敏手机号且无显式换绑记录，本次没有修改生产数据。
- **新身份规则**：账号归属只认不可变 `User.id`；同 openid/unionid 继续进入原账号。只有历史 `wx_<openid>` 占位号首次补真实号时允许自动升级；已有真实号与本次授权号不同则登录照常成功、数据库不改号，并通过 `LoginResult.phoneBinding=mismatch` 下发两侧脱敏号码。原生端在登录完成后做非阻断解释并引导到设置验证新号，H5 同步消费契约；显式设置换绑仍验证新号码，`PHONE_TAKEN` 保持不变。
- **冲突与审计不再含糊**：旧微信关联逻辑不再吞掉所有异常；可识别的 openid/unionid 唯一键竞争返回 409 `WECHAT_ACCOUNT_CONFLICT`，真实数据库故障继续抛出。显式换绑成功审计同时记录新号和旧号的脱敏值，便于后续追溯。
- **遗留策略**：历史日志只有脱敏号，且手机号存在运营商回收风险，因此不自动复原旧号、不建立永久占号；需要恢复时按微信身份与当前号码验证人工处理。后续若真实量级扩大，再单独设计有验证状态和释放策略的凭证历史表。

### 2026-08-14 · 海报双路线预发与微信开发版 0.2.33 验收完成 · 影响面：预发部署、微信开发版、真实方向样例与端到端验收

- **预发基线**：海报双路线、免费改字复用、六个真实方向样例和真机可读方向卡已部署到预发 `795a9c1`；开发者工具问题面板为 0，标准路线从表单、10 钻扣费、worker 到成品页已真实跑通，且只产生 `poster_png`、没有主视觉供应商资产。
- **开发版已上传**：`0.2.33` 已通过正式发布守卫上传到 AppID `wx810ebe6dfef8e75f`，总包 2,278,938 B；构建 API 固定为 `https://wxapi.aibuzz.cn/api_preprod`，仅供次日真机验收，未提交审核或发布。

### 2026-08-14 · 微信开发者工具 TypeScript 诊断归零 · 影响面：app tsconfig、H5 图标与 mock 文案替换、开发者工具代码质量面板

- **兼容内置语言服务**：小程序侧 `tsconfig` 从 ES2021 收回到微信开发者工具 TypeScript 4.1.2 能识别的 ES2020，并移除该版本不支持的 `moduleDetection`；两处 `replaceAll` 改为等价的 `split/join`。不改变运行结果，消除命令行全绿但开发者工具常驻 2 个配置错误、进而掩盖真实问题的噪声。

### 2026-08-14 · 海报方向选择真机可读性与录音声明校正 · 影响面：原生微信/H5 海报确认页与换方向面板、原生 app.json、开发者工具告警与回归守卫

- **方向样例不再挤成三列**：确认页与收费「换方向」面板统一改成横向 3:4 大缩略图卡，一屏保留约两张半作为可滑提示；名称、方向说明与「AI 演绎非本人」提示提升到真机可读字号，真实样例不再被正方形裁切。
- **移除无效录音声明**：微信官方当前 `app.json.permission` 只支持 `scope.userLocation`，原 `scope.record` 会被开发者工具明确报 invalid。录音仍在用户主动点击后走 `getSetting/authorize`，用途在小程序管理后台《用户隐私保护指引》申报；新增原生守卫防无效配置和三列小卡回归。
...
- **提示词整份重写**（`POSTER_DESIGNER_PROMPT`，**不是在 seed 模板上打补丁**）。旧版只讲流程不讲手艺——模型知道该问什么，但不知道什么样的海报算好。新版写进三样东西：
  - **三秒法则**作为总纲：路过的人给三秒，必须接住「讲什么 / 跟我有关吗 / 我该做什么」。由此推出「一张海报只讲一件事」与「删是设计动作，不是妥协」。
  - **五条通用设计原则落到可执行口径**（抽象原则模型执行不了）：对齐＝视觉对齐优先于数学对齐；分组＝组内间距必须明显小于组间间距（比加分割线管用）；识别性＝缩略图大小下主标题仍可读、层级靠字号级差不靠加粗加色；颜色＝一主一辅一强调、强调色只用在最该被看见的那一处；风格统一＝整张图只有一套形状语言。
  - **替客户挡坑清单**（真实客户一定会提、模型不预先知道就会照做）：都放上去等于一条没说 / logo 只需被认出不需被看见 / 空是花钱买的 / 行动路径只留一条 / 大面积高饱和会压掉所有信息。
  - 另含文案手艺：主标题 12 字内、动词开头、说结果不说品类；卖点三条必须不同维度。以及一条硬规矩：**绝不问客户「想要什么风格」**——问了只会得到「高端大气上档次」。
- **新增 `PosterBriefDraft.designNote`**：抽取时顺带产出 2–3 句「这张海报会长什么样」，写作要求同样按五条原则来。**抽不出就不下发这个键**（不是空串）——空串与键不存在对前端是两种行为，`showForm` 初值直接由它决定。上限 240 字。
- **确认页信息架构重排**：设计说明卡 → 档位选择 → 「这些细节要改吗」→ 折叠的表单。档位从「画面长什么样」里搬到主视图——它是用户在本页做的主要决定，不该埋在折叠段里。抽不出说明时表单自动摊开，不会留一片空白。
- 测试：server 1632 全绿。designNote 的 240 字截断走**真实抽取分支**验证（`AI_ALLOW_REAL_PROVIDER=1` + stub fetch，仓库既有手法），不是单测私有函数；另实测 `DRAFT_SYS` 加长后五条原则标记与尾部 JSON 契约完整送达、首次即解析成功。⚠️ 后者只证明**管道扛得住这个长度**，证明不了真实模型不会在更长提示词里走神——`fetch` 被拦在模型之前，这一点在密封测试里无法验证。
...
- 这条与系统提示词**直接矛盾**：提示词第 5 条写着「你自己写的 data:image URI（如内联 SVG）」是合法来源。模型照做 → 连挂三轮 → 静默回落模板，任务仍是绿的。属「不可观测的降级」那一类。
- 预发同一份 brief 实测（真实模型，标准档）：**修前 5 次挂 4 次**（2 次死于本条，2 次为模型侧 `out_of_bounds`）；**修后 4 次成功 3 次**，唯一失败是 `text_overlap`。画面密度同时明显回升（成品 PNG 从 ~154KB 到 750KB–1.4MB，四角裁切标记 / 侧边竖排标注 / 点阵纹理这类靠内联 SVG 的细节回来了）。
- 放行的同时补一道闸：百分号编码会把 `<script>` 藏成 `%3Cscript`，文档级 `<script>` 正则看不见它——对 `svg+xml` 先 `decodeURIComponent` 再查脚本 / `on*` / `javascript:` / `foreignObject`，解码失败按有问题处理。渲染层的 `javaScriptEnabled:false` 与请求拦截白名单仍是后两道。
- 遗留（未修，模型侧版面问题）：`text_overlap` / `out_of_bounds` 在 3 轮回喂后仍偶发——回喂带了精确 selector 与实测数值，但模型倾向保留同一套构图去微调，改不动根因。下一步是让 `fixDirective` 对重叠给出**结构性**指令（拆到不同垂直区间 / 删掉装饰性数字层），而不是继续加轮次。

### 2026-08-12 · 锦囊改为游客不可见（个人私产页口径统一） · 影响面：锦囊页游客态

- 锦囊装的是「军师替你出的成品」，游客本来只能看到一排空架子与零计数的宫格。改为整页一屏登录引导，与图籍同一口径；游客态不取任何数（连 `/agents` 都不请求，少一次请求）。
- **登录门铁律仍然守住**：微信整改要求的是「不得强制登录才能使用」，主线仍留给游客——问策本地开场、发送才登录；战局有游客引导空态。个人私产页（锦囊 / 图籍）要求登录不属于强制注册。
- 实拍核对游客态与登录态；守护测试 64 条全绿、mock 编译通过。
...
- **命盘 / 时运回主流程**：只留主公菜单「藏太深不好找」。战局三势行下加一行时运条（本月攻守 + 拐点月，右侧入命盘报告，整条点进天时日历），`fortuneOn` 时常驻，未录生辰给引导；主公菜单两行保留，两个入口并存。命理仍只调节奏、不参与经营结论。
- **补回「帮我写脚本」**：原在点兵横滑卡组上，随卡组被砍。改挂在待执行军令卡上（与兵器条并列的次级动词），带该条军令原文进 IP 军师对话。「让军师出新方案」按评审意见不补。
- **已办军令限量**：当天已办最多内联 3 条，其余与历史日期收进归档行（可展开）；展开/收起只切本地视图，不重新取数（派生素材放 `this`，由 `applyOrderView` 单点组装）。
- **复盘红点接上产出方**：`custom-tab-bar` 一直在消费 `reviewDue`，但小程序侧从来没有产出方（只有 H5 的 `store.ts` 有），红点从不亮、此前把索引 2→1 是空操作。现在 `services/store.js` 按与 H5 同源的判据产出（账本取到过 + 今日未复盘 + 已过 21:00，15 秒节流 + 单飞 + 失败静默），`syncTabBar` 下发，战局页 onShow 触发拉取。
- **`/api/video/*` 纳入过载闸豁免**：它是到 aidrama 的同步 BFF 代理（上游预算 60s），此前漏在 `isLongRunning` 名单外；作品页升成一级 tab 后每次进入都打一次，上游一慢就能把 `MAX_IN_FLIGHT=200` 的在途槽位耗光、连带把无关快接口打成 503。同时对齐超时：`aidramaJson` 支持 per-call `timeoutCapMs`（只收紧不放大全局预算），`/video/works` 取 10s，端上对应 12s——服务端先于端上放手，槽位及时释放。新增 `test/overloadGateExempt.test.ts` 钉住豁免与超时关系。
- 验证：server 全量单测 1583/1583（带 `.env.test`）、`tsc --noEmit` 通过；小程序守护测试 64 条全绿、mock 编译通过。
- 评审中标记「暂记 TODO」的一项（EcoTool 外跳工具在原生端无 `navigateToMiniProgram` 渲染器）保留在 §13，理由是目前还没有真实的外部小程序。
...
- **取数失败不再伪装成空态**：战局与图籍的 `loadFailed` 判据从「两条主干都挂」改成「任一挂」（只挂案卷时页面会说「还没有案卷 / 还没有军令」）；锦囊从「三条全挂」改成「任一挂」，且失败时不再同屏渲染「还没有作品」空态卡；**主公补上 `loadFailed` 与重试条**——`/me` 挂掉时整页会退化成「0 案卷 / 尚未开通 / 完整度 0%」，与真实空账号无法区分。
- **loading 期不冒充空态**：战局的「三势还没断 / 还没有军令」、图籍的四处 `stage-empty` 都加 `!loading` 前置；战局 `load()` 补并发闸（onShow 与六处写操作都会触发，原先可叠加发车）。
- **图籍游客态**：原先游客早退后仍渲染假额度卡（`200/200MB`、`0 / 30`）＋「还没放资料进来」，把「未登录」说成「功能还没上线」。现在整页出一屏登录引导；同时补两处漏掉的登录门——`refreshForces`（服务端重算三势并落库，战局侧一直有门，两页口径不一致）与 `deepFirst`（SKU 扣费路径）。
- **锦囊细节**：置灰格取不到 agent 时不再静默失效（给 toast）；登录门 reason 从 `save` 改 `purchase`（本页唯一门禁是启用创意军师=扣费）；三条作品通道加 90 秒缓存，压低对 `/api/video/works`（同步 BFF 代理、未纳入过载闸豁免）的调用频次，正解记 §13。
- **失效跳转与旧 tab 文案**：军令详情 `topics` 动作原本跳「图籍能力中心」——该区块已删，现改指锦囊手艺；`upload` 文案「智库」→「图籍」；`report-card` 与军令详情的「执行页」→「战局」；作品库按钮「去找海报设计师」→「去锦囊挑手艺」（落点已是锦囊）；能力目录页的 `placement` 六处旧 tab 名对齐（该页是主公「已启用的兵器」的落点，直接渲染这些字）。
- **死代码清理**：战局删死字段 `dossierSource` / `pendingDecisions`（随横滑卡组与执行信号删除后无渲染方）；战局 scss 去掉整份 NextStepCard 导入（下一步卡已删，55 行全死）与 `.nsc` / `.ml-go`；图籍去掉三条宿主已删的补丁；**锦囊不再 @use 任何页面级 scss**（原先导入战局 392 行 + 点兵 488 行只命中一个 `.section-label`，已内联）——`pages/pouch/index.wxss` 从 25.6KB 降到 5.7KB，实拍确认视觉零变化。
- **守卫收紧**：服务端路由扫描的正则原先要求引号紧跟 `/`，`server/src/services/wechatSubscribe.ts` 下发的无斜杠 `pages/studio/index` 因此逃过检查；已放宽，并在 §13 记明删 studio 注册前必须先改它。
- 已知未修（记 §13 / H5 文档）：`/api/video/works` 的过载闸豁免与端上 30s vs 上游 60s 超时对齐属服务端改动；`/reports` 无分页；`PATCH /modules/:key` 死路由；H5/PC 侧 21 处 tab 分叉（已排期随 H5 移除）；复盘红点 `reviewDue` 在小程序端从来没有产出方（本次索引 2→1 是空操作）。
- 守护测试 64 条全绿；mock 与 server 双模式编译通过；`npx tsc --noEmit` 通过。

### 2026-08-12 · 补回 IA 收敛时丢掉的两样：创意军师启用链、日/周计划打卡机制 · 影响面：战局军令区、锦囊手艺格、mock 案卷种子

- **创意军师启用链接回**：置灰手艺格原先只跳问策导览，而问策列表明确过滤 `type !== 'creative'`，导致全端再无处启用创意军师——付费链断在这里。现在点置灰格开 `agent-unlock`（登录门在前），启用成功后由 `agentUnlocked` 带进这位军师的对话（第一次仍由军师带着做）。卡面仍不标价，价格只在启用层那一刻出现，守护测试同时锁「卡面无价格」与「启用层可达」。
- **日/周计划恢复**：战局军令区加回 `今日军令 / 本周` 两段（复盘仍在抽屉，不回三段）。周计划重做为打卡机制：**七日连续条**（按日历固定七格，实心=全办完 / 半实=办一半 / 空心=有令未动 / 虚线=当天没出令，今日带光圈）+ 连续复盘天数 + 本周完成 x/y + 按天军令记录（每天带 done/total）。原来的扁平七天列表只能回看，撑不住「别断」的压力。
- **mock 案卷种子补历史**：只种今天的话打卡条六格全虚线，等于看不见机制。现在昨天全办完、前天办一半、三天前有令未动、四天前无令、五天前全办完，四种态一次看全。注意案卷按账号持久化且「经营中」优先用存储真值，走查要看新种子需先清 `junshi.dossier.<userId>`。
- 「下一步」卡（journey）按产品决定不恢复；`api.journey` 在小程序端仍无消费方（见 §13）。
- 守护测试 64 条全绿（含新增的日/周两段与七日条断言）。

### 2026-08-12 · 五 tab UI 走查修复：插画手艺格、战局节奏收敛、图籍新图标、mock 档案开关可点 · 影响面：小程序五个 tab 页、共享 app.scss、底栏图标资产

- **锦囊手艺格接插画**：`app/src/assets/craft/{clip,poster,report,agent}.jpg`（imagegen 生成、裁紧压到 180px/合计 29KB，随 ASSET_ROOT 拷进产物，与生成头像同一条资产路子）。手艺格改「插画贴片 + 名称 + 两行角色 + 计数」，宽高全部写死、标题单行截断、角色行定高两行，长文案不撑卡；置灰格插画去饱和、结构与正常格一致。
- **「最近做的」加缩略图**：海报取 `poster.previewUrl`（600 秒签名，故本页不缓存）、成片取 `thumbnailUrl`，取不到或加载失败（`binderror`）落回该类型插画，不留破图。
- **战局页节奏收敛**：容器改 flex + 统一 gap，直接子元素上下 margin 归零（合并页来自两份 scss，各带 margin 是「间距乱」的根因）；页头留白 22→2px、今日献策压成单行带省略；三势强度条 `margin-top:auto` 贴卡底对齐；无军令时不再单列兵器条（「还没有军令」引导卡下面跟两张兵器卡＝一边说没事做一边推工具），有军令时富余处方最多补 2 张。
- **手动加军令改 showModal**：页内 input 在长滚动页里被键盘顶走焦点（用户反馈「页面跳走了」），改为点按开 editable 弹层。
- **图籍换专属图标**：新增自绘 `codex`（两册线装书 + 装订线 + 封印）替换借用的 `muster`（打勾名册语义是点检不是档案），登记进 `CUSTOM_TAB_ICONS` 并发射十一个主题态 SVG。
- **MOCK 角标变成真开关**：原先 `pointer-events:none` 且钉在状态栏下——位置够不着、事件也被吃掉。改到底栏胶囊上方左侧、28px 命中区、半透明压深；五个 tab 页统一显示当前数据档案并可点切换。
- **问策输入区空白栏**：提示问题 pill 是 fixed 定位不占布局，滚动区却无条件预留 36px，浮岛上方常年悬一条纸色空栏。改为 `.pill-on` 按需预留，判据与 pill 的 `wx:if` 逐字同源并由守护测试锁死。
- **主公老师/班级群未分配占位**：两个 sheet 不再在入口处转跳社群页，未分配时展示占位框、说明与「先去军师社群」；三个 sheet 补 `store.setOverlay`（历史欠账：二维码底下压着一排 tab）。
- **上轮 P2 收口**：五页补底部渐隐遮罩（悬浮胶囊底栏与屏幕底缘之间那条带子会半切内容）、`pointer-events:none` 不吞点击；战局/锦囊/图籍主干取数全失败时给可重试提示条（空态与读失败必须分得开）；锦囊游客态不再摆一排「—」。
- 守护测试 62→64（新增 pill 判据同源、渐隐层与角标可点两条），全绿。
...
- 记录容量边界：军师宿主为 4 核 / 7.3 GiB、无 Swap、可用内存约 1.4 GiB，现有 Clip 预发 Java 常驻约 644 MiB；当前不应再叠加 Clip 生产与 FFmpeg 峰值负载。
- 补齐配置边界：军师 BFF 持有接入、用户/积分、审核、脚本 AI、微信通知和审计配置；AIStar Clip 持有 clip 数据、石榴供应商、渲染/质检/存储及其运行参数。小程序不持有服务端密钥，两边只成对配置 service token。
...
- 生产军师通过短时 JWT 只读探针成功读取 `ct_shouyi / ct_kaimen / ct_shiti` 三套内置模板；API、运营后台、H5 源站与 AIStar 公网健康检查均为 200，裸 IP `/admin/` 保持 404。新版问策开关在生产、预发均为 `chat=100%`。
- 石榴生产地址与凭证仅在服务端落盘，军师到 AIStar 使用独立 service token；本次只验证配置、鉴权和模板读取，没有创建训练或生成任务，不消耗供应商点数。生产媒体机审尚未配置，上传入口继续 fail-closed，未以测试旁路冒充生产闭环。
- 微信小程序开发版 `0.2.32` 已上传 AppID `wx810ebe6dfef8e75f`，生产 API 为 `https://wxapi.aibuzz.cn/api`，总包 2,049,556 B；已进入微信后台开发版，按测试阶段约定未提交审核或发布。
- 里程碑的产品可读结论与验收边界已同步到 Notion「军师 · 工程变更日志（持续更新）」。

### 2026-08-11 · 修复 iOS 注册补档点击失效并统一微信头像昵称组件 · 影响面：注册补全、问策对话、独立对话、个人设置、老板页头像入口

- 登录补全层打开时，问策 Tab 与独立对话页都会卸载底层 `fixed + always-embed` 原生 textarea，并主动收起键盘，避免 iOS 独立原生输入层继续截获头像、昵称与保存区域的触摸。
- 注册头像组件未返回临时地址时改为明确提示，不再静默无响应。
- 个人设置头像从普通 `wx.chooseMedia` 统一为平台 `chooseAvatar`，称呼输入改为 `type="nickname"`；头像仍上传到现有 `/me/avatar`，昵称与公司仍经 `PUT /me` 保存。老板页已有头像也可直接点进设置。
- 增加静态回归测试，锁定两个对话宿主页的登录门卸载、头像失败反馈，以及设置页的头像昵称组件和保存链路。
...
- `ClipProject.subtitleStyle.aiWatermark` 成为可见水印唯一开关；新项目显式写入 `false`，历史项目缺字段也按关闭处理。确认页提供可保存开关，只有用户主动开启后才在预览和最终成片右上角显示“AI 生成”。
- AIStar 字幕层与固定尾卡不再无条件烧录水印；正式总装和不耗点数的测试总装都消费同一项目偏好。测试总装仍永久保留独立“测试演示”标识，避免测试片被误认成真实生成。
- 作品 DTO 新增 `aiWatermark`，作品页角标和发布确认文案据此展示；模板预览角标改为“智能模板”，不再误导为默认成片水印。自动化只验证本地叠加层和契约，不创建石榴出片任务。
- 同步修正多数字人卡片回归：每个形象重新展示关联声音的来源、训练百分比/失败状态与增强完成时间，原有声音训练守护测试按多资产 `item` 口径更新并恢复全绿。
- 已部署隔离预发：AIStar `dcf5f37d-20260812T023005Z`、军师 `9285a9b`，两服务均 active/running、`NRestarts=0`，AIStar `force-mock=false`；军师 pgvector 列与存量向量 `2/2` 正常。`9285a9b` 原生 server 包已指向 `https://wxapi.aibuzz.cn/api_preprod` 并 auto-preview 到 AppID `wx810ebe6dfef8e75f`。全程未创建石榴任务。

### 2026-08-11 · 作品默认全量、模板固定视频与多数字人闭环 · 影响面：作品、模板、配画面、分身管理、声音复用、AIStar BFF

- “我的作品”默认进入“全部”，生成中和已完成改为筛选；已完成包含 done/published，列表直接显示真实成片缩略图和播放入口。
- 模板详情不再使用独立营销时长：固定片段、模板总时长和创建后的工程都来自同一模板骨架与 `tailClips` 配置；固定片段明确显示视频封面、名称和秒数，点封面可直接播放，AIStar 内置三段竖屏视频作为缺省素材，运营配置后不覆盖。
- 数字分身升级为多条可管理资产：每个形象有名称、预览和关联声音；新建可复用已有 ready 声音，形象可单独更换或删除。配画面页增加数字人选择器，切换时自动保存对应 `avatarId/voiceId`，确认页继续展示本片所选形象。
- 军师 BFF 新增多分身、声音清单和按 ID 删除路由；AIStar 渲染链严格使用项目选中的形象/声音引用。测试仅使用桩和本地媒体，不创建石榴训练或出片任务。
- 已发布隔离预发：AIStar `296756a9-20260811T180005Z`、军师 `20be475`，两服务均 active/running、`NRestarts=0`；三套模板固定视频/封面/时长和多分身/声音空清单路由只读通过，军师公网 health 正常、pgvector 2/2 完整。发布未上传本人素材，未创建任何石榴任务。

### 2026-08-11 · 修复 AI 改稿假成功 · 影响面：文案对话、结构化模型预算、失败反馈、预发回归

- 预发证据确认两次“换成煎饼果子（摊）”请求均返回 200 / `applied=true`，但实际走了结构化输出失败兜底，只把原修鞋稿重新合并后冒充“已更新”。根因是改稿沿用辅助调用默认 700 token，6–10 段 JSON 容易被截断，`llmJson` 又将解析失败静默收成 `null`。
- 改稿改用 Zod 结构化输出与一次校验修复，输出预算提高到 2400 token；提示词明确“换成/改成品类”必须重写整稿并清除旧行业、旧商品和旧动作。
- 真实模型两轮仍无法产出合法新稿时返回明确错误并保持原稿，禁止再返回假成功；无真实模型的测试环境同样标记 `applied=false`。定向测试覆盖“失败不冒充成功”和“换品类后旧主体不残留”。
- 修复已部署军师预发 `3b0ea28`；公网与本机健康检查均为 `ok/db up`，服务 active/running、`NRestarts=0`。部署后 `knowledge_chunk.embedding_vec` 已由 `pgvector.sql` 恢复为 vector，2/2 条存量向量完整。本轮没有主动调用模型，留给真机实际改稿验收。

### 2026-08-11 · 修复竖屏成片与逐句字幕并重排快出片首页 · 影响面：首页、开拍门槛、草稿、模板、成片预览、AIStar 总装

- 成片页播放器改为独立宽度容器内的真实 9:16 比例，删除把竖屏容器强行覆盖为 420px 横卡的固定高度；视频使用 `object-fit=contain`，封面与播放过程都不再被横向裁切。
- AIStar 将合并画面段继续拆成逐句 caption cue：同一素材仍只生成/使用一次，但字幕按每句估算时长相对真实段音频等比换算，依次烧录，不再把三句合成一张两行省略字幕。
- 首页按 `mobile-app-ui-design` 的“前置条件先于主任务、首屏暴露高频入口、渐进披露”重排：作品与素材成为首屏双入口，数字分身状态先于模板且未就绪时阻止进入制作，返回页面会同时刷新分身和草稿；草稿压成进度行，模板改横向紧凑卡，产品说明压成一张轻量横卡。
- 交互命中区保持至少 44px，未创建/训练中/已就绪三种分身状态均给出唯一明确动作。自动化覆盖分身先于模板、作品入口不藏导航、横滑模板、9:16 容器和逐句字幕时间窗；不创建石榴任务、不消耗点数。
- AIStar v0.126 已发布隔离预发 `3560b942-20260811T170602Z`，force-mock 关闭、服务 active/running、`NRestarts=0`、3 套模板通过；server 真机包 `385c863` 已指向 `https://wxapi.aibuzz.cn/api_preprod` 并 auto-preview 到 AppID `wx810ebe6dfef8e75f`。

### 2026-08-11 · 补齐素材封面与可播放预览 · 影响面：素材库、配画面卡、出片预览、AIStar 素材契约

- 素材库网格由纹理占位改为真实 `previewUrl` 封面；浏览态点整卡直接打开，挑选态点整卡继续完成选择，同时封面上保留独立“播放 / 预览”入口。
- 配画面已选卡拆分基础交互：点封面或素材信息打开原媒体，点“更换”才进入拍摄/相册/素材库；出片预览中的配画面缩略图也可再次点开核对。
- `ClipAsset.contentUrl` 返回原始媒体的短期签名地址，视频列表仍只加载 JPEG 封面，只有用户主动预览才加载视频；图片使用大图预览。三处弹层都避让安全区并保留 44px 关闭/操作区域。

### 2026-08-11 · 专属声音训练状态可见 · 影响面：分身管理、声音增强、训练轮询、订阅提示
...
- 配画面卡不再两处重复显示微信 `tmp_*` 临时文件名；统一显示“我的视频素材 / 我的图片素材”等可读名称，并以单行省略和最小宽度约束阻止横向溢出。
- AIStar 为上传视频抽取约 0.5 秒 JPEG 缩略图，图片直接使用自身预览；历史视频素材在读取时 best-effort 补图。配画面页同时读取素材清单与分身预览，素材段和分身段都展示真实画面。
- 出片预览改为固定头部、独立滚动列表和带安全区的固定底部确认区；确认按钮不再压住最后一个画面段，关闭按钮恢复可见的中性色。
- 本轮按 `mobile-app-ui-design` 的任务聚焦、44px 点击区、8 点间距和内容优先原则收口，没有改变镜头组合、报价或生成逻辑。

### 2026-08-11 · 数字分身展示用户视频预览帧 · 影响面：形象上传、分身接口、首页能力卡、分身管理

- 上传形象视频时由 AIStar 在调用石榴训练前抽取约 0.5 秒的 JPEG 预览帧并持久化，`ClipAvatarView.imagePreviewUrl` 通过军师 BFF 原样返回；首页能力卡与“我的数字分身”主卡优先展示用户真实画面，不再只显示通用人形图标。
- 已存在的形象若还没有预览帧，读取分身时会从保留的源视频 best-effort 补抽并写回；删除分身会同时清理源视频和预览帧，避免遗留个人素材。
- 预览图片按容器 `aspectFill` 展示，横竖视频都能以人物为视觉中心；抽帧失败会在创建石榴任务前明确失败，不消耗供应商点数。

### 2026-08-11 · 修复数字分身更换路由与删除后复活 · 影响面：分身管理、克隆入口、AIStar 删除语义

- 更换形象路由的显式 `mode=avatar` 现在优先于旧版 `step=1` 兼容判断，不会再被默认 step 误送到声音录制页。
- AIStar 删除从“只删最新一条形象/声音”改为清理该用户全部未删除的石榴 Avatar 与 Voice 版本，避免旧声音或旧形象在最新记录删除后重新成为当前分身。
- 分身管理补齐 `imageStatus=none` 的真实空态：显示“还没有创建形象 / 上传形象视频”，不再把未采集状态写成“正在云端训练”。原生 68 项与 AIStar 删除服务 3 项针对性测试通过。

### 2026-08-11 · 「快出片」单视频创建数字人并新增训练通知 · 影响面：创建主链、分身管理、石榴契约、微信订阅消息

- 纠正上一版仍把声音放在 Avatar 前置步骤的问题：石榴 `/avatar/create` 的 `speakerId` 是用于 demo 的选填参数，普通用户现在只需上传/录制一段视频即可立即进入形象训练；`authId` 同样保持选填。AIStar 无可用音色时 best-effort 从视频提取原声创建基础 V2 声音，失败不阻断或回滚形象，专门录音下沉为分身管理中的可选增强。
- 依据 `mobile-app-ui-design` 将创建页收成“上传视频 → 云端训练”两步，把分身管理从两张同权窄卡改为“全宽形象主卡 + 专属声音增强行”；主次动作保持 44px 点击区和统一对齐。竖屏视频继续用紧凑容器与 `object-fit=contain` 完整展示。
- 新增 `avatar` 微信订阅场景：用户在训练页直接点击“训练好通知我”触发一次性订阅，后台任务仅轮询最近 24 小时已订阅的既有训练，不创建供应商任务；形象 ready/failed 后按 32308「服务进度通知」的 `thing13/phrase16/thing5/time12` 字段发送一次。声音状态不再影响“数字分身训练完成”。
- 自动化继续只用 HTTP 桩和本地媒体验证，禁止创建真实 speaker/avatar/video 任务，避免消耗石榴点数。已部署军师预发 `6ca6a64` 与 AIStar `e9e8e43c-20260811T153721Z`；两服务 active、`NRestarts=0`，requirements 仍为 `authorizationVideoRequired=false / avatarMin=5 / voiceMin=3`，微信 avatar 模板在线可见。server 包 `c6c11ae` 已 auto-preview 到 AppID `wx810ebe6dfef8e75f`。预发 db push 再次短暂重建 `knowledge_chunk.embedding_vec`，随后 `pgvector.sql` 已从 JSON 回填，2/2 条向量恢复；生产未修改。
...
- 文案页移除会把真机消息压成竖条的内嵌深色工作台，改为底部右侧 62px 圆形「AI 改稿」入口；点开 82% 屏高独立对话抽屉，消息固定纵向堆叠，快捷建议和输入框各自占一行，关闭后 AI 更新的成稿继续留在原文区域。新全屏业务层以 `video-script-ai` 成对登记/释放 overlay，不再被宿主导航遮挡。
- 配画面不再要求用户从整篇脚本猜起句和止句：首屏直接显示系统默认分好的画面段，每段提供「调整组合」和「与下一段合并」。调整层只展示当前段的句子，默认全选；取消勾选的句子自动单独成段，保留勾选必须连续，也可一键全部拆开。合并已配不同素材的相邻段会先确认并清空冲突素材，避免静默串错画面。
- 镜头纯函数新增当前段重组与相邻段合并，锁住完整连续覆盖、非连续勾选阻断、全取消拆分和冲突素材清空；原生测试 93/93、共享前端测试 103/103 通过，server 包构建指向 `https://wxapi.aibuzz.cn/api_preprod`。本轮没有调用生成接口，不消耗石榴点数。

### 2026-08-11 · 「快出片」按移动生产力产品重做视觉层级 · 影响面：视频分包设计令牌、首页、模板、文案、配画面、确认、生成与成片页

- 依据 `mobile-app-ui-design` 的任务优先、拇指区、8 点间距和 60/30/10 色彩原则重建设计系统：底色收成轻暖灰，主文字改为中性墨黑，暖橙只用于主动作和分身，蓝色只用于素材画面；卡片统一 20px 圆角、软暖阴影和至少 44px 点击区，不再用满屏米黄与密集细边框制造同权重信息。
- 首页首屏只解释“选模板 → 聊文案 → 配画面”的核心价值，返回用户紧接上次进度；模板成为主任务，数字分身下沉为增强能力。模板详情改为竖屏海报与关键信息同屏，三种成片结构按 01/02/03 扫读，底部仅保留一个主按钮。
- 文案页移除占高的固定工具栏，以深色 AI 对话工作台作为第一视觉焦点，快捷改写下沉为次级动作；稿件统一称“段”，阅读态像文章、编辑态才展开操作。配画面页把报价移出固定页头，画面卡按范围顶对齐，圈选入口和当前方案形成清晰两级结构。
- 确认页用“准备完成 → 成片摘要 → 费用明细 → 确认扣费”建立高风险动作的信任顺序；生成页补云端运行状态、可见进度条和阶段卡，成片页用完成反馈承接 peak/end moment。继续保持预发 mock 媒体，不为 UI 走查触发石榴生成。
- 原生端测试 90/90、共享前端测试 103/103 通过；server 包已用微信开发者工具 `auto-preview` 推到 AppID `wx810ebe6dfef8e75f`，构建身份为 `native-weapp / server / https://wxapi.aibuzz.cn/api_preprod / 79e6a9e`。本轮只有端上视觉与进度派生调整，军师/AIStar 预发服务无需重部署，也未发起视频生成。

### 2026-08-11 · 「快出片」文案改为 AI 对话共创，画面支持连续多句共用 · 影响面：视频项目契约、原生分包、军师 BFF、AIStar worker/报价/总装

- 文案页新增可持续追问的「AI 文案搭档」：结合模板、当前稿与最近对话追问信息或直接生成 6～10 个完整语义段；用户/助手消息与应用状态随项目保存，生成后仍可逐段手改、试听和继续对话，不再只有一次性整段改写。
...
- 军师预发新增显式 `CLIP_MEDIA_MODERATION_BYPASS=true` 测试旁路：只允许非生产环境，上传仍校验媒体类型并写独立审计；production 启动时与模拟支付等测试开关一起硬拒绝，不能误带上线。
- AIStar clip 预发新增显式测试媒体模式：保留真实石榴配置但只在隔离预发强制生成确定性素材，作品仍真实经过逐段 ffmpeg、字幕、常驻 AI 标识、固定尾卡、音轨归一、质量门、封面和存储，并永久烧录「测试演示」，不再以状态假成功替代成片。
- 该模式用于测试阶段先走通上传、授权、分身、报价、积分预扣/结算、生成、作品与发布状态等产品链；真实石榴本人素材、机器审核和四平台发布仍是生产放量门槛。
- 已部署军师预发 `718a9f1` 与 AIStar clip 预发 `1dc23695-20260811T095733Z`。公网 BFF 实测一笔任务扣 10 个预发测试积分并成功结算，得到 44.05 秒、720×1280、H.264/AAC、1,072,189 字节成片与缩略图；抽帧确认「测试演示」「AI 生成」和字幕均已烧录，抖音 mock 发布状态已回写。测试引擎未请求石榴、未消耗供应商点数。
- 新 server 包已用微信开发者工具 `auto-preview` 推到 AppID `wx810ebe6dfef8e75f`，构建身份为 `native-weapp / server / https://wxapi.aibuzz.cn/api_preprod / 718a9f1`。本次部署仍出现既有预发专属 `knowledge_chunk.embedding_vec` 丢列警告；生产库与生产服务未修改。

### 2026-08-11 · 「快出片」预发补齐固定尾卡与成片音画质量闸 · 影响面：AIStar clip 总装、预发验收、跨仓状态文档

- AIStar clip v0.113 在缺运营素材时生成三套模板各自的固定品牌尾卡，最终音轨统一归一到 -16 LUFS / -1.5 dBTP；入库前用 ffmpeg `signalstats + loudnorm` 校验平均亮度、综合响度和真峰值，静音、黑白片、过轻/过响、爆音或解析异常全部失败关闭。
- `deploy-clip-preprod.sh` 现在上传前读取远端 env 中的 ffmpeg/ffprobe 路径并硬检 `signalstats`、`metadata`、`loudnorm`，防止缺滤镜的宿主带病发布；版本 `1eaa2c4d-20260811T090307Z` 已部署隔离 clip 预发，3 模板公网 BFF 验收为 200，生产发布号保持 `89941f4`。
- 预发宿主完成 180 秒 720×1280 合成长片探针：单音轨、94.78 MiB、平均亮度 125.50、-16.05 LUFS、180 个亮度采样均可解析；该探针只验证基础设施，不替代本人授权素材的质量/时延/成本验收。Chrome 复查仍是目标账号族下 RAM 用户，但缺 `yundun-greenweb:GetAuditUserConf`，媒体上传继续 fail-closed。

### 2026-08-11 · 「快出片」补齐阿里云多模态机器审核适配器 · 影响面：视频 BFF、媒体上传、本人授权、预发配置

- 新增阿里云内容安全增强版真实适配器：本地图片、视频、语音先使用官方临时 OSS 凭证隔离上传；图片同步审核，视频/语音按 `280/288 → 200` 轮询任务结果，全部在小程序 180 秒上传预算内收口。
- 风险等级只放行 `none/low`；`medium` 因当前没有人工复核队列而与 `high` 一样阻断。配置缺失、权限/欠费、返回结构异常、网络错误和轮询超时全部 fail-closed，并落不含素材正文与密钥的审核审计。
- 新增 `CLIP_MEDIA_MODERATION_PROVIDER` 与 `ALIYUN_GREEN_*` 完整配置面；默认仍为 `none`，不会误用现有文本审核或其它阿里云账号的 OSS/SMS 密钥。当前正确阿里云账号的内容安全尚未开通、Chrome 登录 RAM 身份无开通/配置权限，故预发仍显式返回 `CLIP_MEDIA_MODERATION_NOT_CONFIGURED`，待主账号开通并授予专用 RAM 身份后再落凭据实测。
- 增加 7 组单测，覆盖三类媒体、队列/处理中轮询、风险放行边界、权限错误、返回缺字段、超时、大图和不支持格式；原视频 BFF 积分/幂等回归保持通过。
...
- AIStar clip 后台按石榴官方 API v1 接入授权、音色/形象训练、TTS、文案/音频出片、轮询与删除；上游时效成片立即转存我方持久存储，并内置「为实体发声 / 今天开门了 / 这门手艺」三套模板。
- 新增 `clip-preprod` 隔离实例：只监听预发机 `127.0.0.1:8081`，与军师预发用独立高熵 service token 互通；公网只开放 `/clip_preprod/cdn|files/`，AIStar 生产服务与生产库未触碰。
- 石榴 key 已做只读验真且只保存于 0600 运行时 env；账号当前 12,000 点、没有已训练 speaker/avatar。真实图片/视频/音频机器审核尚未配置，因此预发可以浏览模板与验证 BFF/权限边界，但本人采集和完整真成片继续 fail-closed，不能用 mock 冒充验收。

### 2026-08-10 · 「快出片」收紧真机导航并内置三套完整模板 · 影响面：视频分包 UI、模板目录、mock 全流程

- 11 个视频页面统一复用微信胶囊导航几何，返回、标题和右侧动作落在同一导航层；删除原先 `navInset + 50px` 的重复占位，并统一标题左对齐、胶囊避让、返回按钮和导航分隔线。流程页固定底栏改为 `border-box`，并拆开会被旧基础库整条丢弃的 `constant()` padding shorthand，不再让时长贴边或把按钮推出屏幕。
- 新增包内 `catalog.js` 模板事实源；「为实体发声」「今天开门了」「这门手艺」分别内置 14/8/10 句独立脚本、镜头提示与固定尾段，首页明确显示内置模板数量。
- 首页模板接口失败或返回空列表、详情接口暂不可用时仍可浏览包内目录；mock 的列表、详情、建项目和重置脚本统一从目录生成，避免模板消失或三张卡实际共用同一脚本。
- 新增模板结构、防御性副本与 11 页单层导航回归测试；真实 BFF 登录态仍不静默回退 mock，创建和出片继续服从运行时数据源边界。
- 纯 mock 视频会话补 200 点演示额度，默认无套餐的新 mock 账号不再在确认页被 0 余额挡住；演示额度只在 `api.isMock()` 生效，附身 JWT 与 server 包继续使用真实军师余额。
- 同步修正视频素材注释：`wx.chooseMedia` 不是主包新增 API，本线新增的是 video mediaType 与相关权限/大文件真机验收面。

### 2026-08-10 · 智能体启用弹层不再被自定义底栏遮挡，真机预览 skill 对齐原生构建 · 影响面：原生弹层、custom tabbar、开发验收工具

- `agent-unlock` 随弹层数据成对登记/释放 `store.setOverlay('agent-unlock')`，组件销毁时兜底清理；微信独立 custom tabbar 在弹层打开期间会真正隐藏，不再压住启用按钮与底部内容。
- 原生守卫新增解锁弹层 overlay 生命周期断言；真机预览 skill 改读 `dist-native/`、`WEAPP_APP_*` 与可覆盖 worktree 路径，删除 Taro `dist/`、`TARO_APP_*` 等过时指令。

### 2026-08-10 · 「快出片」视频子应用完成可审计 M1 工程骨架 · 影响面：原生小程序分包、视频 BFF、积分、跨系统身份与合规闸

- 点兵新增「快出片」入口与独立分包，补齐模板、文案、逐段配画面、权威报价确认、克隆、素材、任务、作品和分身管理；页面只经 `host.js` 接宿主，真实模式只连军师 BFF，不向第三方发送军师 JWT。
- 服务端新增 `/api/video/**`、AIStar service-token 网关、外部属主隔离、文本/媒体审核闸、路由限流及积分 hold/settle/refund；服务端报价变化或并发重复提交不会重复扣费、建单或错误退款。
- AIStar 侧新增独立 clip 类型、四表迁移、模板/preset 管理、项目与素材、克隆授权、报价/预检、数据库租约 worker、stale reaper、作品与四平台发布契约；真实石榴契约、媒体审核、ffmpeg/CDN 总装和平台发布均失败关闭，不以 mock 产物冒充生产结果。
- 当前未部署、未上传微信平台。上线仍受微信类目/深度合成备案、石榴 M0 十项尽调与商务条款、真实媒体机审、真实发布接入及 iOS/Android 真机验收约束。

### 2026-08-10 · PC 问策支持上传资料 · 影响面：问策输入框、文档上传、消息引用与历史回显
...
- 生产已发布应用提交 `146cff5` 到 `/var/www/junshi/copilot`；阿里云 DNS A 记录 `copilot → 8.136.36.175` 生效，`https://copilot.aibuzz.cn/` 与同源 `/api/health` 公网通过。Let’s Encrypt 证书有效期至 2026-11-08，systemd 每日续期检查已启用且 dry-run 成功。

### 2026-08-10 · PC 工作台改为强制登录 · 影响面：PC 首屏、登录态验真、线上数据访问边界

- PC 未登录时不再渲染导航轨、五区内容或游客公开目录，只显示手机号 + 短信验证码登录屏；登录层没有关闭按钮，遮罩与 Esc 均不能绕过。
- 存量 token 先用线上 `/me` 验真，完成前只显示核验状态；失效 token 自动清理并退回登录，网络失败停在可重试页，避免个人工作区短暂闪现。移动 H5 与微信小程序的游客浏览策略不变。
- PC 启动不再提前请求公开军师目录或个人接口，成功登录 / 验真后才加载；新增源码守卫锁定硬登录门与禁止游客预拉。

### 2026-08-10 · PC 工作台沙盘、点兵、主公完整落地 · 影响面：PC 五区主界面、桌面执行闭环、账户权益与响应式

- 沙盘从占位切为真实三视图：经营战局读取军师档案、战略案卷、结构化三势、证据计数、待补事实、决策账本与当日经营回填；可刷新三势、打开全解抽屉、认可判断并生成今日军令。时运策/命盘分析按命理开关与真实命盘分态，不用假数据填空。
- 点兵补齐今日战役卡组、第 0 号军令、桌面军令表、单条/批量完成与删除、战果回填、手动新增、CSV 导出、经营日回填、动态内容出品；周计划支持四层目标写回与近 7 天记录，复盘接三势自评、决策验证、行业周指标、复盘账本与提醒。后端没有军令改期接口，「顺延到明天」明确标「施工中」且绝不假写成功。
- 主公补齐账户服务大卡、权益三格、年度谶语、案卷/方案/资料统计、战略段位、档案工作台抽屉和档案/资产/系统全部菜单；新增方案与权益、算力账本两个 PC 子视图，购买能力严格服从 `planOptions.canPurchase`，微信支付交回既有移动支付链。
- 跨区的案卷补问与复盘上下文通过内存草稿预填问策，不写 URL/本地存储、不代用户发送；创作军师沿用 `pc/portraits.ts` 立绘，字体沿用仓库 `/fonts`，未复制设计项目素材。
- 修正工作区重复套 `Stage` 导致的滚动继承/双抽屉：外壳只保留单层 Stage，切区/子视图回到顶部。新增 PC 区域真数据源与边界脚本守卫；mock 浏览器已验 1440×900、1024×768，无整页横向溢出，军令表在窄主区内独立横滚。

### 2026-08-10 · 军师连续主线、真人教练感与渐进交付闭环 · 影响面：总军师对话、长期事实、报告路由、多图、摘要与监控

针对用户反馈“隔天再问同类方案时不像以前聪明、少了理解感和人情味”，完成方案 A0～D：先消除会话断链，
再把事实可信度、输出意图、复杂交付和多图处理做成可恢复的工程链路；server/admin/H5 已部署生产，
小程序代码已提交但尚未上传微信平台，主观体验仍以真机盲测为准。

- 总军师闲置 24 小时默认继续同一 Session，只在真实用户消息前画可重算章节；详情尾页 100 条并用
  `(createdAt,id)` 游标向上翻。逃生开关关闭时改建新 Session，但仍用 lineage + handoff 继承决策与最近脉络。
- 摘要从含糊 `caughtUp` 升级为六态覆盖信息，接近 350 项滚动合并并用数据库版本锁防多实例覆盖；新增
  pending/capped/compaction 指标、Prometheus 告警和默认只读的存量摘要预热脚本。
- 提示词运行时边界增加真人教练式表达：接住具体处境、说清做得好与风险、给明确路径，惊喜来自因果连接和
  反直觉洞察；评测默认 rubric 同步检查客户事实、教练感、具体性和 AI/客服腔。
- 报告否定意图优先且全链只用一个解析真源；`requestedOutput` 与 `deliveryMode` 分离，复杂候选的模型评分
  作为 job 下 classification attempt 留痕，失败默认单次交付；后续阶段只有用户点击才幂等建单。
- 新增可审计 `UserFact`：用户原话直接 asserted；资料抽取和军师新推断只进 pending，并以下一轮独立确认卡
  核对；确认/修改/仅本次不会制造聊天消息，更正会 supersede 旧事实。Memory 强化不再改写原始 `createdAt`。
- 单条图片上限开放到 9，服务端每批最多 4；原件保留并生成最长边 2048px 推理副本，按 12MB/批、24MB/轮
  二次闸门。阅图 attempt 持久化观察和 usage，重启只补缺失批次，单批失败明确图号并继续其余图片。
- Schema 全为加法；发布前置、`sharp` Linux 安装、摘要预热、逃生开关与不可逆删表禁令已写入 DEPLOYMENT。
- 全量回归补齐摘要告警卡知识与 `UserFact` 清库顺序，并修复 Agent 自定义 OpenAI 接入在全局池开启时被
  悄悄改路由的问题：探活与真实生成现都 `poolBypass`，trace 不再误归因到全局端点。
- 发布口径收紧：自动化全量为 server 1530/1530、app 63/63 + 100/100、admin 75/75；开发者工具载包成功
  只证明包可运行，不等于新后端已在真机生效。真实模型新旧盲测与 1,000+ 消息真机滚动列为放量硬闸。
- 生产首次摘要预热暴露两层韧性缺口：辅助模型 700 output token 可能截断批量 JSON，且任一条 kind/source
  格式不规范会被顶层严格 Zod 放大成整批失败。失败批次按设计不推进游标、不覆盖原摘要；抽取与滚动合并
  现分别显式给 4,000 / 8,000 token，并逐条容错后再走不变的 kind/文本/来源真实性闸，坏条丢弃、好条保留；
  服务器内脱敏复现最终确认主因是模型把客户原话双引号写进 JSON string 时未转义，统一解析器现先严格解析、
  仅语法失败时用 `jsonrepair` 修复，再执行 Zod 与来源闸。
  生产 schema 为纯加法，发布前备份 `/var/backups/junshi/junshi-2026-08-10-1932.sql.gz`
  已校验 80 张表；最终业务修复版本 `5f3ae26` 已部署，schema diff 为空、API 与数据库健康、
  admin/H5 公网 200、Prometheus 57 条规则已加载。全部 23 条长主线预热均达到 `caught_up`，
  0 失败；小程序仍未上传微信平台，不将服务端上线冒充为真机主观验收。

### 2026-08-09 · 全系统错误语义与重试规则收口 · 影响面：原生/H5 请求与上传、对话、全局提示、运营后台

从服务端业务码反查三端后确认：此前全局层只认识登录、网络和未开通方案；额度/算力、方案到期、限流、
审核、参数、冲突、404 等会被页面 fallback 盖成「操作失败/请重试」，原生 5xx/SSE 还可能暴露服务端原文，
H5 文档上传会丢 `code/statusCode`。运营后台另有少量写操作吞掉 403 的具体权限原因、读取失败误装成空数据。

- 原生/H5 新增镜像 `api-error` 分类器，统一 message/action/retryable；方案、额度与算力给可执行入口，
  审核/参数/限流/冲突/不存在不再给必失败的立即重试，网络/超时/可恢复服务异常才允许重试。
- JSON、SSE、四类 H5 multipart 上传和原生上传统一 HTTP 降级：408/504 超时、429 限流、5xx 安全文案，
  业务 `code/data` 继续透传，技术原文只留 `technicalMessage`。
- 核心 C 端路由（会话、军师、案卷、方案、记忆）的 4xx/404 补齐中文业务原因与机器码；流式链路把
  `code/statusCode` 一路带到错误卡，避免 H5 在 SSE/HTTP 失败时重新退化成只看文案。
- 修掉“失败却报成功/空数据”的写入与读取：手动存方案、处方状态、入局档案/天势保存不再吞错；
  品牌资产、下一步与专项建议加载失败均与真实空态分开并可重试，速诊问卷断网使用本地兜底。
- 运营后台评测测试集与用户服务区分失败/空态，所有本轮审出的写操作 catch 透出服务端 `e.message`，
  保留 403 owner/agent 权限原因；新增 `docs/ERROR_HANDLING.md` 作为错误矩阵与新增码检查清单。
- 双端新增分类、402、算力、限流、参数和 5xx 回归；聊天错误卡扩展「查看算力」入口。

### 2026-08-09 · 对话额度错误改为可执行提示 · 影响面：原生问策 + H5 对话失败态

真机 402 原本显示「请求失败（402）+ 重新回答」；额度已经耗尽时原样重试必然再次失败，既没解释原因，
也把用户引向死循环。另一个直接原因是微信 `enableChunked` 在部分基础库把 4xx JSON 作为
`ArrayBuffer` 返回，原生请求层未解码，导致服务端 `INSUFFICIENT_QUOTA` 业务码丢失。

- 原生请求层为字符串、`ArrayBuffer` 与 TypedArray 统一解析 JSON，保留 4xx 的业务错误码。
- 原生与 H5 新增对话错误语义映射：额度耗尽 / 未开通 / 已到期改为明确原因 +「查看方案（与权益）」；
  网络、超时等可恢复错误才显示重试，内容审核失败要求修改内容而非原样重试。
- 新增双端回归，锁定「402 不退化成 HTTP_402」「额度耗尽不可重试」「网络错误仍可重试」。

### 2026-08-09 · 修复停止回复后续发持续 402 · 影响面：原生问策对话 + 持久生成额度结算

真机复现：AI 回复到一半点停止，紧接着再发消息会报「请求失败（402）」，点「重新回答」仍失败。
根因不是新消息内容，而是 `running` 任务的软取消只写 `cancelRequestedAt`；其 Token 预留要等 worker
最终收尾才释放。旧任务若恰好占完可用额度，新任务虽然已获会话让位，仍会被冻结预留拦截。

- `requestGenerationCancel` 对运行中任务改为在 job 行锁事务内：标记取消并立即释放未结算预留；job 的
  `quotaReserved` 归零但保持待结算，worker 随后仍按 provider 真实 usage 扣费，避免“停止免单”。
- 原生 `chat-core` 保存取消请求 Promise；停止后立即续发时先等服务端取消事务确认，避免 cancel/send
  两条请求在网络上乱序。
- 扩展 `generationJob` 集成回归：构造旧任务占完余额，验证停止返回即恢复可用额度、下一轮建单成功，
  且旧任务最终实际消耗仍准确入账；原生静态闸门同步锁定取消确认顺序。

### 2026-08-08 · 老板页服务双卡对齐修正 · 影响面：profile 双卡样式（H5 与原生共用）

真机：「老师微信」与「班级群」两卡没对齐——右卡标题偏高、左卡图标偏高。根因是**结构不对称**：
群卡横排（图标对「标题+次行」整块垂直居中），老师卡纵排且图标参与首行排版，首行被 26px 图标
撑高 → 标题中心比群卡低约 4.5px、图标中心又高约 2px。

- 老师卡图标改为绝对定位、对整卡垂直居中，两行文本由卡片 `padding-left: 42px` 给同一条左基线
  （删掉两处 `padding-left: 33px` 的行内缩进）——**不动 DOM**，H5 与原生共用同一份 scss，一处修两端。
- 两个图标底盒统一为 26px / 圆角 9 / 居中；群卡去掉「四点码眼」时代残留的 `padding: 4px` 与
  `flex-wrap`，群图标 18→16 与消息图标做光学配平；原生侧的 `.sa-qr` 局部覆盖随之删除。
- 新增静态断言钉死「双卡内部节奏一致」；app 静态 62 + src 93 全绿，原生与 H5 构建均通过。

### 2026-08-08 · 首次入局教学层适配问策终态浮岛 · 影响面：coach-marks + 问策终态

真机：五步教学层的面板下半截（含「下一步」按钮）被底部浮岛盖住。根因是教学层的底部留白按
66px 旧底栏写死（`padding-bottom: safe-area + 122px`），而终态浮岛约 159px，且浮岛 z=100 高于
教学层 z=90。

- `coach-marks` 新增 `coachstate` 事件，开合广播给宿主页。
- 问策终态收到后：浮岛收成**纯 tab 行**（输入行 + 细线让位）、提示 pill 收起、对话区底部留白同步收紧；
  tab 行保留——教学箭头指的就是底栏，这也是原设计「底栏必须可见可点」的口径。
- 静态断言：新增教学层让位一测；顺带把两条过脆的断言改为「条件集合/允许条件渲染」写法
  （原先逐字匹配整串 `wx:if`，每加一个隐藏条件就误红）。
- app 静态 61 + src 93 全绿；`build:weapp:server` 绿。

### 2026-08-08 · 微信审核驳回整改：登录页去除平台品牌元素 · 影响面：原生登录弹层 + 协议页 + 静态闸门

驳回原文：「登录页面或弹窗（调用手机号快速验证组件的前置页面），存在混淆腾讯官方的元素，包括但不限于
『微信』字样、微信官方 logo，请去除相关元素，如：将手机号授权登录提示修改为『手机号快捷登录』」。

- 登录弹层：主按钮文案「微信手机号一键登录」→「手机号快捷登录」，去掉品牌 SVG 图标；
  返回入口「返回微信账号登录」→「返回手机号快捷登录」；补档页去掉「使用微信头像/昵称」表述；
  JS toast 文案一并中性化。**能力未降级**——主按钮仍是 `open-type="getPhoneNumber"`。
- 删除 `weapp-native/assets/brand-icons/`（两个 Simple Icons WeChat SVG）。
- 协议页同步按钮新名（登录弹层链出去的页面同受约束）；隐私政策里对第三方服务的事实披露保留。
- **推翻旧约定**：AGENTS.md 原「登录页必须使用固定版本 Simple Icons WeChat 品牌 SVG」已作废，
  §7.2 改为反向红线；`npm test` 的对应用例同步反转为「登录弹层不得命中『微信』且不得存在品牌图形」。
- app 静态 56（原 54，反转 1 条 + 保留其余）+ src 93；`build:weapp:server` 绿，产物实测零命中。

### 2026-08-08 · 修复运营后台存量套餐保存时 `null.trim()` 崩溃 · 影响面：server 套餐后台 API + 回归测试

`Plan` 的商业字段为兼容迁移期仍允许数据库存 `null`，但 `GET /admin/plans` 先前直接透传 Prisma 行，违反 `AdminPlan` 的非空契约；运营编辑存量档后保存时，表单对 `planFamilyKey` / `usageLabel` 调 `.trim()` 因而在浏览器崩溃。后台套餐响应现统一复用 `planRules` 补齐 family、档位、用量等级/名称和状态阈值；新增 nullable 存量行的列表与回填保存回归测试。无需改库，正常保存该档后会把补齐值写回。

### 2026-08-08 · 问策终态真机三修（停止后卡死 / 历史行铺屏 / 资料库混入附卷） · 影响面：生成任务让位逻辑 + 问策终态 + 资料库

真机走查三条反馈，根因各不相同：

- **【P0】发送 → 停止 → 再发送，石沉大海**：`running` 态取消是软取消（只写 `cancelRequestedAt`，
  落终态等 worker 那一拍），窗口内重发被 `GENERATION_IN_PROGRESS` 挡回。改为：`createGenerationJob`
  对已请求取消的在途任务**让位**（清 `activeGenerationId` 放行新任务，老任务 finalize 的条件更新抢不回会话）。
  端上配套：thinking 阶段按停止时移除那条空气泡（否则与下一轮 thinking 点叠成两个「军师 ···」）；
  仍收到 409 时接管该 generation 而不是报错。新增服务端用例钉死「取消未终结不得挡住下一条消息」。
- **【P1】历史抽屉行文字铺满半屏**：CSS 三行省略是对的，但微信 `<text>` 把内容里的 `\n` 当真换行渲染，
  `white-space:nowrap` 管不住。改在数据层折行（`oneLine()` 统一空白），省略仍交给 CSS。
- **【P1】资料库混进对话粘贴的长文**：粘贴归卷本就写 `sourceType='paste'` 的 KnowledgeItem，服务端也一直
  在返回该字段。端上按来源分组：主列表只放主动上传，附卷收进可展开的「对话附卷 · N 份」——**不隐藏**
  （它们仍被会话引用，用户得能找到和删除）。存量无字段的按标题启发式归类，不写迁移。
- 测试：app 静态 54（+3 回归闸门）+ src 93；server 1480（+1）；`build:weapp:server` 绿。
