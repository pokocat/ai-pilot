...
> 状态：**已实施并上线**（阶段 B/C/D 均已落地；本文自 2026-07-29 起转为设计依据与决策记录，不再是待办清单）  
> 日期：2026-07-28 初稿 · 2026-07-29 按代码现状评审修订 · 2026-08-12 状态订正  
> ⚠️ 本文若与代码冲突，以代码与 `AGENTS.md` §8.4 为准。已上线且本文未覆盖的后续演进：AI 排版引擎（第 3 档）、
> 影像主导模式与 13 档风格库、作品库、**看图打磨闭环**（2026-08-12）——详见 `docs/CHANGELOG.md`。  
...
执行步骤（★ 2026-07-29 改：**AI 排版引擎为默认，三套模板降为回落路径**。原文写的是「模板是唯一引擎」，
那一版上线后真机实测画质不合格——见下方「实际落地」）：

1. 基于 PosterBrief、品牌资产和用户历史生成“视觉哲学”（走现有服务端 LLM 池，token 消耗计入任务成本，不额外扣用户 token 额度）；
2. ~~把视觉哲学约束为结构化 `CanvasSpec`~~ → **不做**（那是「第 2 档：结构化规格驱动固定渲染器」，已被否）。
   改为：把哲学写成 4–6 段中文**宣言**，交给模型直接产出整页 HTML/CSS（第 3 档，最接近上游 skill）；
3. AI 引擎路径**不调图片模型**（图形与排印即艺术，也省一笔生图成本）；模板回落路径仍按需调用；
4. 使用 HTML/CSS/SVG 构建海报（AI 路径由模型自己写整页，模板路径用白名单模板）；
5. 复用 `reportPdf.ts` 的浏览器单例/队列/超时骨架，新增 `page.screenshot` 生成 PNG（现有管线只做 PDF，截图是新增的函数级能力，不是新依赖）；
6. 需要 PDF 时复用浏览器打印管线；
7. 校验尺寸、文字溢出和文件完整性；**AI 路径另加渲染后量测 + refine 闭环**（下条）；
8. 上传 OSS；
9. 更新任务、成果消息和资产列表。

**实际落地（2026-07-29，`layoutEngine` 默认 `'ai'`）**：

```text
manifesto(LLM #1) → canvasEngine(LLM #2 产出整页 HTML)
  → 静态审计（只拒不洗）→ 占位符替换 + AI 标识注入
  → 渲染（js 禁用 + 请求白名单）+ 量测（越界/边距/字号/重叠/文案在场/二维码/占位符残留）
  → **无条件打磨一轮**（上游 "take a second pass"）→ 最多再修一轮（HTML 相关 LLM ≤3 次，整段 ≤180s）
  → 任一步走不通 → 回落三套模板的完整逻辑（含图片供应商与 degraded 留痕）
```

因此本文其余章节里凡是「三套模板是唯一引擎」「文字全部由确定性渲染器排版」的表述，
**只适用于模板/回落路径**；AI 路径的文字由模型排版、由量测器把关（不出画、不重叠、字号与边距下限、
主标题原文必须在场、AI 标识必须可见）。机制细节与违规码表见 `AGENTS.md` §8.4「AI 排版引擎」。
...
★ 2026-07-29 补：**排版引擎重新有了一个真开关，但它在 payload 里、不是 env**——`layoutEngine: 'ai' | 'template'`
（缺省 `'ai'`）。它与被删掉的 `CANVAS_DESIGN_ENGINE` 的区别正是当初删那个 env 的理由：
① 它有真实分支（`worker.runPipeline` 按它走 AI 引擎或模板路径），不是一个只改标签字面值的假旋钮；
② 它在后台改、约 1 分钟生效，不需要 SSH + 重启；③ 它不是风险开关——AI 引擎失败会自动回落模板，
所以运营能安全地开着它。`CreativeJob.engine` 列仍恒为 `'native'`（那是任务模型的实现引擎，两者别混）。
...
- ★ 2026-07-29：渲染**模型生成的 HTML** 时必须走加固形态——`renderHtmlToPng` 的 `javaScriptEnabled:false`
  + `allowUrlPrefixes`（只放行 `data:` 与 OSS 签名域）+ `domScan`（量测）三个 **opt-in** 参数。
  它们默认关闭，所以报告 PDF 与模板海报的行为一字未变；反过来说，**新增任何「渲染不可信 HTML」的调用点
  都必须显式打开它们**，否则等于把无头浏览器当执行环境用。`domScan` 的函数必须自包含（会被序列化进页面）。
...
★ 2026-07-29 起，**这三套模板是回落路径**（`layoutEngine='template'`，或 AI 排版引擎失败时自动走它）。
默认路径由模型自己写整页 HTML/CSS，没有固定版式；下面「所有模板必须满足」的那张清单同时被
**量测器**逐条执行到 AI 产物上（不出画、不重叠、二维码静区 ≥4px 且 ≥64px、Logo 不拉伸、字体只用镜像内置栈、
不引用会过期的外链），违反即把带 selector 与实测数值的 critique 回喂给模型重排——
也就是说这张清单从「模板作者的自律」变成了「机器闸门」。
