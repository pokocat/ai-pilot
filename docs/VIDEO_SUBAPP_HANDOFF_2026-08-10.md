...
> **2026-08-11 AI 改稿修复**：旧链路用辅助调用默认 700 token 生成 6–10 段 JSON，真实输出被截断后回落到“重排原稿但声称已更新”，预发两次换品类请求因此假成功。现在改用 Zod 结构化输出、2400 token 和一次修复轮；换行业/产品必须清掉旧主体。两轮仍失败则明确保持原稿，`applied=false`，禁止假成功。
> 修复已部署军师预发 `3b0ea28`，当前真机 server 包直接生效；部署没有主动调用模型。

> **2026-08-11 首页/成片/字幕增量**：首页不再先铺大介绍、纵向草稿和三张大模板；首屏先给“我的作品 / 素材库”，随后展示数字分身开拍门槛。模板详情仍可浏览，但未 ready 时点击“开始制作”会引导创建或查看训练进度，不再先建项目后让用户返回补分身。草稿压为进度行，模板改横滑紧凑卡。成片页使用真实 9:16 容器。AIStar v0.126 对同一画面承载的多句文案保留逐句 caption cue，按真实段音频等比分配时间窗并依次烧录，画面共用不再导致整段字幕同时出现或两行截断。
> 隔离预发 `3560b942-20260811T170602Z` 已关闭 force-mock 并保持 active/running、`NRestarts=0`；军师 server 真机包 `385c863` 已指向 `https://wxapi.aibuzz.cn/api_preprod` auto-preview。

> **2026-08-11 石榴 v0.119 增量**：官方 `/avatar/create` 的 `speakerId` 是“选填，用于制作 demo”，`authId` 也只在确需授权校验时选填，因此数字人主链已改为“上传一段视频 → 云端训练”。AIStar 在没有现成音色时会从视频原声 best-effort 创建基础 V2 声音，失败不阻断形象；单独录音/上传音频是分身管理中的可选增强。形象 ready 即视为创建完成。训练页支持用户主动订阅微信 32308 服务进度通知，后台只轮询既有任务，不会额外消耗石榴点数。
...
> **2026-08-11 石榴 v0.117 增量**：采集页先读服务端 requirements；时长硬门只按石榴官方限制执行——声音真实时长 `>2s`（端上提示 3 秒）、授权/形象视频 `>=5s`，较长时长仅作为软建议。BFF + AIStar ffprobe 双层阻断格式、大小、编码等真正错误素材。授权文案、`authId` 语义、speaker/avatar 真实进度和错误码已对齐官方文档；声音固定 V2.0，整片 avatar/b-roll 统一由段 TTS 音频驱动。重新采集支持只重做失败项。自动化只跑桩，不创建真实任务、不耗点数；本人真机素材是最终体验验收。
...
> **执行状态（更新至 2026-08-12）**：本文列出的代码任务已在隔离 worktree 完成；前端、军师 BFF、AIStar clip 域和针对性测试均已落地。AIStar v0.119 已对齐石榴官方单视频 Avatar 创建、采集硬门（声音 `>2s`、视频 `>=5s`）、可选声音增强、V2 音频驱动视频、状态进度、错误码与时效结果转存，并完成多段 ffmpeg 总装、三套固定品牌尾卡、字幕/AI 标识、音轨归一和音画质量门。隔离预发仍可显式 force-mock 做零点数工程验收；切真实供应商后应只由用户本人真机提交素材。军师 BFF 已实现媒体审核适配器和训练完成微信通知；测试期旁路仍逐素材留审计，production 只有 `CLIP_MEDIA_MODERATION_BYPASS + CLIP_MEDIA_MODERATION_ALLOW_PRODUCTION` 双开关才允许运营临时放开。真实上线仍缺：目标阿里云账号内容安全授权、本人素材质量/耗时/成本实测、授权群像尾片或运营 preset、真实平台发布、微信类目与深度合成备案。后续以 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md` 顶部“执行结果”和两仓 TODO 为当前事实。
