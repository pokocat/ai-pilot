...
> **执行状态（2026-08-10）**：本文列出的代码任务已在隔离 worktree 完成；前端、军师 BFF、AIStar clip 域和针对性测试均已落地。石榴官方真实原子链路与三套模板已部署隔离预发：AIStar 仅监听同机 127.0.0.1:8081，军师预发 BFF 通过独立 service token 回源。真实 key 已验真但账号当前没有已训练分身/音色；视频/音频机器审核、本人素材质量实测、完整多段 ffmpeg 总装、真实平台发布、微信类目与深度合成备案仍是门槛，不能宣称全流程或生产验收。后续以 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md` 顶部“执行结果”和两仓 TODO 为当前事实。
