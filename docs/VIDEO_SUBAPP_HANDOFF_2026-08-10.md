...
> **执行状态（更新至 2026-08-11）**：本文列出的代码任务已在隔离 worktree 完成；前端、军师 BFF、AIStar clip 域和针对性测试均已落地。石榴官方真实原子链路与三套模板已部署隔离预发：AIStar 仅监听同机 127.0.0.1:8081，军师预发 BFF 通过独立 service token 回源。AIStar v0.113 已完成逐段 avatar/b-roll 标准化、多段 ffmpeg 总装、三套运行时固定品牌尾卡、字幕、全程 AI 标识、最终音轨归一、亮度/响度/真峰值失败关闭和缩略图；180 秒 720×1280 合成探针已在预发宿主通过。军师 BFF 已实现阿里云内容安全图片同步审核和视频/语音异步审核适配器。外部仍缺：目标阿里云账号开通内容安全并授予当前/专用 RAM 身份、本人授权素材实测（石榴账号当前没有已训练分身/音色）、授权群像尾片或运营 preset、真实平台发布、微信类目与深度合成备案。因此预发媒体上传继续 fail-closed，不能宣称完整真人分身或生产验收。后续以 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md` 顶部“执行结果”和两仓 TODO 为当前事实。
