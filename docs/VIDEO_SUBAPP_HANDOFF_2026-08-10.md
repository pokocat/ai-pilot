...
> **2026-08-11 增量**：文案首步已加入可持续追问的 AI 对话共创；项目新增独立 `shots` 镜头计划，配画面可圈选连续句范围共用一个素材。报价、预检和 AIStar 生成/总装已统一按 shot 聚合，不再逐句强制切画面。AI 文案走军师 LLM，不消耗石榴点数。

> **创建**：2026-08-10。上一个 session 已完成前端框架搭建 + 技术方案 + 一轮 code review，
> 本文是**自包含交接书**：读完本文 + 引用的文档，不需要上一个 session 的对话记录即可继续开发。

> **执行状态（更新至 2026-08-11）**：本文列出的代码任务已在隔离 worktree 完成；前端、军师 BFF、AIStar clip 域和针对性测试均已落地。石榴官方真实原子链路与三套模板已部署隔离预发：AIStar 仅监听同机 127.0.0.1:8081，军师预发 BFF 通过独立 service token 回源。AIStar v0.114 已完成逐段 avatar/b-roll 标准化、多段 ffmpeg 总装、三套运行时固定品牌尾卡、字幕、全程 AI 标识、最终音轨归一、亮度/响度/真峰值失败关闭和缩略图；隔离预发还可显式生成永久带「测试演示」的可播放测试 MP4，以便在没有本人采集素材时走完整产品链。军师 BFF 已实现阿里云内容安全图片同步审核和视频/语音异步审核适配器；测试阶段预发显式开启非生产审核旁路并留审计，production 启动硬拒绝。真实上线仍缺：目标阿里云账号内容安全授权、本人授权素材实测、授权群像尾片或运营 preset、真实平台发布、微信类目与深度合成备案。后续以 `docs/VIDEO_SUBAPP_PLAN_2026-08-10.md` 顶部“执行结果”和两仓 TODO 为当前事实。
