...
mock 模式默认开着，不用起后端。PC 现在有不可关闭的硬登录门，直接用 mock 手机号登录即可；
server 模式则必须使用目标环境的真实手机号 + 短信验证码。不要再通过手写 localStorage 绕过登录，
存量 token 也必须先经 `/me` 验真后才会挂载工作区。
...
| 分支 | `feat/pc-workbench`（基线为 main 的 `89941f4`） |
| 基线提交 | `e070796` 问策 + 脱 Taro · `54dd3c6` 预览端口 · `0601fba` 锦囊 + 外壳修复 |
| 当前状态 | 五大区主工作区已落地；本轮沙盘、点兵、主公及文档改动均只在此 worktree |
| 主干边界 | **不要把 PC 代码写回主干**；所有后续修改仍在此 worktree 完成 |
...
- **沙盘**：经营战局、三势判断与全解抽屉、判断依据、待补事实、决策验证入口、今日经营回填、风险锁、时运策与命盘摘要
- **点兵**：今日 / 周计划 / 复盘三视图、案卷军令批量完成 / 顺延 / 删除、手动补令、战果与经营数据回填、复盘检查、内容出品入口
- **主公**：账户与会员权益、年度谶语、资产与战略统计、档案 / 资产 / 系统菜单、方案概览、算力账本
- **登录**：手机号 + 短信验证码硬登录门；未登录不挂载五区与公开目录，存量 token 先经 `/me` 验真，`pc/authBridge.ts` 保留为运行中掉线的第二道防线
- **支付**：PC 展示真实方案关系；下单仍交给移动 H5 / 微信小程序安全链路

### 已知边界（下一个人接着验证）

1. 问策未做项：反问选项（`ChatReply.asks`）、事实确认卡、引用角标、历史向上翻页、附件上传。
2. 案卷只有 `date`，没有服务端“顺延原因 / 新日期”动作；点兵表的顺延明确显示「施工中」，没有伪造成功。
3. PC 不直接接微信支付；方案按钮会把用户带到既有移动支付页。
4. 接真后端回归（见第 5 节）。

设计稿原始 HTML 与用户上传的 handoff 压缩包都可作为视觉依据，取法见第 6 节。
...
**⑥ 主工作区只能有一个 `Stage`。** `App.tsx` 已经用 `Stage` 包住每个区的 `Main`，区组件只返回
`.pc-page` 内容。再在沙盘 / 点兵 / 主公内部套 `Stage` 会继承错误滚动位置、出现双抽屉；
`pc-workbench-regions.test.mjs` 已加守卫，`Chrome.tsx` 也会在换区 / 换视图时把主区滚动归零。

---

## 5. 验证边界（接真后端时必须逐条回归）

**已验证（mock）**：1440×900 与 1024×768 都走过沙盘、点兵、主公及子视图；页面没有整页横向溢出，
点兵宽表只在自身容器内横向滚动；跨区「补清事实 → 问策」能带入草稿但不会擅自发送；抽屉保持单实例；
浏览器控制台没有运行时错误。mock 验证的是布局、状态和交互，不等于真接口验收。
...
**沙盘 / 点兵 / 主公**：仍需用 server 模式逐项回归 `refreshForces`、`battleCommit`、案卷军令写操作、
经营回填、复盘、资产统计、方案关系与算力明细；重点核对 401 / 403 / 套餐到期错误是否沿用全局错误口径。

**全局**：1024 与 1440 已验证；超宽屏与系统缩放组合尚未专项测试。全程 mock，尚未连真后端。

---

## 6. 设计稿来源与取法

设计稿是用户在 Claude Design 里画好的，**不是我们编的**。本轮用户还上传了可直接交接的压缩包：

- `/Users/donis/Downloads/军师小程序H5转PC版适配-handoff.zip`
...
| 问策 · 对话 | 183–267 | ✅ 已实现 |
| **沙盘** | **270–389** | ✅ 已实现，并按真实契约扩展时运策 / 命盘分析 |
| **点兵** | **392–538** | ✅ 已实现，并按案卷契约扩展周计划 / 复盘 |
| 锦囊 | 541–725 | ✅ 已实现 |
| **主公** | **728–785** | ✅ 已实现，并扩展方案 / 算力子视图 |
...
右键菜单项与快捷键标注。维护沙盘/点兵时先读这一段，能直接看出每个字段的预期形态和文案口径，
