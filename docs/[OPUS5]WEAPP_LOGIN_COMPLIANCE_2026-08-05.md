...
|---|---|---|---|
| 微信账号标识 | `components/Login` 的 `Taro.login` | 用户主动点「微信账号登录」，服务端用 code 换 openid/unionid 建立账号 | 声明用于账号登录与身份识别；不写成手机号登录 |
...
当前代码**没有** `getPhoneNumber`、`getUserProfile`、`getUserInfo`、`getDeviceInfo`、读取剪贴板或位置接口；手机号仅由用户主动选择短信登录/绑定并手动输入。除非后续代码真实启用，否则后台不要申报这些接口。小程序内《隐私政策》已同步为同一口径。
...
**C. 手机号最小必要**
- [ ] 微信登录（`wx.login`）成功后**不弹绑定手机号**，直接进入
- [ ] 全流程"仅浏览 + 微信登录"路径中，`getPhoneNumber` 一次都不被调起
- [ ] 主动去「设置」绑定手机号时，有「暂不绑定」可退出
