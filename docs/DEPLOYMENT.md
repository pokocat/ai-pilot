...
新版本的读路径默认先走归一化表；四张表为空或 chat 路由不可用时会回落旧表，故迁移窗口内不会直接停摆。
但**后台从这个版本起只写新表**，旧表不会再同步，所以首次发布必须按 §E2 在开放后台写操作前完成迁移和就绪检查。
`AI_CONFIG_V2=false` 只会读一份不再更新的旧表历史快照，适合短时救急，不是长期回滚方案。
...
| # | 位置 | 该看到什么 | 不对的话说明 |
|---|---|---|---|
| 1 | 页面结构 | 三层：**接入点**（上游参数）+ **路由**（六个用途同构配置）+ **凭证**（确认厂商/轮换 Key）。不应再有旧「快速切换 / 已添加模型 / 端点池」分区 | 还是旧分区＝构建产物没更新 |
...
| 10 | 路由 · 嵌入/重排 | 只允许对应厂商确实支持的 OpenAI 兼容端点；选 Anthropic 或七牛端点时保存被拒并说明原因 | 保存成功＝用途校验没接上 |
| 11 | 路由 · 用途预算 | 可分别保存超时 / 正文 token / 温度，清空后恢复系统默认 | 不显示或清空无效＝预算契约/写路径没接上 |
| 12 | 凭证 | 迁移或自定义凭证标「待确认」并可选厂商/自定义确认；确认前加入路由应被拒 | 只能换 Key 不能确认＝迁移闭环未完成 |

第 9、10 两条是**故意造错来验拦截**，验完记得改回去。

### E2. 归一化接入配置首次迁移（重设计三期 · 首次发布必做）

> 背景与设计见 `docs/[OPUS5]AI_CONFIG_REDESIGN_2026-08-07.md`。新版本后台只写归一化表，
> 因此首次发布不能长期跳过本步骤；短暂为空时虽会回落旧快照，但后台一旦改配置，两边就开始分叉。
...
③ 启动新版本后打开后台「模型」页，或直接 `GET /api/admin/ai-v2-status`：
   - `ready` 必须为 `true`（chat 路由有可用端点）。**`false` 时切过去就是把 AI 关掉**，别切。
   - 有「待确认接入商」的凭证就先在凭证区确认；迁移本身会保留现有路由，但确认前不能做新的路由变更。

④ 无需再设置开关：`AI_CONFIG_V2` 默认就是 `true`。请求和探活确认运行时命中 `ai_endpoint` id。
   若切换当天必须短时逃生，可显式设 `AI_CONFIG_V2=false` 后重启；此时读的是发布前历史快照，期间后台新改动不会出现，排障后应尽快恢复 `true`。
...
>
> **预发记录（2026-08-08 彩排时）**：`AI_CONFIG_V2=false`、`AI_PROBE_SCHEDULED=false`
> ——预发带的是**生产真实 key**，定时探活会持续产生真实费用，非生产环境不该常开。
> 该 `false` 不再等同代码默认值；下次验证应先确认迁移状态，再恢复 `true` 并重启。
...
- **Nginx 前置条件（2026-08-09 实测踩到）**：本项目 **H5 挂在 `aibuzz.cn`、API 与小程序合法域名是 `wxapi.aibuzz.cn`，是两个 server 块**。H5 用相对路径不受影响，但小程序给的是 `https://wxapi.aibuzz.cn/fonts/…` 绝对地址，而那个 server 块原本只有 `/api/`、`/admin/`、`/grafana/`，压根没有 `/fonts/` → 首次部署后真机字体 404。已在两个 server 块都加 `location /fonts/ { root /var/www/junshi/h5; }`（模板同步在 `deploy/nginx.conf.example`）。**症状很隐蔽：H5 一切正常、只有小程序回退系统字体，且机型间时有时无。**
