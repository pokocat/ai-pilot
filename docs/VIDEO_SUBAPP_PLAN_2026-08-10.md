...
> **状态**：M1 工程、石榴真实原子链路、阿里云多模态机审适配器与完整多段总装已落地，隔离预发仍因阿里云内容安全未开通而对真实媒体上传失败关闭；真实发布与生产合规门槛未完成。
> **创建**：2026-08-10
...
- 军师 BFF 已实现阿里云内容安全增强版图片/视频/语音审核：本地文件通过官方临时 OSS 凭证上传，图片同步判定，视频/语音轮询异步任务；只放行 `none/low`，`medium/high`、配置/权限/欠费、超时和异常返回全部 fail-closed。当前正确阿里云账号尚未开通内容安全，已登录 RAM 用户也无开通/配置权限，所以预发默认 `provider=none`，待主账号开通与专用调用身份授权后才能做本人素材验收。
- AIStar 预发已完成逐段 avatar/b-roll/尾段标准化、真实 TTS 时长、H.264/AAC 多段总装、可选低音量 BGM、字幕与全程 AI 标识、输出音轨/时长校验和作品缩略图；固定正式尾片素材、亮度/响度阈值和长视频压力验收仍需在真实素材阶段补齐。
...
- 非生产 mock 可以闭环演示；纯 `api.isMock()` 会话使用 200 点演示额度，避免主应用默认无套餐的新 mock 账号在确认页被 0 余额挡住。附身 JWT / server 模式不使用演示额度。生产若石榴正式契约、真实媒体审核或 ffmpeg/CDN 总装未完成，会分别以明确 503/任务失败退出。代发也保持 `CLIP_PUBLISH_NOT_CONFIGURED`，不以“记录一条假发布”冒充成功。
- 端内新增 `catalog.js` 作为三套体验模板的唯一事实源；列表、详情、mock 建项目与重置脚本共用同一目录，每套模板都有独立文案和镜头结构。11 个页面的自定义导航已收成与微信胶囊同层，不再重复叠加 50px 标题栏。
- 外部上线门槛仍包括：军师小程序类目/重新提审与深度合成备案、阿里云内容安全开通和专用 RAM 调用身份、本人合规素材的 §3.2 质量/时延/规格/成本实测、7 项商务/授权决策、固定正式尾片/质量阈值、四平台真实发布接入与双端真机验收。
...
| 02 首页 | `home/` | 做实 |
| 03 模板详情 | `template/` | 做实 |
| 04 克隆向导 ①②③④ | `clone/` | 四步在一页内切，做实；人脸核验与提交训练是桩 |
...
| 4 | 视觉基调：快出片的暖橙 vs 军师的墨绿米白 | 同一小程序内两套色系 | 已按设计稿实现暖橙；需产品确认能否接受 |
