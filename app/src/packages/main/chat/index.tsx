...
export default function Chat() {
  ...
  const acceptPlan = async (d: Deliverable, messageId?: string) => {
    ...
    setTimeout(() => Taro.switchTab({ url: '/pages/junling/index' }), 620);
  };
  ...
  return (
    <View className={`page chat ${s.themeClass()}`} style={{ '--keyboard-height': `${keyboardHeight}px`, '--jump-bottom': `${jumpBottom}px` } as ChatStyle}>
      ...
      <SafeHeader
        className="chat-head"
        rightReserve={false}
        left={<View className="safe-hbtn" onClick={() => Taro.navigateTo({ url: '/pages/sessions/index' })}><Icon name="chat" size={19} color="#565C63" /></View>}
      >
