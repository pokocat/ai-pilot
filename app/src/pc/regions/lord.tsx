...
function Credits({ st }: { st: PcState }) {
  ...
  return (
    <div className="pc-page pc-lord-page">
      <section className="pc-lord-credit-hero"><div><div className="pc-lord-eyebrow">算力账本</div><h1>{me?.creditBalance && me.creditBalance < 0 ? '不限量' : me?.creditBalance ?? 0}</h1><p>当前可见算力余额 · 本月已用 {me?.usage?.usagePercent ?? 0}%</p></div><button type="button" className="pc-btn" onClick={() => st.setView('plans')}>查看方案</button></section>
