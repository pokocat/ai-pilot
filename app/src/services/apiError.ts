export type ApiErrorKind =
  | 'unauthorized' | 'network' | 'cancelled'
  | 'plan_required' | 'plan_expired' | 'quota' | 'credits'
  | 'moderation' | 'rate_limited' | 'conflict' | 'not_found'
  | 'validation' | 'unavailable' | 'blocked' | 'other';

export type ApiErrorAction = 'login' | 'plans' | 'credits' | 'edit' | 'retry_later' | 'refresh';
...
const CONFLICT_CODES = new Set([
  ...
  'ORDER_NOT_PAYABLE', 'ORDER_EXPIRED', 'JOB_NOT_RETRIABLE', 'WECHAT_ACCOUNT_CONFLICT',
]);
...
export function apiErrorCode(error: unknown): string {
  return String((error as any)?.code || (error as any)?.data?.code || '');
}

function errorStatus(error: unknown): number {
  const value = Number((error as any)?.statusCode || (error as any)?.status);
...
function readableBusinessMessage(message: unknown): string {
  const value = String(message || '').trim();
  if (!value || !/[\u3400-\u9fff]/.test(value)) return '';
  if (/\b(?:Error|Exception|Prisma|Fastify|SQL|stack|undefined|null)\b/i.test(value)) return '';
  return value;
}

export function httpErrorInfo(statusCode: number, data: unknown, noun = '请求'): { message: string; code?: string; technicalMessage?: string } {
  ...
  return {
    message: USER_MESSAGES[body.code || ''] || readable || `${noun}未能完成，请检查后再试。`,
    code: body.code,
    technicalMessage: readable ? undefined : technical,
  };
}

/** 所有 C 端错误的语义入口；页面只提供本动作 fallback，不再自行猜 HTTP 状态或重试性。 */
export function apiErrorPresentation(error: unknown, fallback = ''): ApiErrorPresentation {
  const code = apiErrorCode(error);
  const status = errorStatus(error);
  const message = USER_MESSAGES[code]
    || readableBusinessMessage(rawMessage(error))
    || fallback
  ...
  if (code === 'NETWORK_ERROR') return { kind: 'network', message, retryable: true };
  if (code === 'CANCELLED') return { kind: 'cancelled', message, retryable: false };
  ...
  if (RETRYABLE_CODES.has(code) || status === 408 || status >= 500) return { kind: 'unavailable', message, retryable: true };
  if (code === 'AGENT_LOCKED' || code === 'SKU_REQUIRED' || code === 'FEATURE_DISABLED' || code === 'AI_CONFIG_INVALID') return { kind: 'blocked', message, retryable: false };
  return { kind: 'other', message, retryable: status === 0 };
}
