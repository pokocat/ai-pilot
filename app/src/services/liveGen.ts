...
// key（临时 key 与真实 sessionId 皆可）→ entry；entries 为去重后的实体集合（供列表遍历）。
const byKey = new Map<string, LiveGenEntry>();
const entries = new Set<LiveGenEntry>();

// 列表页订阅：任意 entry 生灭都通知，用于「军师推演中」标记的实时增删。
const subs = new Set<() => void>();
function notify() { subs.forEach((f) => { try { f(); } catch { /* noop */ } }); }
export function subscribeLiveGen(fn: () => void): () => void {
  subs.add(fn);
...
export function startLiveGen(p: LiveGenStartParams): string {
  ...
  if (p.sessionId) register(entry, p.sessionId);
  entries.add(entry);
  notify();
  void drive(entry);
  return entry.key;
...
// —— 列表页标记 ——
export function liveGenActiveSessionIds(): Set<string> {
  const out = new Set<string>();
  entries.forEach((e) => { if (e.stage === 'active' && e.sessionId) out.add(e.sessionId); });
  return out;
}
// 仅收「尚未绑定 sessionId」的推演（新会话首个 session 事件到达前的窗口期）。
// 已绑定的推演一律走 liveGenActiveSessionIds 精确匹配——否则同一军师名下的
// 其他历史会话行会在整轮生成期间被误标为「军师推演中」。
export function liveGenActiveAgentKeys(): Set<string> {
  const out = new Set<string>();
  entries.forEach((e) => { if (e.stage === 'active' && !e.sessionId && e.agentKey) out.add(e.agentKey); });
  return out;
}
